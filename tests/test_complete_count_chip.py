"""Read-only Mission Control complete count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "complete_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the complete-count chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "COMPLETES " + UNAVAILABLE
        assert view["label"] != "COMPLETES 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "COMPLETES 0"


def test_counts_mission_completed_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "COMPLETES 3"
    ...
    assert cases["ignoresSucceeded"]["count"] == 0
    assert cases["ignoresFinished"]["count"] == 0
    ...
    assert cases["oneCompleteNotPayload"]["label"] == "COMPLETES 1"
    ...
    assert unreadable["label"] == "COMPLETES " + UNAVAILABLE
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function completeCountView" in state
    assert "export function recordedCompleteCountFeed" in state
    assert 'const COMPLETE_COUNT_EVENT = "mission.completed"' in state
    view_body = state.split("export function completeCountView")[1].split("export function applyEvent")[0]
    assert "COMPLETE_COUNT_EVENT" in view_body
    ...
    assert "mission.succeeded" not in view_body
    assert "mission.finished" not in view_body
    assert "token_spent" not in view_body
    assert "payload.completes" not in view_body
    assert "state.preview?null:recordedCompleteCountFeed(eventLog,replayCursor,completeCountFeedLoaded)" in control
    assert "completeCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "completeCountFeedLoaded=options.completeCountFeedLoaded===true" in control
    assert "if(Array.isArray(events))" in control
    assert "completeCountFeedLoaded=true" in control
    assert 'id="completeCountChip"' in html
    chip = html.split('id="completeCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.complete-count")[1].split(".hud-stats")[0]
    assert '.hud-chip.complete-count[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_complete_count_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="completeCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function completeCountView" in state.text
