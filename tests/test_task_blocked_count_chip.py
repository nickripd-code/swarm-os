"""Read-only Mission Control task-blocked count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "task_blocked_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the task-blocked chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "TASK BLOCKS " + UNAVAILABLE
        assert view["label"] != "TASK BLOCKS 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "TASK BLOCKS 0"


def test_counts_task_blocked_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "TASK BLOCKS 3"
    assert "$" not in view["label"]
    assert "token" not in view["label"].lower()
    assert cases["ignoresMissionBlocked"]["count"] == 0
    ...
    assert cases["oneBlockNotPayload"]["label"] == "TASK BLOCKS 1"
    ...
    assert unreadable["label"] == "TASK BLOCKS " + UNAVAILABLE
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function taskBlockedCountView" in state
    assert "export function recordedTaskBlockedCountFeed" in state
    assert 'const TASK_BLOCKED_COUNT_EVENT = "task.blocked"' in state
    view_body = state.split("export function taskBlockedCountView")[1].split("export function applyEvent")[0]
    assert "TASK_BLOCKED_COUNT_EVENT" in view_body
    ...
    assert "payload.blocked" not in view_body
    assert "state.preview?null:recordedTaskBlockedCountFeed(eventLog,replayCursor,taskBlockedFeedLoaded)" in control
    assert "taskBlockedCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "taskBlockedFeedLoaded=options.taskBlockedFeedLoaded===true" in control
    assert 'taskBlockChip.textContent=taskBlockView.hidden?"":taskBlockView.label' in control
    assert "if(Array.isArray(events))" in control
    assert "taskBlockedFeedLoaded=true" in control
    assert 'id="taskBlockedCountChip"' in html
    chip = html.split('id="taskBlockedCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.task-blocked")[1].split(".hud-stats")[0]
    assert '.hud-chip.task-blocked[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_task_blocked_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="taskBlockedCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function taskBlockedCountView" in state.text
