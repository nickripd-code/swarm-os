...
def test_mean_includes_passed_and_failed_durations_not_the_newest(cases):
    ...
    failed = cases["failedOnly"]
    assert failed["count"] == 1
    assert failed["meanMs"] == pytest.approx(200)
    assert failed["label"] == "AVG VERIFICATION LATENCY 200ms"
    ...
    assert paired["label"] == "AVG VERIFICATION LATENCY 300ms"
    ...
    assert cases["ignoresOther"]["count"] == 1
    assert cases["ignoresOther"]["meanMs"] == pytest.approx(80)
    assert cases["ignoresOther"]["label"] == "AVG VERIFICATION LATENCY 80ms"
