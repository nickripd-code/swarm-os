"""Read-only Mission Control job-completed count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "job_completed_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the job-completed chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "JOB COMPLETED " + UNAVAILABLE
        assert view["label"] != "JOB COMPLETED 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "JOB COMPLETED 0"


def test_counts_job_completed_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "JOB COMPLETED 3"
    ...
    assert cases["ignoresMission"]["count"] == 0
    assert cases["ignoresTask"]["count"] == 0
    assert cases["ignoresBudget"]["count"] == 0
    assert cases["oneCompletedNotPayload"]["count"] == 1
    assert cases["oneCompletedNotPayload"]["label"] == "JOB COMPLETED 1"
    ...
    assert unreadable["label"] == "JOB COMPLETED " + UNAVAILABLE
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function jobCompletedCountView" in state
    assert "export function recordedJobCompletedCountFeed" in state
    assert 'const JOB_COMPLETED_COUNT_EVENT = "job.completed"' in state
    view_body = state.split("export function jobCompletedCountView")[1].split("export function applyEvent")[0]
    assert "JOB_COMPLETED_COUNT_EVENT" in view_body
    ...
    assert "payload.completed" not in view_body
    assert "state.preview?null:recordedJobCompletedCountFeed(eventLog,replayCursor,jobCompletedFeedLoaded)" in control
    assert "jobCompletedCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "jobCompletedFeedLoaded=options.jobCompletedFeedLoaded===true" in control
    assert 'jobChip.textContent=jobView.hidden?"":jobView.label' in control
    assert "if(Array.isArray(events))" in control
    assert "jobCompletedFeedLoaded=true" in control
    assert 'id="jobCompletedCountChip"' in html
    chip = html.split('id="jobCompletedCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.job-completed")[1].split(".hud-stats")[0]
    assert '.hud-chip.job-completed[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_job_completed_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="jobCompletedCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function jobCompletedCountView" in state.text
