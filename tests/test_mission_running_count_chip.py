"""Read-only Mission Control mission-running count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "mission_running_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the mission-running chip harness")
...
def test_catalog_event_is_mission_running():
    from app.events import EventType

    assert EventType.MISSION_RUNNING == "mission.running"
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "MISSION RUNNING " + UNAVAILABLE
        assert view["label"] != "MISSION RUNNING 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "MISSION RUNNING 0"


def test_counts_mission_running_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "MISSION RUNNING 3"
    ...
    assert cases["oneRunningNotPayload"]["count"] == 1
    assert cases["oneRunningNotPayload"]["label"] == "MISSION RUNNING 1"
    ...
    assert unreadable["label"] == "MISSION RUNNING " + UNAVAILABLE


def test_replay_prefix_does_not_include_later_running_events(cases):
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function missionRunningCountView" in state
    assert "export function recordedMissionRunningCountFeed" in state
    assert 'const MISSION_RUNNING_COUNT_EVENT = "mission.running"' in state
    view_body = state.split("export function missionRunningCountView")[1].split("export function applyEvent")[0]
    assert "MISSION_RUNNING_COUNT_EVENT" in view_body
    ...
    assert "state.preview?null:recordedMissionRunningCountFeed(eventLog,replayCursor,missionRunningFeedLoaded)" in control
    assert "missionRunningCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "missionRunningFeedLoaded=options.missionRunningFeedLoaded===true" in control
    assert 'missionRunningChip.textContent=missionView.hidden?"":missionView.label' in control
    assert "if(Array.isArray(events))" in control
    assert "missionRunningFeedLoaded=true" in control
    assert 'id="missionRunningCountChip"' in html
    chip = html.split('id="missionRunningCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.mission-running")[1].split(".hud-stats")[0]
    assert '.hud-chip.mission-running[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_mission_running_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="missionRunningCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function missionRunningCountView" in state.text
