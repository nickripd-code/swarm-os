import {lastEvidenceAtView, LAST_EVIDENCE_AT_NONE, LAST_EVIDENCE_AT_UNAVAILABLE, recordedEvidenceAtFeed} from "../app/static/state.mjs";

const cases = {};
const visible = {visible: true};

function view(feed, options = visible) {
  return lastEvidenceAtView(feed, options);
}

cases.unavailable = LAST_EVIDENCE_AT_UNAVAILABLE;
cases.none = LAST_EVIDENCE_AT_NONE;
...
cases.empty = view([]);
cases.cursorBefore = view(recordedEvidenceAtFeed([
  {id: 1, event_type: "verification.evidence.started", created_at: "2026-09-23T12:35:04Z"},
], -1, true));
cases.feedNotLoaded = view(recordedEvidenceAtFeed([
...
cases.newestByTime = view([
  {event_type: "verification.evidence.passed", created_at: "2026-09-23T12:40:00Z"},
  {event_type: "verification.evidence.started", created_at: "2026-09-23T12:10:00Z"},
  {event_type: "verification.evidence.failed", created_at: "2026-09-23T12:50:09Z"},
]);
cases.kinds = view([
  {event_type: "verification.evidence.started", created_at: "2026-09-23T01:02:03Z"},
]);
cases.verdictDoesNotOverride = view([
  {event_type: "verification.evidence.passed", created_at: "2026-09-23T12:35:04Z"},
  {event_type: "verification.failed", created_at: "2026-09-23T13:00:00Z"},
]);
cases.replayPrefix = view(recordedEvidenceAtFeed([
...
cases.replayAtLater = view(recordedEvidenceAtFeed([
  {id: 1, event_type: "mission.started", created_at: "2026-09-23T12:00:00Z"},
  {id: 2, event_type: "verification.evidence.started", created_at: "2026-09-23T12:35:04Z"},
  {id: 3, event_type: "verification.evidence.passed", created_at: "2026-09-23T12:36:00Z"},
], 2, true));
