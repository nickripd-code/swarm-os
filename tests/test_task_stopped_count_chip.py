"""Read-only Mission Control task-stopped count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "task_stopped_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the task-stopped chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "TASK STOPS " + UNAVAILABLE
        assert view["label"] != "TASK STOPS 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "TASK STOPS 0"


def test_counts_task_stopped_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "TASK STOPS 3"
    ...
    assert cases["oneStopNotPayload"]["count"] == 1
    assert cases["oneStopNotPayload"]["label"] == "TASK STOPS 1"
    ...
    assert unreadable["label"] == "TASK STOPS " + UNAVAILABLE


def test_replay_prefix_does_not_include_later_stops(cases):
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function taskStoppedCountView" in state
    assert "export function recordedTaskStoppedCountFeed" in state
    assert 'const TASK_STOPPED_COUNT_EVENT = "task.stopped"' in state
    view_body = state.split("export function taskStoppedCountView")[1].split("export function applyEvent")[0]
    assert "TASK_STOPPED_COUNT_EVENT" in view_body
    ...
    assert "payload.stopped" not in view_body
    assert "state.preview?null:recordedTaskStoppedCountFeed(eventLog,replayCursor,taskStoppedFeedLoaded)" in control
    assert "taskStoppedCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "taskStoppedFeedLoaded=options.taskStoppedFeedLoaded===true" in control
    assert 'taskStopChip.textContent=taskStopView.hidden?"":taskStopView.label' in control
    assert "if(Array.isArray(events))" in control
    assert "taskStoppedFeedLoaded=true" in control
    assert 'id="taskStoppedCountChip"' in html
    chip = html.split('id="taskStoppedCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.task-stopped")[1].split(".hud-stats")[0]
    assert '.hud-chip.task-stopped[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_task_stopped_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="taskStoppedCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function taskStoppedCountView" in state.text
