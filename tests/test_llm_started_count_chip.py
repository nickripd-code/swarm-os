...
def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the LLM-started chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "LLM START " + UNAVAILABLE
        assert view["label"] != "LLM START 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "LLM START 0"
...
def test_counts_llm_started_only(cases):
    ...
    assert unreadable["label"] == "LLM START " + UNAVAILABLE
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function recordedLlmStartedFeed" in state
    assert 'const LLM_STARTED_EVENT = "llm.started"' in state
    view_body = state.split("export function llmStartedCountView")[1].split("export function applyEvent")[0]
    assert "LLM_STARTED_EVENT" in view_body
    ...
    assert "state.preview?null:recordedLlmStartedFeed(eventLog,replayCursor,llmStartedFeedLoaded)" in control
    assert "llmStartedCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "llmStartedFeedLoaded=options.llmStartedFeedLoaded===true" in control
    assert "if(Array.isArray(events))" in control
    assert "llmStartedFeedLoaded=true" in control
    assert 'id="llmStartedCountChip"' in html
    chip = html.split('id="llmStartedCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    assert ".hud-chip.llm-started[data-known=\"false\"]" in css
    assert "animation" not in css.split(".hud-chip.llm-started")[1].split(".hud-stats")[0]


def test_static_llm_started_chip_is_served(tmp_path, monkeypatch):
