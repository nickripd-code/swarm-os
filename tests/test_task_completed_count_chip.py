"""Read-only Mission Control task-completed count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "task_completed_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the task-completed chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "TASKS " + UNAVAILABLE
        assert view["label"] != "TASKS 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "TASKS 0"


def test_counts_task_completed_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "TASKS 3"
    ...
    assert cases["oneCompletionNotPayload"]["count"] == 1
    assert cases["oneCompletionNotPayload"]["label"] == "TASKS 1"
    ...
    assert unreadable["label"] == "TASKS " + UNAVAILABLE
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function taskCompletedCountView" in state
    assert "export function recordedTaskCompletedCountFeed" in state
    assert 'const TASK_COMPLETED_COUNT_EVENT = "task.completed"' in state
    view_body = state.split("export function taskCompletedCountView")[1].split("export function applyEvent")[0]
    assert "TASK_COMPLETED_COUNT_EVENT" in view_body
    ...
    assert "state.preview?null:recordedTaskCompletedCountFeed(eventLog,replayCursor,taskCompletedFeedLoaded)" in control
    assert "taskCompletedCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "taskCompletedFeedLoaded=options.taskCompletedFeedLoaded===true" in control
    assert 'taskChip.textContent=taskView.hidden?"":taskView.label' in control
    assert "if(Array.isArray(events))" in control
    assert "taskCompletedFeedLoaded=true" in control
    assert 'id="taskCompletedCountChip"' in html
    chip = html.split('id="taskCompletedCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.task-completed")[1].split(".hud-stats")[0]
    assert '.hud-chip.task-completed[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_task_completed_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="taskCompletedCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function taskCompletedCountView" in state.text
