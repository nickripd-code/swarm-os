...
from app.llm import (
    ...
    MistralModelProvider, build_controller, build_model_provider, build_router,
)
...
def completed_chat(output=None, model="mistral-small-latest"):
...
@pytest.mark.asyncio
async def test_mistral_adapter_implements_provider_contract_and_tracks_usage(no_extra_providers):
    captured = {}
    ...
    provider = MistralModelProvider(api_key="mistral-secret", transport=mistral_transport(handler))
    assert isinstance(provider, ModelProvider)
    models = await provider.list_models()
    assert models[0].provider == "mistral"
    assert models[0].model == "mistral-small-latest"
    ...
    assert "mistral-secret" not in json.dumps(response.model_dump())
...
@pytest.mark.parametrize("response,expected,failure_class", [
    (httpx.Response(401, json={"error": {"message": "mistral-secret"}}),
...
async def test_mistral_errors_are_classified_and_do_not_leak(
        no_extra_providers, response, expected, failure_class):
    provider = MistralModelProvider(api_key="mistral-secret", transport=mistral_transport(lambda _: response))
    with pytest.raises(ProviderError, match=expected) as error:
        await provider.complete(REQUEST)
    assert "mistral-secret" not in str(error.value)
    assert error.value.failure_class == failure_class


@pytest.mark.asyncio
async def test_mistral_timeout_and_outage_are_classified(no_extra_providers):
    timeout_provider = MistralModelProvider(
        api_key="mistral-secret",
        transport=mistral_transport(lambda _: (_ for _ in ()).throw(httpx.ReadTimeout("timed out"))))
    ...
    outage_provider = MistralModelProvider(
        api_key="mistral-secret",
        transport=mistral_transport(lambda _: (_ for _ in ()).throw(httpx.ConnectError("offline"))))
    with pytest.raises(ProviderError, match="Could not reach Mistral") as outage_error:
...
@pytest.mark.asyncio
async def test_mistral_model_env_does_not_opt_in(monkeypatch, no_extra_providers):
...
async def test_custom_base_url_is_used(no_extra_providers):
    ...
    def handler(request):
        captured["url"] = str(request.url)
        return completed_chat(model="mistral-large-latest")

    provider = MistralModelProvider(
        model="mistral-large-latest",
        api_key="mistral-secret",
        base_url="https://mistral.internal/v1",
        transport=mistral_transport(handler),
    )
    await provider.complete(REQUEST.model_copy(update={"model": "mistral-large-latest"}))
    assert captured["url"] == "https://mistral.internal/v1/chat/completions"


def test_build_model_provider_openai_only_ignores_unconfigured_mistral(monkeypatch, no_extra_providers):
...
def test_build_router_registers_mistral_when_key_set(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    ...
    assert stacked.secondary.provider_id == "mistral"
    router = build_router()
    assert [item.provider_id for item in router.providers] == ["openai", "mistral"]


def test_build_router_cloud_plus_mistral_and_ollama(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    ...
    assert [item.provider_id for item in router.providers] == [
        "openai", "openrouter", "xai", "anthropic", "mistral", "ollama",
    ]
...
def test_build_model_provider_mistral_only(monkeypatch, no_extra_providers):
    monkeypatch.setenv("MISTRAL_API_KEY", "mistral-test")
    ...
    assert [item.provider_id for item in router.providers] == ["mistral"]


def test_openai_plus_openrouter_still_failover_when_mistral_set(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setenv("OPENROUTER_API_KEY", "or-test")
    monkeypatch.setenv("MISTRAL_API_KEY", "mistral-test")
    stacked = build_model_provider()
    ...
    assert stacked.secondary.provider_id == "openrouter"
    router = build_router()
    assert [item.provider_id for item in router.providers] == ["openai", "openrouter", "mistral"]


@pytest.mark.asyncio
async def test_router_outage_walks_to_mistral(no_extra_providers):
    from tests.test_router import openai_like, openrouter_like

    def handler(_request):
        return completed_chat(output={"answer": "from mistral"})
    ...
    mistral = MistralModelProvider(api_key="mistral-secret", transport=mistral_transport(handler))
    router = ModelRouter([openai, openrouter, mistral])
    response = await router.complete(CapabilityRequest(reasoning="high"), REQUEST)
    assert openai.calls == 1
    assert openrouter.calls == 1
    assert response.provider == "mistral"
    assert response.output == {"answer": "from mistral"}
    assert response.failover_from == "openai"
    assert response.failover_reason == "PROVIDER_OUTAGE"


@pytest.mark.asyncio
async def test_local_only_does_not_select_mistral(no_extra_providers):
    from tests.test_router import openai_like, openrouter_like

    mistral = MistralModelProvider(
        api_key="mistral-secret", transport=mistral_transport(lambda _: completed_chat()))
    ollama = OllamaModelProvider(model="llama3.2", transport=mistral_transport(lambda _: completed_chat()))
    router = ModelRouter([openai_like(), openrouter_like(), mistral, ollama])
    decision = await router.select(CapabilityRequest(privacy="local_only"))
    assert decision.selected.provider_id == "ollama"
    assert all(candidate.provider_id != "mistral" for candidate in decision.chain)


@pytest.mark.asyncio
async def test_unconfigured_mistral_is_skipped_and_cloud_still_serves(no_extra_providers):
    from tests.test_router import openai_like

    mistral = MistralModelProvider()
    openai = openai_like(output={"answer": "cloud"})
    router = ModelRouter([openai, mistral])
    response = await router.complete(CapabilityRequest(reasoning="high"), REQUEST)
    assert response.provider == "openai"
    assert openai.calls == 1
    with pytest.raises(ProviderError, match="not configured"):
        await mistral.complete(REQUEST)
