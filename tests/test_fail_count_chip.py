"""Read-only Mission Control fail count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "fail_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the fail-count chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "FAILS " + UNAVAILABLE
        assert view["label"] != "FAILS 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "FAILS 0"


def test_counts_mission_failed_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "FAILS 3"
    ...
    assert cases["ignoresJobFailed"]["count"] == 0
    ...
    assert cases["oneFailNotPayload"]["count"] == 1
    assert cases["oneFailNotPayload"]["label"] == "FAILS 1"
    ...
    assert unreadable["label"] == "FAILS " + UNAVAILABLE
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function failCountView" in state
    assert "export function recordedFailCountFeed" in state
    assert 'const FAIL_COUNT_EVENT = "mission.failed"' in state
    view_body = state.split("export function failCountView")[1].split("export function applyEvent")[0]
    assert "FAIL_COUNT_EVENT" in view_body
    ...
    assert "payload.fails" not in view_body
    assert "state.preview?null:recordedFailCountFeed(eventLog,replayCursor,failCountFeedLoaded)" in control
    assert "failCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "failCountFeedLoaded=options.failCountFeedLoaded===true" in control
    assert "if(Array.isArray(events))" in control
    assert "failCountFeedLoaded=true" in control
    assert 'id="failCountChip"' in html
    chip = html.split('id="failCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.fail-count")[1].split(".hud-stats")[0]
    assert '.hud-chip.fail-count[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_fail_count_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="failCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function failCountView" in state.text
