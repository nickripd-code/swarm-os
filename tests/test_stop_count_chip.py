"""Read-only Mission Control stop count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "stop_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the stop-count chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "STOPS " + UNAVAILABLE
        assert view["label"] != "STOPS 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "STOPS 0"


def test_counts_mission_stopped_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "STOPS 3"
    ...
    assert cases["oneStopNotPayload"]["label"] == "STOPS 1"
    ...
    assert unreadable["label"] == "STOPS " + UNAVAILABLE
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function stopCountView" in state
    assert "export function recordedStopCountFeed" in state
    assert 'const STOP_COUNT_EVENT = "mission.stopped"' in state
    view_body = state.split("export function stopCountView")[1].split("export function applyEvent")[0]
    assert "STOP_COUNT_EVENT" in view_body
    ...
    assert "payload.stops" not in view_body
    assert "state.preview?null:recordedStopCountFeed(eventLog,replayCursor,stopCountFeedLoaded)" in control
    assert "stopCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "stopCountFeedLoaded=options.stopCountFeedLoaded===true" in control
    assert "if(Array.isArray(events))" in control
    assert "stopCountFeedLoaded=true" in control
    assert 'id="stopCountChip"' in html
    chip = html.split('id="stopCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.stop-count")[1].split(".hud-stats")[0]
    assert '.hud-chip.stop-count[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_stop_count_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="stopCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function stopCountView" in state.text
