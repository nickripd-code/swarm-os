"""Read-only Mission Control LLM success-rate chip. Never invents a ratio."""
...
HARNESS = ROOT / "tests" / "llm_success_rate_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the LLM success-rate harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["rate"] is None
        assert view["label"] == "LLM SUCCESS RATE " + UNAVAILABLE
        assert view["label"] != "LLM SUCCESS RATE 0%"
...
def test_ratio_is_llm_completed_over_completed_attempts(cases):
    ...
    assert counted["label"] == "LLM SUCCESS RATE 50%"
    assert cases["completesOnly"]["completed"] == 2
    assert cases["completesOnly"]["failed"] == 0
    assert cases["completesOnly"]["rate"] == 1
    assert cases["completesOnly"]["label"] == "LLM SUCCESS RATE 100%"
    assert cases["half"]["rate"] == 0.5
    assert cases["half"]["label"] == "LLM SUCCESS RATE 50%"
    assert cases["allFailed"]["completed"] == 0
    assert cases["allFailed"]["failed"] == 2
    assert cases["allFailed"]["rate"] == 0
    assert cases["allFailed"]["label"] == "LLM SUCCESS RATE 0%"
    assert cases["third"]["label"] == "LLM SUCCESS RATE 66.67%"
    retries = cases["retriesIgnored"]
    assert retries["completed"] == 1
    assert retries["failed"] == 0
    assert retries["attempts"] == 1
    assert retries["label"] == "LLM SUCCESS RATE 100%"
    assert cases["skipsHoles"]["completed"] == 1
    assert cases["skipsHoles"]["failed"] == 1
    assert cases["skipsHoles"]["label"] == "LLM SUCCESS RATE 50%"
...
def test_unreadable_tiny_rate_stays_unavailable(cases):
    ...
    assert tiny["failed"] == 1000000
    assert tiny["attempts"] == 1000001
    assert tiny["rate"] is None
    assert tiny["label"] == "LLM SUCCESS RATE " + UNAVAILABLE
    assert tiny["label"] != "LLM SUCCESS RATE 0%"
...
def test_replay_prefix_uses_only_the_visible_llm_outcomes(cases):
    ...
    assert cases["prefix"]["label"] == "LLM SUCCESS RATE 50%"
    assert cases["prefixAll"]["completed"] == 2
    assert cases["prefixAll"]["failed"] == 2
    assert cases["prefixAll"]["label"] == "LLM SUCCESS RATE 50%"
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function llmSuccessRateView" in state
    assert "export function recordedLlmSuccessRateFeed" in state
    assert 'event_type === "llm.completed"' in state
    assert 'event_type === "llm.failed"' in state
    view_body = state.split("export function llmSuccessRateView")[1].split("export function applyEvent")[0]
    ...
    assert "state.preview?null:recordedLlmSuccessRateFeed(eventLog,replayCursor,llmSuccessRateFeedLoaded)" in control
    assert "llmSuccessRateView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "llmSuccessRateFeedLoaded=options.llmSuccessRateFeedLoaded===true" in control
    assert "if(Array.isArray(events))" in control
    assert "llmSuccessRateFeedLoaded=true" in control
    assert 'id="llmSuccessRateChip"' in html
    chip = html.split('id="llmSuccessRateChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert ".hud-chip.llm-success-rate[data-known=\"false\"]" in css
    assert "animation" not in css.split(".hud-chip.llm-success-rate")[1].split(".hud-stats")[0]


def test_static_llm_success_rate_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="llmSuccessRateChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function llmSuccessRateView" in state.text
