"""Read-only Mission Control fail-age chip. No invented timestamps."""
...
HARNESS = ROOT / "tests" / "fail_age_chip_cases.mjs"
UNAVAILABLE = "FAIL AGE unavailable"
...
def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the fail-age chip harness")
...
def test_no_mission_preview_or_not_failed_stays_hidden(cases):
    for name in (
        "idle",
        "noMission",
        "preview",
...
def test_recorded_fail_stamp_is_relative(cases):
    assert cases["known"] == {
        "hidden": False,
        "label": "Failed 4m ago",
        "known": True,
    ...
    for name in (
        ...
        "suspendKeepsFail",
        "answerKeepsFail",
        "llmFailKeepsMissionFail",
        "replayAt",
    ):
        assert cases[name]["label"] == "Failed 4m ago", name
    ...
    assert cases["offset"]["label"] == "Failed 4m ago"
    assert cases["offset"]["at"] == "2026-09-23T09:00:00+01:00"
    assert cases["seconds"]["label"] == "Failed 59s ago"
    assert cases["justNow"]["label"] == "Failed 0s ago"
    assert "just now" not in cases["justNow"]["label"]
    assert cases["hours"]["label"] == "Failed 2h ago"
    assert cases["oneDay"]["label"] == "Failed 1d ago"
    assert cases["micros"]["label"] == "Failed 59s ago"
    assert cases["skewOk"]["label"] == "Failed 0s ago"
    assert cases["noClock"]["label"] == "Failed " + STAMP
    assert "ago" not in cases["noClock"]["label"]
    assert cases["badNow"]["label"] == "Failed " + STAMP
    assert cases["zeroNow"]["label"] == "Failed " + STAMP
    assert "1970" not in cases["zeroNow"]["label"]


def test_blank_or_invalid_fail_time_is_unavailable(cases):
...
def test_chip_markup_is_hidden_until_the_shell_is_failed():
    ...
    assert ".hud-chip.fail-age.unavailable" in css
    block = css.split(".hud-chip.fail-age.unavailable")[1].split("}")[0]
    assert "animation" not in block


def test_static_fail_age_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="failAgeChip" class="hud-chip fail-age unavailable" hidden' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function failAgeChip" in state.text
