...
from app.llm import (
    ...
    CohereModelProvider, build_controller, build_model_provider, build_router,
)
...
def completed_chat(output=None, model="command-a-03-2025"):
...
@pytest.mark.asyncio
async def test_cohere_adapter_implements_provider_contract_and_tracks_usage(no_extra_providers):
    captured = {}
    ...
    provider = CohereModelProvider(api_key="cohere-secret", transport=cohere_transport(handler))
    assert isinstance(provider, ModelProvider)
    models = await provider.list_models()
    assert models[0].provider == "cohere"
    assert models[0].model == "command-a-03-2025"
    ...
    assert captured["url"] == "https://api.cohere.ai/compatibility/v1/chat/completions"
    assert captured["authorization"] == "Bearer cohere-secret"
    assert captured["body"]["model"] == "command-a-03-2025"
    assert captured["body"]["response_format"]["type"] == "json_schema"
    assert "json_schema" in captured["body"]["response_format"]
    assert "provider" not in captured["body"]
    assert "cohere-secret" not in json.dumps(response.model_dump())
...
@pytest.mark.parametrize("response,expected,failure_class", [
    (httpx.Response(401, json={"error": {"message": "cohere-secret"}}),
...
async def test_cohere_errors_are_classified_and_do_not_leak(
        no_extra_providers, response, expected, failure_class):
    provider = CohereModelProvider(api_key="cohere-secret", transport=cohere_transport(lambda _: response))
    with pytest.raises(ProviderError, match=expected) as error:
        await provider.complete(REQUEST)
    assert "cohere-secret" not in str(error.value)
    assert error.value.failure_class == failure_class


@pytest.mark.asyncio
async def test_cohere_timeout_and_outage_are_classified(no_extra_providers):
    timeout_provider = CohereModelProvider(
        api_key="cohere-secret",
        transport=cohere_transport(lambda _: (_ for _ in ()).throw(httpx.ReadTimeout("timed out"))))
    ...
    outage_provider = CohereModelProvider(
        api_key="cohere-secret",
        transport=cohere_transport(lambda _: (_ for _ in ()).throw(httpx.ConnectError("offline"))))
    with pytest.raises(ProviderError, match="Could not reach Cohere") as outage_error:
...
@pytest.mark.asyncio
async def test_cohere_model_env_does_not_opt_in(monkeypatch, no_extra_providers):
...
async def test_custom_base_url_is_used(no_extra_providers):
    ...
    def handler(request):
        captured["url"] = str(request.url)
        return completed_chat(model="command-r-08-2024")

    provider = CohereModelProvider(
        model="command-r-08-2024",
        api_key="cohere-secret",
        base_url="https://cohere.internal/compatibility/v1",
        transport=cohere_transport(handler),
    )
    await provider.complete(REQUEST.model_copy(update={"model": "command-r-08-2024"}))
    assert captured["url"] == "https://cohere.internal/compatibility/v1/chat/completions"


def test_build_model_provider_openai_only_ignores_unconfigured_cohere(monkeypatch, no_extra_providers):
...
def test_build_router_registers_cohere_when_key_set(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    ...
    assert stacked.secondary.provider_id == "cohere"
    router = build_router()
    assert [item.provider_id for item in router.providers] == ["openai", "cohere"]


def test_build_router_cloud_plus_cohere_and_ollama(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    ...
    assert [item.provider_id for item in router.providers] == [
        "openai", "openrouter", "xai", "anthropic", "mistral", "gemini", "cohere", "ollama",
    ]
...
def test_build_model_provider_cohere_only(monkeypatch, no_extra_providers):
    monkeypatch.setenv("COHERE_API_KEY", "cohere-test")
    ...
    assert isinstance(provider, CohereModelProvider)
    router = build_router(model_provider=provider)
    assert [item.provider_id for item in router.providers] == ["cohere"]


def test_openai_plus_openrouter_still_failover_when_cohere_set(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setenv("OPENROUTER_API_KEY", "or-test")
    monkeypatch.setenv("COHERE_API_KEY", "cohere-test")
    stacked = build_model_provider()
    ...
    assert stacked.secondary.provider_id == "openrouter"
    router = build_router()
    assert [item.provider_id for item in router.providers] == ["openai", "openrouter", "cohere"]


@pytest.mark.asyncio
async def test_router_outage_walks_to_cohere(no_extra_providers):
    from tests.test_router import openai_like, openrouter_like

    def handler(_request):
        return completed_chat(output={"answer": "from cohere"})
    ...
    cohere = CohereModelProvider(api_key="cohere-secret", transport=cohere_transport(handler))
    router = ModelRouter([openai, openrouter, cohere])
    response = await router.complete(CapabilityRequest(reasoning="high"), REQUEST)
    assert openai.calls == 1
    assert openrouter.calls == 1
    assert response.provider == "cohere"
    assert response.output == {"answer": "from cohere"}
    assert response.failover_from == "openai"
    assert response.failover_reason == "PROVIDER_OUTAGE"


@pytest.mark.asyncio
async def test_local_only_does_not_select_cohere(no_extra_providers):
    from tests.test_router import openai_like, openrouter_like

    cohere = CohereModelProvider(
        api_key="cohere-secret", transport=cohere_transport(lambda _: completed_chat()))
    ollama = OllamaModelProvider(model="llama3.2", transport=cohere_transport(lambda _: completed_chat()))
    router = ModelRouter([openai_like(), openrouter_like(), cohere, ollama])
    decision = await router.select(CapabilityRequest(privacy="local_only"))
    assert decision.selected.provider_id == "ollama"
    assert all(candidate.provider_id != "cohere" for candidate in decision.chain)


@pytest.mark.asyncio
async def test_unconfigured_cohere_is_skipped_and_cloud_still_serves(no_extra_providers):
    from tests.test_router import openai_like

    cohere = CohereModelProvider()
    openai = openai_like(output={"answer": "cloud"})
    router = ModelRouter([openai, cohere])
    response = await router.complete(CapabilityRequest(reasoning="high"), REQUEST)
    assert response.provider == "openai"
    assert openai.calls == 1
    with pytest.raises(ProviderError, match="not configured"):
        await cohere.complete(REQUEST)
