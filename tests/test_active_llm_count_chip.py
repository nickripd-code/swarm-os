"""Read-only Mission Control chip for model calls still in flight."""
...
HARNESS = ROOT / "tests" / "active_llm_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the active-llm chip harness")
...
def test_explicit_empty_feed_is_known_zero(cases):
    for key in ("empty", "beforeAny", "matchedComplete", "matchedFailed", "retryThenComplete", "ignoresNonLlm"):
        view = cases[key]
...
def test_chip_is_wired_to_the_objective_hud_only():
    ...
    assert 'id="activeLlmCountChip"' in html
    assert "hidden" in html.split('id="activeLlmCountChip"')[1].split(">")[0]
    assert html.index('id="activeLlmCountChip"') < html.index('id="hudElapsed"')
    assert "export function activeLlmCountView" in state
    assert "export function recordedActiveLlmFeed" in state
    assert 'type === "llm.started"' in state
    view_body = state.split("export function activeLlmCountView")[1].split("export function applyEvent")[0]
    ...
    assert "state.preview?null:recordedActiveLlmFeed(eventLog,replayCursor,activeLlmFeedLoaded)" in control
    assert "activeLlmCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "activeLlmFeedLoaded=options.activeLlmFeedLoaded===true" in control
    assert "if(Array.isArray(events))" in control
    assert "activeLlmFeedLoaded=true" in control
    assert ".hud-chip.active-llm[data-known=\"false\"]" in css
    assert "animation" not in css.split(".hud-chip.active-llm")[1].split(".hud-stats")[0]
    for banned in ("WorkQueue", "ProcessWorkerPool", "event-relay"):
        assert banned not in view_body
        assert banned not in control.split("activeLlmCountChip")[1].split("const running")[0]
