...
from app.health import together_status
from app.llm import (
    OllamaModelProvider, OpenAIResponsesModelProvider,
    TogetherModelProvider, build_controller, build_model_provider, build_router,
)
...
def together_transport(handler):
...
@pytest.mark.asyncio
async def test_together_adapter_implements_provider_contract_and_tracks_usage(no_extra_providers):
    captured = {}
    ...
    provider = TogetherModelProvider(api_key="together-secret", transport=together_transport(handler))
    assert isinstance(provider, ModelProvider)
    models = await provider.list_models()
    assert models[0].provider == "together"
    ...
    assert captured["authorization"] == "Bearer together-secret"
    ...
    assert "together-secret" not in json.dumps(response.model_dump())
...
@pytest.mark.parametrize("response,expected,failure_class", [
    (httpx.Response(401, json={"error": {"message": "together-secret"}}),
...
async def test_together_errors_are_classified_and_do_not_leak(
        no_extra_providers, response, expected, failure_class):
    provider = TogetherModelProvider(api_key="together-secret", transport=together_transport(lambda _: response))
    with pytest.raises(ProviderError, match=expected) as error:
        await provider.complete(REQUEST)
    assert "together-secret" not in str(error.value)
    assert error.value.failure_class == failure_class


@pytest.mark.asyncio
async def test_together_timeout_and_outage_are_classified(no_extra_providers):
    timeout_provider = TogetherModelProvider(
        api_key="together-secret",
        transport=together_transport(lambda _: (_ for _ in ()).throw(httpx.ReadTimeout("timed out"))))
    ...
    outage_provider = TogetherModelProvider(
        api_key="together-secret",
        transport=together_transport(lambda _: (_ for _ in ()).throw(httpx.ConnectError("offline"))))
    with pytest.raises(ProviderError, match="Could not reach Together") as outage_error:
...
async def test_health_unconfigured_when_key_missing(no_extra_providers):
    provider = TogetherModelProvider()
...
@pytest.mark.asyncio
async def test_together_model_env_does_not_opt_in(monkeypatch, no_extra_providers):
    monkeypatch.setenv("TOGETHER_MODEL", "Qwen/Qwen3.5-9B")
    provider = TogetherModelProvider()
    assert provider.configured() is False
...
async def test_custom_base_url_is_used(no_extra_providers):
    ...
    def handler(request):
        captured["url"] = str(request.url)
        return completed_chat(model="Qwen/Qwen3.5-9B")

    provider = TogetherModelProvider(
        model="Qwen/Qwen3.5-9B",
        api_key="together-secret",
        base_url="https://together.internal/v1",
        transport=together_transport(handler),
    )
    await provider.complete(REQUEST.model_copy(update={"model": "Qwen/Qwen3.5-9B"}))
    assert captured["url"] == "https://together.internal/v1/chat/completions"


def test_together_health_status_is_truthful_and_does_not_expose_key(monkeypatch, no_extra_providers):
    ...
    monkeypatch.setenv("TOGETHER_MODEL", "Qwen/Qwen3.5-9B")
    configured = together_status()
    assert configured["configured"] is True
    assert configured["model"] == "Qwen/Qwen3.5-9B"
    assert configured["fallback"] is False
    assert "together-secret" not in json.dumps(configured)


def test_build_model_provider_openai_only_ignores_unconfigured_together(monkeypatch, no_extra_providers):
...
def test_build_router_registers_together_when_key_set(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    ...
    assert stacked.secondary.provider_id == "together"
    router = build_router()
    assert [item.provider_id for item in router.providers] == ["openai", "together"]


def test_build_router_cloud_plus_together_and_ollama(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    ...
    assert [item.provider_id for item in router.providers] == [
        "openai", "openrouter", "xai", "anthropic", "mistral", "gemini", "cohere", "deepseek", "together", "ollama",
    ]
...
def test_build_model_provider_together_only(monkeypatch, no_extra_providers):
    monkeypatch.setenv("TOGETHER_API_KEY", "together-test")
    ...
    assert isinstance(provider, TogetherModelProvider)
    router = build_router(model_provider=provider)
    assert [item.provider_id for item in router.providers] == ["together"]


def test_openai_plus_openrouter_still_failover_when_together_set(monkeypatch, no_extra_providers):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setenv("OPENROUTER_API_KEY", "or-test")
    monkeypatch.setenv("TOGETHER_API_KEY", "together-test")
    stacked = build_model_provider()
    ...
    assert stacked.secondary.provider_id == "openrouter"
    router = build_router()
    assert [item.provider_id for item in router.providers] == ["openai", "openrouter", "together"]


@pytest.mark.asyncio
async def test_router_outage_walks_to_together(no_extra_providers):
    from tests.test_router import openai_like, openrouter_like

    def handler(_request):
        return completed_chat(output={"answer": "from together"})
    ...
    together = TogetherModelProvider(api_key="together-secret", transport=together_transport(handler))
    router = ModelRouter([openai, openrouter, together])
    response = await router.complete(CapabilityRequest(reasoning="high"), REQUEST)
    assert openai.calls == 1
    assert openrouter.calls == 1
    assert response.provider == "together"
    assert response.output == {"answer": "from together"}
    assert response.failover_from == "openai"
    assert response.failover_reason == "PROVIDER_OUTAGE"


@pytest.mark.asyncio
async def test_local_only_does_not_select_together(no_extra_providers):
    from tests.test_router import openai_like, openrouter_like

    together = TogetherModelProvider(
        api_key="together-secret", transport=together_transport(lambda _: completed_chat()))
    ollama = OllamaModelProvider(model="llama3.2", transport=together_transport(lambda _: completed_chat()))
    router = ModelRouter([openai_like(), openrouter_like(), together, ollama])
    decision = await router.select(CapabilityRequest(privacy="local_only"))
    assert decision.selected.provider_id == "ollama"
    assert all(candidate.provider_id != "together" for candidate in decision.chain)


@pytest.mark.asyncio
async def test_unconfigured_together_is_skipped_and_cloud_still_serves(no_extra_providers):
    from tests.test_router import openai_like

    together = TogetherModelProvider()
    openai = openai_like(output={"answer": "cloud"})
    router = ModelRouter([openai, together])
    response = await router.complete(CapabilityRequest(reasoning="high"), REQUEST)
    assert response.provider == "openai"
    assert openai.calls == 1
    with pytest.raises(ProviderError, match="not configured"):
        await together.complete(REQUEST)
