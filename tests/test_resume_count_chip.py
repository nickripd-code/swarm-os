"""Read-only Mission Control resume count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "resume_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the resume-count chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "RESUMES " + UNAVAILABLE
        assert view["label"] != "RESUMES 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "RESUMES 0"


def test_counts_mission_resumed_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "RESUMES 3"
    ...
    assert cases["oneResumeNotPayload"]["count"] == 1
    assert cases["oneResumeNotPayload"]["label"] == "RESUMES 1"
    ...
    assert unreadable["label"] == "RESUMES " + UNAVAILABLE


def test_replay_prefix_does_not_include_later_resumes(cases):
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function resumeCountView" in state
    assert "export function recordedResumeCountFeed" in state
    assert 'const RESUME_COUNT_EVENT = "mission.resumed"' in state
    view_body = state.split("export function resumeCountView")[1].split("export function applyEvent")[0]
    assert "RESUME_COUNT_EVENT" in view_body
    ...
    assert "payload.resumes" not in view_body
    assert "state.preview?null:recordedResumeCountFeed(eventLog,replayCursor,resumeCountFeedLoaded)" in control
    assert "resumeCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "resumeCountFeedLoaded=options.resumeCountFeedLoaded===true" in control
    assert "if(Array.isArray(events))" in control
    assert "resumeCountFeedLoaded=true" in control
    assert 'id="resumeCountChip"' in html
    chip = html.split('id="resumeCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.resume-count")[1].split(".hud-stats")[0]
    assert '.hud-chip.resume-count[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_resume_count_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="resumeCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function resumeCountView" in state.text
