"""Read-only Mission Control job-failed count chip. Never invents a count or spend."""
...
HARNESS = ROOT / "tests" / "job_failed_count_chip_cases.mjs"
UNAVAILABLE = "unavailable"


def _load_cases() -> dict:
    if shutil.which("node") is None:
        pytest.skip("node is required to execute the job-failed chip harness")
...
def test_missing_feed_is_unavailable_not_zero(cases):
    for key in ("missingNull", "missingUndefined", "missingObject", "unloadedView"):
        ...
        assert view["known"] is False
        assert view["count"] is None
        assert view["label"] == "JOB FAILED " + UNAVAILABLE
        assert view["label"] != "JOB FAILED 0"
...
def test_explicit_empty_list_is_zero(cases):
    ...
    assert before["label"] == "JOB FAILED 0"


def test_counts_job_failed_only(cases):
    view = cases["counted"]
    assert view["known"] is True
    assert view["count"] == 3
    assert view["label"] == "JOB FAILED 3"
    ...
    assert cases["oneFailedNotPayload"]["count"] == 1
    assert cases["oneFailedNotPayload"]["label"] == "JOB FAILED 1"
    ...
    assert unreadable["label"] == "JOB FAILED " + UNAVAILABLE
...
def test_chip_is_wired_to_the_loaded_event_feed():
    ...
    assert "export function jobFailedCountView" in state
    assert "export function recordedJobFailedCountFeed" in state
    assert 'const JOB_FAILED_COUNT_EVENT = "job.failed"' in state
    view_body = state.split("export function jobFailedCountView")[1].split("export function applyEvent")[0]
    assert "JOB_FAILED_COUNT_EVENT" in view_body
    ...
    assert "state.preview?null:recordedJobFailedCountFeed(eventLog,replayCursor,jobFailedFeedLoaded)" in control
    assert "jobFailedCountView(feed,{visible:!!(state.mission&&!state.preview)})" in control
    assert "jobFailedFeedLoaded=options.jobFailedFeedLoaded===true" in control
    assert 'jobFailedChip.textContent=jobView.hidden?"":jobView.label' in control
    assert "if(Array.isArray(events))" in control
    assert "jobFailedFeedLoaded=true" in control
    assert 'id="jobFailedCountChip"' in html
    chip = html.split('id="jobFailedCountChip"')[1].split(">")[0]
    assert "hidden" in chip
    assert "unavailable" not in chip
    assert "0" not in chip
    css_rule = css.split(".hud-chip.job-failed")[1].split(".hud-stats")[0]
    assert '.hud-chip.job-failed[data-known="false"]' in css
    assert "animation" not in css_rule


def test_static_job_failed_chip_is_served(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert 'id="jobFailedCountChip"' in page.text
        state = client.get("/static/state.mjs")
        assert state.status_code == 200
        assert "export function jobFailedCountView" in state.text
