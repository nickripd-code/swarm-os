import {lastQuestionAtView, LAST_QUESTION_AT_NONE, LAST_QUESTION_AT_UNAVAILABLE, recordedQuestionAtFeed} from "../app/static/state.mjs";

const cases = {};
const visible = {visible: true};

function view(feed, options = visible) {
  return lastQuestionAtView(feed, options);
}

cases.unavailable = LAST_QUESTION_AT_UNAVAILABLE;
cases.none = LAST_QUESTION_AT_NONE;
...
cases.empty = view([]);
cases.cursorBefore = view(recordedQuestionAtFeed([
  {id: 1, event_type: "mission.question", created_at: "2026-09-23T12:35:04Z"},
], -1, true));
cases.feedNotLoaded = view(recordedQuestionAtFeed([
...
cases.newestByTime = view([
  ...
  {event_type: "mission.question", created_at: "2026-09-23T12:10:00Z"},
  {event_type: "user.answer_consumed", created_at: "2026-09-23T12:50:09Z"},
]);
cases.kinds = view([
  {event_type: "mission.question", created_at: "2026-09-23T01:02:03Z"},
]);
cases.waitingDoesNotOverride = view([
  {event_type: "mission.question", created_at: "2026-09-23T12:35:04Z"},
  {event_type: "mission.waiting", created_at: "2026-09-23T13:00:00Z", payload: {question_id: "q1"}},
]);
cases.replayPrefix = view(recordedQuestionAtFeed([
  {id: 1, event_type: "mission.started", created_at: "2026-09-23T12:00:00Z"},
  {id: 2, event_type: "mission.question", created_at: "2026-09-23T12:35:04Z"},
  {id: 3, event_type: "user.answered", created_at: "2026-09-23T12:36:00Z"},
], 1, true));
cases.replayAtLater = view(recordedQuestionAtFeed([
  {id: 1, event_type: "mission.started", created_at: "2026-09-23T12:00:00Z"},
  {id: 2, event_type: "mission.question", created_at: "2026-09-23T12:35:04Z"},
  {id: 3, event_type: "user.answered", created_at: "2026-09-23T12:36:00Z"},
], 2, true));
cases.missingTime = view([{event_type: "mission.question"}]);
...
cases.numericZero = view([{event_type: "mission.question", created_at: 0}]);
cases.numericNow = view([{event_type: "user.answered", created_at: 1758630904000}]);
cases.stringZero = view([{event_type: "mission.question", created_at: "0"}]);
...
cases.spendIgnored = view([
  {event_type: "mission.question", created_at: "2026-09-23T12:35:04Z", payload: {token_spent: 9, input_tokens: 100}},
]);
