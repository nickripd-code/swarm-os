...
def test_v4_database_upgrades_to_objective_columns_without_losing_rows(tmp_path):
    path = tmp_path / "v4.db"
    engine = _engine(path)
    assert apply_migrations(engine, MIGRATIONS[:4]) == 4
    mission = Mission(goal="keep through v5", status=MissionStatus.RUNNING)
    with engine.begin() as conn:
        conn.execute(
            text("INSERT INTO missions (id, payload, updated_at) VALUES (:id, :payload, :updated_at)"),
            {"id": str(mission.id), "payload": mission.model_dump_json(),
        ...
        columns = {row[1] for row in conn.execute(text("PRAGMA table_info(missions)"))}
    assert "objective_id" not in columns
    assert "success_criteria" not in columns

    store = Store(str(path))
    assert _version(store.engine) == CURRENT_SCHEMA_VERSION
    with store.engine.connect() as conn:
        columns = {row[1] for row in conn.execute(text("PRAGMA table_info(missions)"))}
    assert {"objective_id", "success_criteria"} <= columns
    loaded = store.get_mission(mission.id)
    assert loaded is not None
