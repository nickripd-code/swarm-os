...
**This slice:** ModelRouter historical success + simple cost-aware ranking. `complete()` / `complete_on()` record recent success/failure per provider (window of 8). Scoring uses that window and a relative cost penalty among eligible models of the same known cost kind. Empty history is unknown, not success. Failover classes, hard capability rejects, and fail-closed single-provider outage are unchanged. Selection rationale is on `RouteDecision` and `_meta.route`. Rebased onto latest `origin/main` after Anthropic (`#27`), PolicyGate (`#23`), worker leases (`#22`), migrations (`#24`), xAI (`#25`), and typed events (`#20`); those modules were not rewritten. No OrganizationDesigner. No React/Pixi / LangGraph. Tests: `python3 -m pytest tests/ -q` → **316 passed** (existing SQLite datetime adapter warnings).

**Landed on main (keep):** Anthropic Messages adapter (`#27`); PolicyGate (`#23`); durable worker leases / idempotency (`#22`); versioned SQLite migrations (`#24`); xAI adapter (`#25`); typed `EventType` catalog (`#20`); ToolProvider / MCP seam (`#21`); mission answers path (`#19`); `PaymentProvider` behind `WalletAdapter` (`#18`); llama.cpp (`#17`); vLLM (`#16`); verifier seed (`#15`).

**Recommended next backend slice:** OrganizationDesigner remains reserved. Remaining cloud adapters (Gemini, Mistral, DeepSeek, Cohere) or worker-side tool requests (`WORK_FORMAT`) are also valid. Still fail closed. Still no game-world UI rewrite.
