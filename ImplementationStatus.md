...
**Recommended next backend slice:** OrganizationDesigner remains reserved. External evidence runners (tests/endpoints/files) or Anthropic Messages adapter. Isolated live wallet behind the PaymentProvider seam is later — do not enable spending. Still fail closed. Still no game-world UI rewrite.
...
- Tree: `app/` (runtime, llm, providers, router, planning, verifier, payments, store, models, main, health, credentials, static UI), `tests/`, `scripts/check_openai.py`, `NORTH_STAR.md`, this file. No CI, no Docker, no `.env.example`.
