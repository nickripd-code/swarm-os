...
| Events | Typed-ish string events + WebSocket replay of history; additive `planner.proposal` / `judge.decision` when multi-planner actually runs; additive `mission.waiting` / `mission.running`, `task.pending`, `tool.started` / `tool.completed` / `tool.failed`; additive `verification.started` / `verification.passed` / `verification.failed`; additive `mission.question` / consumed `user.answered` | Missing most remaining north-star event types (replan, org change, self-mod, budget warning). HUD/activity understand verification events; activity names `ask` / question / answered truthfully. No fake success animation. |
...
- Tree: `app/` (runtime, llm, providers, router, planning, verifier, tools, payments, store, models, main, health, credentials, static UI), `tests/`, `scripts/check_openai.py`, `NORTH_STAR.md`, this file. No CI, no Docker, no `.env.example`.
