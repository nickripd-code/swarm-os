...
**This slice:** durable worker-lease / idempotency seed, rebased onto main after versioned SQLite migrations (`#24`), xAI (`#25`), typed events (`#20`), ToolProvider (`#21`), answers (`#19`), PaymentProvider (`#18`), and llama.cpp (`#17`). Lease tables are migration **v2** (`worker_leases` / `idempotency_keys`); v1 databases upgrade in place. Runtime claims a mission lease for `run()` and a task lease for `_execute_task`; a live lease from another worker is fail-closed (no double-run). `create_payment` requires an idempotency key; `invoke_tool` uses an explicit key or a derived attempt key and replays the first stored outcome. PaymentProvider, ToolProvider, and the `#24` migration framework are wrapped/extended, not rewritten. Additive `EventType` members: `lease.claimed` / `lease.expired` / `lease.released`. Did not rewrite OrganizationDesigner, verifier, ToolProvider, PaymentProvider, typed-event schema design, xAI/provider adapters, or `app/static/*`. Tests: `python3 -m pytest tests/ -q` → **272 passed**.

**Landed on main (keep):** versioned SQLite migrations (`#24`); xAI OpenAI-compatible adapter (`#25`); typed `EventType` catalog (`#20`); ToolProvider / MCP seam (`#21`); answers path (`#19`); PaymentProvider behind `WalletAdapter` (`#18`); llama.cpp (`#17`). Isolated live wallet remains later — do not enable spending.

**Recommended next backend slice:** OrganizationDesigner remains reserved for Codex/Dominic. Do not steal Verifier / ToolProvider / Payments / typed-event schema work. A future column change is `Migration(version=3)` in `app/migrations.py`. Still fail closed. Still no game-world UI rewrite.

**Explicitly not next:** game-world UI, self-modification, PaymentProvider rewrite, Playwright, OrganizationDesigner.

---

## Audit notes (repo facts)

- Tree: `app/` (runtime, llm, providers, router, planning, verifier, events, tools, payments, leases, store, migrations, models, main, health, credentials, static UI), `tests/`, `scripts/check_openai.py`, `NORTH_STAR.md`, this file. No CI, no Docker, no `.env.example`.
