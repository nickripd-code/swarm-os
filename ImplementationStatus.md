...
- Health endpoint reports OpenAI, OpenRouter, xAI, Anthropic, Mistral, Ollama, vLLM, and llama.cpp configured/model status, `fallback: false`, plus a `tools` block. Ollama, vLLM, and llama.cpp also report daemon `status` (`unconfigured` / `unavailable` / `healthy`). Does not expose keys.
...
- Additional provider adapters (Gemini, DeepSeek, Cohere) beyond OpenAI + OpenRouter + xAI + Anthropic + Mistral + Ollama + vLLM + llama.cpp.
...
**This slice:** Mistral OpenAI-compatible Chat Completions adapter (`MistralModelProvider`). Opt-in via `MISTRAL_API_KEY`; `MISTRAL_MODEL` does not opt in by itself. Registered in `ModelRouter` when configured; OpenAI-only catalogs stay one-entry. Fail-closed unconfigured complete (`AUTHORIZATION_REQUIRED`); classified HTTP/timeout/outage/422 errors; no fake completions; keys never in responses or error strings. Health reports configured/model/`fallback: false` without probing the live API. `POST {MISTRAL_BASE_URL}/chat/completions` with Bearer auth. Rebased onto latest `origin/main` after ModelRouter ranking (`#26`) and Anthropic (`#27`). Did not rewrite EventType, ToolProvider/MCP, OrganizationDesigner, worker leases, PolicyGate, SQLite migrations, ModelRouter ranking, Gemini, xAI, Anthropic, React/Pixi, or LangGraph. Tests: `python3 -m pytest tests/ -q` → **338 passed**.

**Landed on main (keep):** ModelRouter historical success + cost-aware ranking (`#26`); Anthropic Messages (`#27`); PolicyGate (`#23`); durable worker leases / idempotency (`#22`); versioned SQLite migrations (`#24`); xAI (`#25`); typed `EventType` catalog (`#20`); ToolProvider / MCP seam (`#21`); mission answers path (`#19`); `PaymentProvider` behind `WalletAdapter` (`#18`); llama.cpp (`#17`); vLLM (`#16`); verifier seed (`#15`).

**Recommended next backend slice:** OrganizationDesigner remains reserved. Remaining cloud adapters (Gemini, DeepSeek, Cohere) or worker-side tool requests (`WORK_FORMAT`). Isolated live wallet is later — do not enable spending. Still fail closed. Still no game-world UI rewrite.
