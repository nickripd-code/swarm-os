...
| `SWARM_WORKSPACE_ROOT` | Optional root for the local `WorkspaceProvider` filesystem sandbox. Defaults to `{tempdir}/swarm-workspaces`. Isolated per-workspace directories are created under this root; path traversal outside it fails closed. `destroy` removes only that sandbox directory. Not Docker. |
| `SWARM_WORKSPACE_TTL_SECONDS` | Optional positive integer. When set, an explicit `WorkspaceProvider.gc()` may delete local sandboxes older than this many seconds. Unset, blank, non-integer, or non-positive leaves GC off. Health reports the setting and does not sweep. Not Docker. |
