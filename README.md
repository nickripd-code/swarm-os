...
| `COMPOSIO_USER_ID` | Stable application user id for Composio connections (default `swarm-local`). Use a real per-user id in multi-user deployments. |
| `COMPOSIO_TOOLKITS` | Optional comma-separated toolkit allowlist such as `gmail,googlecalendar,googledrive,github`. Empty leaves toolkit selection to the read-only session search. |
| `COMPOSIO_BASE_URL` | Optional API base (default `https://backend.composio.dev/api/v3.1`). |
| `SWARM_BROWSER` | Optional browser tools (`browser.navigate`, `browser.snapshot`, `browser.click`). Unset stays disabled and PolicyGate still denies browser tools. Set to `1`/`true`/`yes`/`on` to opt in. Default driver is local Playwright (`pip install -e ".[browser]"` and `playwright install chromium`). CI uses fakes and does not browse. |
| `SWARM_BROWSER_REMOTE` | Separate opt-in for a Browserbase/Stagehand remote driver. Requires `SWARM_BROWSER` plus `BROWSERBASE_API_KEY` and `BROWSERBASE_PROJECT_ID`. Does not replace Playwright unless all three are set. Missing credentials stay unconfigured and do not fall back to Playwright. Checkout, payment, and booking clicks are refused. |
| `BROWSERBASE_API_KEY` | Browserbase API key for the remote driver only. Server environment only. Never logged, never returned over HTTP, never placed in model context. |
| `BROWSERBASE_PROJECT_ID` | Browserbase project id for the remote driver. Required together with `BROWSERBASE_API_KEY`. Not returned by health. |
| `STAGEHAND_BASE_URL` | Optional Stagehand API base (default `https://api.stagehand.browserbase.com`). Must be http(s). |
...
| `SWARM_WORKSPACE_ROOT` | Optional root for the local `WorkspaceProvider` filesystem sandbox. Defaults to `{tempdir}/swarm-workspaces`. Isolated per-workspace directories are created under this root; path traversal outside it fails closed. Not Docker. |
...
The runtime enforces mission-wide depth, agent, task, tool-call, runtime, payment, and estimated token-cost limits through an explicit `PolicyGate` before spawn, finish, and tool use, plus `ResourceScheduler` on the model-call path. Optional `MissionLimits.require_finish_approval` parks `finish` in `WAITING` until a matching human `approve`/`deny` (deny and ambiguous answers fail closed; there is no silent auto-approve). Dangerous actions (shell, browser, network, live payment, deploy, self-modification) are denied by default. `SWARM_BROWSER` opts in `browser.navigate` / `browser.snapshot` / `browser.click` only; other dangerous names stay refused, and `local_only` still forbids them. `SWARM_BROWSER_REMOTE` with `BROWSERBASE_API_KEY` and `BROWSERBASE_PROJECT_ID` uses the Stagehand HTTP driver instead of local Playwright; without those credentials the remote opt-in fails closed and does not browse locally. Checkout, payment, and booking selectors are refused. `COMPOSIO_API_KEY` opts in only `composio.*` tools backed by a server-created session filtered to `readOnlyHint`; the generic Composio execute meta-tool is not exposed, each app action retains a concrete policy-visible name, and `local_only` still blocks all Composio calls. `SWARM_SELFMOD` opts in `selfmod.propose` / `selfmod.diff` (dry-run only). `selfmod.apply` additionally requires `SWARM_SELFMOD_WRITE` and still writes a sandbox unless `SWARM_SELFMOD_PRODUCTION_WRITE` is set; protected core files cannot be applied. Optional mission `privacy=local_only` forbids cloud/network tools and is forwarded to `ModelRouter` capability requests. Payments go through `PaymentProvider` behind `WalletAdapter` and are simulated by default. Token USD estimates are separate from wallet `spent` and fail closed as `RESOURCE_EXHAUSTED` when the remaining token budget cannot cover a known estimate (or when price metadata is missing and `SWARM_TOKEN_UNKNOWN_PRICE=fail`). Live settlement intentionally fails closed until an isolated wallet is configured; private keys must remain outside the agent process. This slice does not enable live spending. Transient `RATE_LIMIT` and `TIMEOUT` provider errors are retried with bounded exponential backoff (3 retries, 0.5s / 1s / 2s) against the same adapter. Classified `PROVIDER_OUTAGE` (or an unconfigured primary) may use the next model in the router fallback chain (OpenRouter when `OPENROUTER_API_KEY` is set; xAI when `XAI_API_KEY` is set; Anthropic when `ANTHROPIC_API_KEY` is set; Mistral when `MISTRAL_API_KEY` is set; Gemini when `GEMINI_API_KEY` is set; Cohere when `COHERE_API_KEY` is set; DeepSeek when `DEEPSEEK_API_KEY` is set; Together when `TOGETHER_API_KEY` is set; Groq when `GROQ_API_KEY` is set; Fireworks when `FIREWORKS_API_KEY` is set; Azure OpenAI when `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT`, and `AZURE_OPENAI_DEPLOYMENT` are set; Perplexity when `PERPLEXITY_API_KEY` is set; Bedrock when `BEDROCK_API_KEY` or `AWS_BEARER_TOKEN_BEDROCK` is set; Hugging Face when `HUGGINGFACE_API_KEY` is set; Cerebras when `CEREBRAS_API_KEY` is set; SambaNova when `SAMBANOVA_API_KEY` is set; Vertex AI when `VERTEX_API_KEY` and `VERTEX_PROJECT` (or `GOOGLE_CLOUD_PROJECT`) are set; Ollama when `OLLAMA_MODEL` or `OLLAMA_BASE_URL` is set; vLLM when `VLLM_MODEL` or `VLLM_BASE_URL` is set; llama.cpp when `LLAMACPP_MODEL` or `LLAMACPP_BASE_URL` is set). Exhausted retries or a failed failover still fail closed with `{error, failure_class}`.

Mission, agent, task, event, worker-lease, and idempotency state is durable in SQLite. Schema is applied by versioned migrations (`app/migrations.py`); existing create_all-era databases are upgraded in place and rows are not dropped. A graceful server shutdown suspends active execution without converting it into a user stop; the next configured runtime resumes it against the original mission deadline. An interrupted text-only task is retained as a stopped attempt and retried under a new task ID. Mission and task worker leases heartbeat and expire so a restarted process can reclaim unfinished work; a live lease blocks a second worker. Payments and tool invokes go through an idempotency key path and replay the first stored outcome instead of running twice. Isolated agent files go through `WorkspaceProvider` (`app/workspace.py`): a local directory sandbox under `SWARM_WORKSPACE_ROOT`, fail-closed on path traversal. This is not a host shell and not Docker. Tools run only through `ToolProvider`: allowlisted local tools, an MCP JSON-RPC stub, an optional read-only-first Composio Sessions provider (`COMPOSIO_API_KEY`), an optional Playwright or Browserbase browser provider (`SWARM_BROWSER`, optional `SWARM_BROWSER_REMOTE`), and an optional self-modification sandbox (`SWARM_SELFMOD`). Controller `use_tool` and worker `WORK_FORMAT` status `use_tool` both go through `PolicyGate` then `invoke_tool`. Unconfigured or unknown tools fail `TOOL_MISSING` without charging `max_tool_calls`. A real invoke emits `tool.started` then `tool.completed` or `tool.failed` — never a fabricated success. Composio search registers only bounded concrete app slugs and OAuth link results omit link tokens. Browser health is `unconfigured` until opted in, `unavailable` if Playwright is missing, `browserbase` when the remote driver is configured, and never reports a successful browse without a driver result. Remote health does not open a session or return credentials. Self-mod health is `unconfigured` until opted in and never reports an applied production write unless the write flags are set and apply actually ran.

## Next integration seams

- Add a write-capability approval layer and toolkit/action allowlists before enabling any mutating Composio tool. The current Composio session is read-only-first and intentionally cannot send, create, delete, purchase, or pay.
- Browserbase/Stagehand remote `goto` / `snapshot` / `click` is a partial seed behind `BrowserToolProvider` (`SWARM_BROWSER_REMOTE`). Checkout, payment, and booking clicks are refused; there is no approval-resume path and no act/type/fill automation yet. The local Playwright driver remains the default.
...
- Extend `WorkspaceProvider` (`app/workspace.py`) with Docker / remote backends. The local filesystem sandbox exists; do not give agents a host shell.
