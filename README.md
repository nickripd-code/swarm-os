...
| `EXA_API_KEY` | Optional Exa web search. Registers `exa.search`, `search.web`, and `exa.contents`. Keep it in the server environment only. Unset stays out of the catalog. |
| `SWARM_EXA` | Optional policy opt-in (`1`/`true`/`yes`/`on`) for those same Exa tool names. Without `EXA_API_KEY` the provider stays unconfigured and calls fail `TOOL_MISSING`. |
| `EXA_BASE_URL` | Optional Exa API base (default `https://api.exa.ai`). Does not opt in by itself. |
...
- Exa search is a PARTIAL seed (`app/exa.py`): opt-in `exa.search` / `search.web` / `exa.contents` behind `ToolProvider`. It is not a second search stack and it does not auto-run inside `EvidenceRunner`.
