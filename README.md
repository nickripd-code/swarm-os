...
| `SWARM_WORKSPACE_ROOT` | Optional root for the local `WorkspaceProvider` filesystem sandbox. Defaults to `{tempdir}/swarm-workspaces`. Isolated per-workspace directories are created under this root; path traversal outside it fails closed. Ignored for file storage when `SWARM_WORKSPACE_E2B` is set. Not Docker. |
| `SWARM_WORKSPACE_E2B` | Optional remote E2B sandbox (`1`/`true`/`yes`/`on`). Unset keeps the local filesystem even if `E2B_API_KEY` is present. Set without a key fails closed and does not fall back to local. |
| `E2B_API_KEY` | API key for the E2B control plane. Read from the environment only when `SWARM_WORKSPACE_E2B` is set. Never logged, never returned over HTTP, never placed in model context. |
| `SWARM_WORKSPACE_E2B_TTL_SECONDS` | Sandbox lifetime passed as E2B `timeout` (default `300`, allowed `15`–`3600`). Invalid values fail closed. This seed does not destroy sandboxes or copy artifacts out before expiry. |
| `SWARM_WORKSPACE_E2B_ALLOW_INTERNET` | Optional egress opt-in. Unset denies internet (`allow_internet_access: false` and `denyOut` `0.0.0.0/0`). |
| `SWARM_WORKSPACE_E2B_TEMPLATE` | Optional E2B template id (default `base`). |
...
- Add Browserbase/Stagehand behind `BrowserToolProvider` for remote sites without dependable APIs. The Playwright browser seed exists and stays disabled by default.
...
- Extend the E2B `WorkspaceProvider` seed with destroy/GC and artifact copy before teardown, or add Docker backends. Filesystem read/write exists; do not give agents a host shell.
