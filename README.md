...
| `SWARM_SELFMOD_WORKTREE_ROOT` | Optional absolute directory for isolated self-mod worktrees. When `SWARM_SELFMOD` is on, propose/diff clone the source locally and `git worktree add` a branch under this root (default: a temp directory). Relative roots fail closed. A root inside the source or production tree fails closed unless `SWARM_SELFMOD_PRODUCTION_WRITE` is also set. Health does not create a worktree. |
...
- Extend the self-modification worktree seed (`app/selfmod.py`) with tests/review and rollback. Propose/diff uses an isolated worktree when `SWARM_SELFMOD` is on; do not enable production writes from the agent process.
