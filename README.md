...
| `SWARM_LOCAL_TOOLS` | Optional comma-separated allowlist of in-process tools (`echo`, `clock.utc`, `hash.sha256`). Naming `memory.remember` or `memory.recall` also opts that local memory tool in. Empty means no local tools. |
| `SWARM_MEMORY_TOOLS` | Optional opt-in for both local memory tools (`memory.remember`, `memory.recall`). Set to `1`/`true`/`yes`/`on`. Unset leaves them off unless those names are in `SWARM_LOCAL_TOOLS`. PolicyGate denies them until one of those opts in. They are not cloud tools, so `local_only` missions may call them. Recall stays inside the existing character cap; an empty store returns an empty list. |
