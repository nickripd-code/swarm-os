...
| `COMPOSIO_API_KEY` | Optional Composio Sessions action gateway. Enables read-only tool search, OAuth link creation, and concrete tools returned by that scoped session. Keep it in the server environment only. |
...
| `COMPOSIO_WRITE_ALLOWLIST` | Optional `toolkit:ACTION` grants, comma-separated. Empty refuses every mutating Composio slug, including GitHub writes and `destructiveHint`, even if a payload claims `readOnlyHint`. An allowlisted write still parks for human approve/deny before the first execute. |
