...
| `TWILIO_ACCOUNT_SID` | Optional Twilio SMS account SID. All four Twilio variables are required; a partial set stays unconfigured. |
| `TWILIO_AUTH_TOKEN` | Optional Twilio auth token. Keep it in the server environment only. Never logged, never returned over HTTP, never placed in model context. |
| `TWILIO_FROM_NUMBER` | Optional Twilio sender in E.164 (`+15551230000`). The model cannot choose a different sender. |
| `TWILIO_SMS_ALLOWLIST` | Required destination allowlist when Twilio SMS is enabled, comma-separated E.164 numbers. Empty does not mean send anywhere. Voice is not enabled. |
