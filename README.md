...
| `SWARM_REQUIRE_TOKEN_METERS` | Optional. When `1`/`true`, a model call without token usage meters fails closed (`RESOURCE_EXHAUSTED`). Default is observe-only: missing meters are recorded as unmetered and do not fail the mission. |
| `SWARM_TOKEN_PRICES` | Optional JSON object of per-million USD prices used only to estimate cost, e.g. `{"openai":{"input":5,"output":15}}`. Keys may be `provider`, `model`, or `provider:model`. Unset means token totals are recorded and estimated cost stays unknown. This is not ResourceScheduler and does not cap spend. |
