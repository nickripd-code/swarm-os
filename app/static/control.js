import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmSuccessRateFeed,llmSuccessRateView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmSuccessRateFeedLoaded=false;
...
function renderHud(){
  ...
  const llmSuccessChip=$("llmSuccessRateChip");
  if(llmSuccessChip){
    const feed=state.preview?null:recordedLlmSuccessRateFeed(eventLog,replayCursor,llmSuccessRateFeedLoaded);
    const llmSuccessView=llmSuccessRateView(feed,{visible:!!(state.mission&&!state.preview)});
    llmSuccessChip.hidden=llmSuccessView.hidden;
    llmSuccessChip.textContent=llmSuccessView.label;
    llmSuccessChip.dataset.known=llmSuccessView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  llmSuccessRateFeedLoaded=options.llmSuccessRateFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmSuccessRateFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
