import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationFailRateFeed,verificationFailRateView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationFailRateFeedLoaded=false;
...
function renderHud(){
  ...
  const verificationFailChip=$("verificationFailRateChip");
  if(verificationFailChip){
    const feed=state.preview?null:recordedVerificationFailRateFeed(eventLog,replayCursor,verificationFailRateFeedLoaded);
    const verificationFailView=verificationFailRateView(feed,{visible:!!(state.mission&&!state.preview)});
    verificationFailChip.hidden=verificationFailView.hidden;
    verificationFailChip.textContent=verificationFailView.label;
    verificationFailChip.dataset.known=verificationFailView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  verificationFailRateFeedLoaded=options.verificationFailRateFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        verificationFailRateFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationFailRateFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
