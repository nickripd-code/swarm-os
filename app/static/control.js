import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskCompletedAtFeed,lastTaskCompletedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskCompletedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const taskCompletedAtChip=$("lastTaskCompletedAtChip");
  if(taskCompletedAtChip){
    const feed=state.preview?null:recordedTaskCompletedAtFeed(eventLog,replayCursor,taskCompletedAtFeedLoaded);
    const taskCompletedAt=lastTaskCompletedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    taskCompletedAtChip.hidden=taskCompletedAt.hidden;
    taskCompletedAtChip.textContent=taskCompletedAt.hidden?"":taskCompletedAt.label;
    taskCompletedAtChip.dataset.known=taskCompletedAt.known?"true":"false";
    if(taskCompletedAt.hidden||!taskCompletedAt.title)taskCompletedAtChip.removeAttribute("title");
    else taskCompletedAtChip.title=taskCompletedAt.title;
    if(taskCompletedAt.hidden)taskCompletedAtChip.removeAttribute("aria-label");
    else taskCompletedAtChip.setAttribute("aria-label",taskCompletedAt.label);
  }
...
function reset(mission, options={}){
  ...
  taskCompletedAtFeedLoaded=options.taskCompletedAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        taskCompletedAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskCompletedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
