import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmFailFeed,llmFailCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmFailFeedLoaded=false;
...
function renderHud(){
  ...
  const failChip=$("llmFailCountChip");
  if(failChip){
    const feed=state.preview?null:recordedLlmFailFeed(eventLog,replayCursor,llmFailFeedLoaded);
    const failView=llmFailCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  llmFailFeedLoaded=options.llmFailFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmFailFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
