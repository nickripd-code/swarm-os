import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedSpawnedAtFeed,lastSpawnedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, spawnedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const spawnedAtChip=$("lastSpawnedAtChip");
  if(spawnedAtChip){
    const feed=state.preview?null:recordedSpawnedAtFeed(eventLog,replayCursor,spawnedAtFeedLoaded);
    const spawnedAt=lastSpawnedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    spawnedAtChip.hidden=spawnedAt.hidden;
    spawnedAtChip.textContent=spawnedAt.hidden?"":spawnedAt.label;
    spawnedAtChip.dataset.known=spawnedAt.known?"true":"false";
    if(spawnedAt.hidden||!spawnedAt.title)spawnedAtChip.removeAttribute("title");
    else spawnedAtChip.title=spawnedAt.title;
    if(spawnedAt.hidden)spawnedAtChip.removeAttribute("aria-label");
    else spawnedAtChip.setAttribute("aria-label",spawnedAt.label);
  }
...
function reset(mission, options={}){
  ...
  spawnedAtFeedLoaded=options.spawnedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{spawnedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
