import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAvgVerificationLatencyFeed,avgVerificationLatencyView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, avgVerificationLatencyFeedLoaded=false;
...
function renderHud(){
  ...
  const avgVerificationLatencyChip=$("avgVerificationLatencyChip");
  if(avgVerificationLatencyChip){
    const feed=state.preview?null:recordedAvgVerificationLatencyFeed(eventLog,replayCursor,avgVerificationLatencyFeedLoaded);
    const avgVerificationLatency=avgVerificationLatencyView(feed,{visible:!!(state.mission&&!state.preview)});
    avgVerificationLatencyChip.hidden=avgVerificationLatency.hidden;
    avgVerificationLatencyChip.textContent=avgVerificationLatency.label;
    avgVerificationLatencyChip.dataset.known=avgVerificationLatency.known?"true":"false";
  }
...
function reset(mission, options={}){
  stopReplayPlay();
  avgVerificationLatencyFeedLoaded=options.avgVerificationLatencyFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        avgVerificationLatencyFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{avgVerificationLatencyFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
