import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationAtFeed,lastVerificationAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationAtFeedLoaded=false;
...
function renderHud(){
  ...
  const verifyAtChip=$("lastVerificationAtChip");
  if(verifyAtChip){
    const feed=state.preview?null:recordedVerificationAtFeed(eventLog,replayCursor,verificationAtFeedLoaded);
    const verifyAt=lastVerificationAtView(feed,{visible:!!(state.mission&&!state.preview)});
    verifyAtChip.hidden=verifyAt.hidden;
    verifyAtChip.textContent=verifyAt.hidden?"":verifyAt.label;
    verifyAtChip.dataset.known=verifyAt.known?"true":"false";
    if(verifyAt.hidden||!verifyAt.title)verifyAtChip.removeAttribute("title");
    else verifyAtChip.title=verifyAt.title;
    if(verifyAt.hidden)verifyAtChip.removeAttribute("aria-label");
    else verifyAtChip.setAttribute("aria-label",verifyAt.label);
  }
...
function reset(mission, options={}){
  ...
  verificationAtFeedLoaded=options.verificationAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
