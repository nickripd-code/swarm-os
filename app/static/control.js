import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedFailCountFeed,failCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, failCountFeedLoaded=false;
...
function renderHud(){
  ...
  const failChip=$("failCountChip");
  if(failChip){
    const feed=state.preview?null:recordedFailCountFeed(eventLog,replayCursor,failCountFeedLoaded);
    const failView=failCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  failCountFeedLoaded=options.failCountFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{failCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
