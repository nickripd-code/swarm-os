import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAgentUpdatedCountFeed,agentUpdatedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, agentUpdatedFeedLoaded=false;
...
function renderHud(){
  ...
  const updatedChip=$("agentUpdatedCountChip");
  if(updatedChip){
    const feed=state.preview?null:recordedAgentUpdatedCountFeed(eventLog,replayCursor,agentUpdatedFeedLoaded);
    const updatedView=agentUpdatedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    updatedChip.hidden=updatedView.hidden;
    updatedChip.textContent=updatedView.hidden?"":updatedView.label;
    updatedChip.dataset.known=updatedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  agentUpdatedFeedLoaded=options.agentUpdatedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{agentUpdatedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
