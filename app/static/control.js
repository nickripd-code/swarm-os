import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAgentReparentCountFeed,agentReparentCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, agentReparentFeedLoaded=false;
...
function renderHud(){
  ...
  const reparentChip=$("agentReparentCountChip");
  if(reparentChip){
    const feed=state.preview?null:recordedAgentReparentCountFeed(eventLog,replayCursor,agentReparentFeedLoaded);
    const reparentView=agentReparentCountView(feed,{visible:!!(state.mission&&!state.preview)});
    reparentChip.hidden=reparentView.hidden;
    reparentChip.textContent=reparentView.hidden?"":reparentView.label;
    reparentChip.dataset.known=reparentView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  agentReparentFeedLoaded=options.agentReparentFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{agentReparentFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
