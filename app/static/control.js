import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskStartedCountFeed,taskStartedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskStartedFeedLoaded=false;
...
function renderHud(){
  ...
  const taskChip=$("taskStartedCountChip");
  if(taskChip){
    const feed=state.preview?null:recordedTaskStartedCountFeed(eventLog,replayCursor,taskStartedFeedLoaded);
    const taskView=taskStartedCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  taskStartedFeedLoaded=options.taskStartedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskStartedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
