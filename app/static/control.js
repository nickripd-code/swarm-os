import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedReparentedAtFeed,lastReparentedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, reparentedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const reparentedAtChip=$("lastReparentedAtChip");
  if(reparentedAtChip){
    const feed=state.preview?null:recordedReparentedAtFeed(eventLog,replayCursor,reparentedAtFeedLoaded);
    const reparentedAt=lastReparentedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    reparentedAtChip.hidden=reparentedAt.hidden;
    reparentedAtChip.textContent=reparentedAt.hidden?"":reparentedAt.label;
    reparentedAtChip.dataset.known=reparentedAt.known?"true":"false";
    if(reparentedAt.hidden||!reparentedAt.title)reparentedAtChip.removeAttribute("title");
    else reparentedAtChip.title=reparentedAt.title;
    if(reparentedAt.hidden)reparentedAtChip.removeAttribute("aria-label");
    else reparentedAtChip.setAttribute("aria-label",reparentedAt.label);
  }
...
function reset(mission, options={}){
  ...
  reparentedAtFeedLoaded=options.reparentedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{reparentedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
