import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedPaymentAtFeed,lastPaymentAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, paymentAtFeedLoaded=false;
...
function renderHud(){
  ...
  const paymentAtChip=$("lastPaymentAtChip");
  if(paymentAtChip){
    const feed=state.preview?null:recordedPaymentAtFeed(eventLog,replayCursor,paymentAtFeedLoaded);
    const paymentAt=lastPaymentAtView(feed,{visible:!!(state.mission&&!state.preview)});
    paymentAtChip.hidden=paymentAt.hidden;
    paymentAtChip.textContent=paymentAt.hidden?"":paymentAt.label;
    paymentAtChip.dataset.known=paymentAt.known?"true":"false";
    if(paymentAt.hidden||!paymentAt.title)paymentAtChip.removeAttribute("title");
    else paymentAtChip.title=paymentAt.title;
    if(paymentAt.hidden)paymentAtChip.removeAttribute("aria-label");
    else paymentAtChip.setAttribute("aria-label",paymentAt.label);
  }
...
function reset(mission, options={}){
  ...
  paymentAtFeedLoaded=options.paymentAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{paymentAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
