import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJobCompletedAtFeed,lastJobCompletedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, jobCompletedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const jobCompletedAtChip=$("lastJobCompletedAtChip");
  if(jobCompletedAtChip){
    const feed=state.preview?null:recordedJobCompletedAtFeed(eventLog,replayCursor,jobCompletedAtFeedLoaded);
    const jobCompletedAt=lastJobCompletedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    jobCompletedAtChip.hidden=jobCompletedAt.hidden;
    jobCompletedAtChip.textContent=jobCompletedAt.hidden?"":jobCompletedAt.label;
    jobCompletedAtChip.dataset.known=jobCompletedAt.known?"true":"false";
    if(jobCompletedAt.hidden||!jobCompletedAt.title)jobCompletedAtChip.removeAttribute("title");
    else jobCompletedAtChip.title=jobCompletedAt.title;
    if(jobCompletedAt.hidden)jobCompletedAtChip.removeAttribute("aria-label");
    else jobCompletedAtChip.setAttribute("aria-label",jobCompletedAt.label);
  }
...
function reset(mission,options={}){
  ...
  jobCompletedAtFeedLoaded=options.jobCompletedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{jobCompletedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
