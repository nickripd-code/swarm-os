import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJobCompletedCountFeed,jobCompletedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, jobCompletedFeedLoaded=false;
...
function renderHud(){
  ...
  const jobChip=$("jobCompletedCountChip");
  if(jobChip){
    const feed=state.preview?null:recordedJobCompletedCountFeed(eventLog,replayCursor,jobCompletedFeedLoaded);
    const jobView=jobCompletedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    jobChip.hidden=jobView.hidden;
    jobChip.textContent=jobView.hidden?"":jobView.label;
    jobChip.dataset.known=jobView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  jobCompletedFeedLoaded=options.jobCompletedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{jobCompletedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
