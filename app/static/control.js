import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLeaseReleasedAtFeed,lastLeaseReleasedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, leaseReleasedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const leaseReleasedAtChip=$("lastLeaseReleasedAtChip");
  if(leaseReleasedAtChip){
    const feed=state.preview?null:recordedLeaseReleasedAtFeed(eventLog,replayCursor,leaseReleasedAtFeedLoaded);
    const leaseReleasedAt=lastLeaseReleasedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    leaseReleasedAtChip.hidden=leaseReleasedAt.hidden;
    leaseReleasedAtChip.textContent=leaseReleasedAt.hidden?"":leaseReleasedAt.label;
    leaseReleasedAtChip.dataset.known=leaseReleasedAt.known?"true":"false";
    if(leaseReleasedAt.hidden||!leaseReleasedAt.title)leaseReleasedAtChip.removeAttribute("title");
    else leaseReleasedAtChip.title=leaseReleasedAt.title;
    if(leaseReleasedAt.hidden)leaseReleasedAtChip.removeAttribute("aria-label");
    else leaseReleasedAtChip.setAttribute("aria-label",leaseReleasedAt.label);
  }
...
function reset(mission, options={}){
  ...
  leaseReleasedAtFeedLoaded=options.leaseReleasedAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        leaseReleasedAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{leaseReleasedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
