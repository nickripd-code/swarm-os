import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,blockAgeChip,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive} from "./state.mjs";
...
function renderHud(){
  ...
  let blockAge=null;
  if($("blockAgeChip")){
    blockAge=blockAgeChip(state,{now:Date.now()});
    const chip=$("blockAgeChip");
    chip.hidden=blockAge.hidden;
    chip.textContent=blockAge.hidden?"":blockAge.label;
    chip.className="hud-chip block-age "+(blockAge.known?"known":"unavailable");
    if(blockAge.hidden||!blockAge.title)chip.removeAttribute("title");
    else chip.title=blockAge.title;
  }
  const running=!!mission&&!terminal.has(status)&&replayLive&&!state.preview;
  const ageTicking=!!blockAge&&blockAge.known&&!blockAge.hidden;
