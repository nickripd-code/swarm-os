import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAnswerConsumedFeed,answerConsumedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, answerConsumedFeedLoaded=false;
...
function renderHud(){
  ...
  const consumedChip=$("answerConsumedCountChip");
  if(consumedChip){
    const feed=state.preview?null:recordedAnswerConsumedFeed(eventLog,replayCursor,answerConsumedFeedLoaded);
    const consumedView=answerConsumedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    consumedChip.hidden=consumedView.hidden;
    consumedChip.textContent=consumedView.label;
    consumedChip.dataset.known=consumedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  answerConsumedFeedLoaded=options.answerConsumedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{answerConsumedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
