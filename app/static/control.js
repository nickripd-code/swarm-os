import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskFailedAtFeed,lastTaskFailedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskFailedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const taskFailedAtChip=$("lastTaskFailedAtChip");
  if(taskFailedAtChip){
    const feed=state.preview?null:recordedTaskFailedAtFeed(eventLog,replayCursor,taskFailedAtFeedLoaded);
    const taskFailedAt=lastTaskFailedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    taskFailedAtChip.hidden=taskFailedAt.hidden;
    taskFailedAtChip.textContent=taskFailedAt.hidden?"":taskFailedAt.label;
    taskFailedAtChip.dataset.known=taskFailedAt.known?"true":"false";
    if(taskFailedAt.hidden||!taskFailedAt.title)taskFailedAtChip.removeAttribute("title");
    else taskFailedAtChip.title=taskFailedAt.title;
    if(taskFailedAt.hidden)taskFailedAtChip.removeAttribute("aria-label");
    else taskFailedAtChip.setAttribute("aria-label",taskFailedAt.label);
  }
...
function reset(mission, options={}){
  ...
  taskFailedAtFeedLoaded=options.taskFailedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskFailedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
