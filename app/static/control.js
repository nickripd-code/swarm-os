import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolFeed,toolCallCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolFeedLoaded=false;
...
function renderHud(){
  ...
  const toolChip=$("toolCallChip");
  if(toolChip){
    const feed=state.preview?null:recordedToolFeed(eventLog,replayCursor,toolFeedLoaded);
    const toolView=toolCallCountView(feed,{visible:!!(state.mission||state.preview)});
    toolChip.hidden=toolView.hidden;
    toolChip.textContent=toolView.label;
    toolChip.dataset.known=toolView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  toolFeedLoaded=options.toolFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
