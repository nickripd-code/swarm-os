import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJobRetryScheduledCountFeed,jobRetryScheduledCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, jobRetryScheduledFeedLoaded=false;
...
function renderHud(){
  ...
  const jobRetryChip=$("jobRetryScheduledCountChip");
  if(jobRetryChip){
    const feed=state.preview?null:recordedJobRetryScheduledCountFeed(eventLog,replayCursor,jobRetryScheduledFeedLoaded);
    const jobView=jobRetryScheduledCountView(feed,{visible:!!(state.mission&&!state.preview)});
    jobRetryChip.hidden=jobView.hidden;
    jobRetryChip.textContent=jobView.hidden?"":jobView.label;
    jobRetryChip.dataset.known=jobView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  jobRetryScheduledFeedLoaded=options.jobRetryScheduledFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        jobRetryScheduledFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{jobRetryScheduledFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
