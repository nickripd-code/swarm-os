import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmFailRateFeed,llmFailRateView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmFailRateFeedLoaded=false;
...
function renderHud(){
  ...
  const llmFailChip=$("llmFailRateChip");
  if(llmFailChip){
    const feed=state.preview?null:recordedLlmFailRateFeed(eventLog,replayCursor,llmFailRateFeedLoaded);
    const llmFailView=llmFailRateView(feed,{visible:!!(state.mission&&!state.preview)});
    llmFailChip.hidden=llmFailView.hidden;
    llmFailChip.textContent=llmFailView.label;
    llmFailChip.dataset.known=llmFailView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  llmFailRateFeedLoaded=options.llmFailRateFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmFailRateFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
