import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedQuestionFeed,questionCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, questionFeedLoaded=false;
...
function renderHud(){
  ...
  const questionChip=$("questionCountChip");
  if(questionChip){
    const feed=state.preview?null:recordedQuestionFeed(eventLog,replayCursor,questionFeedLoaded);
    const questionView=questionCountView(feed,{visible:!!(state.mission&&!state.preview)});
    questionChip.hidden=questionView.hidden;
    questionChip.textContent=questionView.label;
    questionChip.dataset.known=questionView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  questionFeedLoaded=options.questionFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{questionFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
