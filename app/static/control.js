import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedSuspendAtFeed,lastSuspendAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, suspendAtFeedLoaded=false;
...
function renderHud(){
  ...
  const suspendAtChip=$("lastSuspendAtChip");
  if(suspendAtChip){
    const feed=state.preview?null:recordedSuspendAtFeed(eventLog,replayCursor,suspendAtFeedLoaded);
    const suspendAt=lastSuspendAtView(feed,{visible:!!(state.mission&&!state.preview)});
    suspendAtChip.hidden=suspendAt.hidden;
    suspendAtChip.textContent=suspendAt.hidden?"":suspendAt.label;
    suspendAtChip.dataset.known=suspendAt.known?"true":"false";
    if(suspendAt.hidden||!suspendAt.title)suspendAtChip.removeAttribute("title");
    else suspendAtChip.title=suspendAt.title;
    if(suspendAt.hidden)suspendAtChip.removeAttribute("aria-label");
    else suspendAtChip.setAttribute("aria-label",suspendAt.label);
  }
...
function reset(mission, options={}){
  ...
  suspendAtFeedLoaded=options.suspendAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{suspendAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
