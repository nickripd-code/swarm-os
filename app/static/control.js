import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedMissionRunningAtFeed,lastMissionRunningAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, missionRunningAtFeedLoaded=false;
...
function renderHud(){
  ...
  const missionRunningAtChip=$("lastMissionRunningAtChip");
  if(missionRunningAtChip){
    const feed=state.preview?null:recordedMissionRunningAtFeed(eventLog,replayCursor,missionRunningAtFeedLoaded);
    const missionRunningAt=lastMissionRunningAtView(feed,{visible:!!(state.mission&&!state.preview)});
    missionRunningAtChip.hidden=missionRunningAt.hidden;
    missionRunningAtChip.textContent=missionRunningAt.hidden?"":missionRunningAt.label;
    missionRunningAtChip.dataset.known=missionRunningAt.known?"true":"false";
    if(missionRunningAt.hidden||!missionRunningAt.title)missionRunningAtChip.removeAttribute("title");
    else missionRunningAtChip.title=missionRunningAt.title;
    if(missionRunningAt.hidden)missionRunningAtChip.removeAttribute("aria-label");
    else missionRunningAtChip.setAttribute("aria-label",missionRunningAt.label);
  }
...
function reset(mission,options={}){
  ...
  missionRunningAtFeedLoaded=options.missionRunningAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        missionRunningAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{missionRunningAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
