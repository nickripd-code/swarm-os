import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskFailedCountFeed,taskFailedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskFailedFeedLoaded=false;
...
function renderHud(){
  ...
  const taskFailChip=$("taskFailedCountChip");
  if(taskFailChip){
    const feed=state.preview?null:recordedTaskFailedCountFeed(eventLog,replayCursor,taskFailedFeedLoaded);
    const taskFailView=taskFailedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    taskFailChip.hidden=taskFailView.hidden;
    taskFailChip.textContent=taskFailView.hidden?"":taskFailView.label;
    taskFailChip.dataset.known=taskFailView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  taskFailedFeedLoaded=options.taskFailedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskFailedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
