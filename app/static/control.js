import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedControllerDecisionFeed,controllerDecisionCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, controllerDecisionFeedLoaded=false;
...
function renderHud(){
  ...
  const decisionChip=$("controllerDecisionCountChip");
  if(decisionChip){
    const feed=state.preview?null:recordedControllerDecisionFeed(eventLog,replayCursor,controllerDecisionFeedLoaded);
    const decisionView=controllerDecisionCountView(feed,{visible:!!(state.mission&&!state.preview)});
    decisionChip.hidden=decisionView.hidden;
    decisionChip.textContent=decisionView.label;
    decisionChip.dataset.known=decisionView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  controllerDecisionFeedLoaded=options.controllerDecisionFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        controllerDecisionFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{controllerDecisionFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
