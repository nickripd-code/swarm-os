import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolStartedAtFeed,lastToolStartedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolStartedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const toolStartedAtChip=$("lastToolStartedAtChip");
  if(toolStartedAtChip){
    const feed=state.preview?null:recordedToolStartedAtFeed(eventLog,replayCursor,toolStartedAtFeedLoaded);
    const toolStartedAt=lastToolStartedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    toolStartedAtChip.hidden=toolStartedAt.hidden;
    toolStartedAtChip.textContent=toolStartedAt.hidden?"":toolStartedAt.label;
    toolStartedAtChip.dataset.known=toolStartedAt.known?"true":"false";
    if(toolStartedAt.hidden||!toolStartedAt.title)toolStartedAtChip.removeAttribute("title");
    else toolStartedAtChip.title=toolStartedAt.title;
    if(toolStartedAt.hidden)toolStartedAtChip.removeAttribute("aria-label");
    else toolStartedAtChip.setAttribute("aria-label",toolStartedAt.label);
  }
...
function reset(mission, options={}){
  ...
  toolStartedAtFeedLoaded=options.toolStartedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolStartedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
