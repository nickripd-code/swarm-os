import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,lastToolNameChip,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive} from "./state.mjs";
...
function renderHud(){
  ...
  if($("lastToolNameChip")){
    const tool=lastToolNameChip(state);
    const chip=$("lastToolNameChip");
    chip.hidden=tool.hidden;
    chip.textContent=tool.hidden?"":tool.label;
    chip.className="hud-chip tool "+(tool.known?"known":"unavailable");
    chip.title=tool.hidden?"":(tool.known?"Last recorded tool":"Last tool unavailable");
  }
