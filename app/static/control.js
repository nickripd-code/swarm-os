import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmStartedFeed,llmStartedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmStartedFeedLoaded=false;
...
function renderHud(){
  ...
  const startedChip=$("llmStartedCountChip");
  if(startedChip){
    const feed=state.preview?null:recordedLlmStartedFeed(eventLog,replayCursor,llmStartedFeedLoaded);
    const startedView=llmStartedCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  llmStartedFeedLoaded=options.llmStartedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmStartedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
