import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJudgeDecisionFeed,judgeDecisionCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, judgeDecisionFeedLoaded=false;
...
function renderHud(){
  ...
  const judgeChip=$("judgeDecisionCountChip");
  if(judgeChip){
    const feed=state.preview?null:recordedJudgeDecisionFeed(eventLog,replayCursor,judgeDecisionFeedLoaded);
    const judgeView=judgeDecisionCountView(feed,{visible:!!(state.mission&&!state.preview)});
    judgeChip.hidden=judgeView.hidden;
    judgeChip.textContent=judgeView.label;
    judgeChip.dataset.known=judgeView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  judgeDecisionFeedLoaded=options.judgeDecisionFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{judgeDecisionFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
