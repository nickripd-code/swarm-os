import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolFailAtFeed,lastToolFailAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolFailAtFeedLoaded=false;
...
function renderHud(){
  ...
  const toolFailAtChip=$("lastToolFailAtChip");
  if(toolFailAtChip){
    const feed=state.preview?null:recordedToolFailAtFeed(eventLog,replayCursor,toolFailAtFeedLoaded);
    const toolFailAt=lastToolFailAtView(feed,{visible:!!(state.mission&&!state.preview)});
    toolFailAtChip.hidden=toolFailAt.hidden;
    toolFailAtChip.textContent=toolFailAt.hidden?"":toolFailAt.label;
    toolFailAtChip.dataset.known=toolFailAt.known?"true":"false";
    if(toolFailAt.hidden||!toolFailAt.title)toolFailAtChip.removeAttribute("title");
    else toolFailAtChip.title=toolFailAt.title;
    if(toolFailAt.hidden)toolFailAtChip.removeAttribute("aria-label");
    else toolFailAtChip.setAttribute("aria-label",toolFailAt.label);
  }
...
function reset(mission, options={}){
  ...
  toolFailAtFeedLoaded=options.toolFailAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolFailAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
