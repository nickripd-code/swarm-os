import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedSuspendCountFeed,suspendCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, suspendCountFeedLoaded=false;
...
function renderHud(){
  ...
  const suspendChip=$("suspendCountChip");
  if(suspendChip){
    const feed=state.preview?null:recordedSuspendCountFeed(eventLog,replayCursor,suspendCountFeedLoaded);
    const suspendView=suspendCountView(feed,{visible:!!(state.mission&&!state.preview)});
    suspendChip.hidden=suspendView.hidden;
    suspendChip.textContent=suspendView.label;
    suspendChip.dataset.known=suspendView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  suspendCountFeedLoaded=options.suspendCountFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{suspendCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
