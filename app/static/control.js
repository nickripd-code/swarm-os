import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskPendingCountFeed,taskPendingCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskPendingFeedLoaded=false;
...
function renderHud(){
  ...
  const pendingChip=$("taskPendingCountChip");
  if(pendingChip){
    const feed=state.preview?null:recordedTaskPendingCountFeed(eventLog,replayCursor,taskPendingFeedLoaded);
    const pendingView=taskPendingCountView(feed,{visible:!!(state.mission&&!state.preview)});
    pendingChip.hidden=pendingView.hidden;
    pendingChip.textContent=pendingView.hidden?"":pendingView.label;
    pendingChip.dataset.known=pendingView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  taskPendingFeedLoaded=options.taskPendingFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskPendingFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
