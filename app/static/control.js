import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedBlockCountFeed,blockCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, blockCountFeedLoaded=false;
...
function renderHud(){
  ...
  const blockChip=$("blockCountChip");
  if(blockChip){
    const feed=state.preview?null:recordedBlockCountFeed(eventLog,replayCursor,blockCountFeedLoaded);
    const blockView=blockCountView(feed,{visible:!!(state.mission&&!state.preview)});
    blockChip.hidden=blockView.hidden;
    blockChip.textContent=blockView.label;
    blockChip.dataset.known=blockView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  blockCountFeedLoaded=options.blockCountFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{blockCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
