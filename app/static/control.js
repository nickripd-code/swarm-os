import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAgentMessageAtFeed,lastAgentMessageAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, agentMessageAtFeedLoaded=false;
...
function renderHud(){
  ...
  const agentMessageAtChip=$("lastAgentMessageAtChip");
  if(agentMessageAtChip){
    const feed=state.preview?null:recordedAgentMessageAtFeed(eventLog,replayCursor,agentMessageAtFeedLoaded);
    const agentMessageAt=lastAgentMessageAtView(feed,{visible:!!(state.mission&&!state.preview)});
    agentMessageAtChip.hidden=agentMessageAt.hidden;
    agentMessageAtChip.textContent=agentMessageAt.hidden?"":agentMessageAt.label;
    agentMessageAtChip.dataset.known=agentMessageAt.known?"true":"false";
    if(agentMessageAt.hidden||!agentMessageAt.title)agentMessageAtChip.removeAttribute("title");
    else agentMessageAtChip.title=agentMessageAt.title;
    if(agentMessageAt.hidden)agentMessageAtChip.removeAttribute("aria-label");
    else agentMessageAtChip.setAttribute("aria-label",agentMessageAt.label);
  }
...
function reset(mission, options={}){
  ...
  agentMessageAtFeedLoaded=options.agentMessageAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{agentMessageAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
