import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskBlockedCountFeed,taskBlockedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskBlockedFeedLoaded=false;
...
function renderHud(){
  ...
  const taskBlockChip=$("taskBlockedCountChip");
  if(taskBlockChip){
    const feed=state.preview?null:recordedTaskBlockedCountFeed(eventLog,replayCursor,taskBlockedFeedLoaded);
    const taskBlockView=taskBlockedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    taskBlockChip.hidden=taskBlockView.hidden;
    taskBlockChip.textContent=taskBlockView.hidden?"":taskBlockView.label;
    taskBlockChip.dataset.known=taskBlockView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  taskBlockedFeedLoaded=options.taskBlockedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskBlockedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
