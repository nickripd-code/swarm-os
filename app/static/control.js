import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolAtFeed,lastToolAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolAtFeedLoaded=false;
...
function renderHud(){
  ...
  const toolAtChip=$("lastToolAtChip");
  if(toolAtChip){
    const feed=state.preview?null:recordedToolAtFeed(eventLog,replayCursor,toolAtFeedLoaded);
    const toolAt=lastToolAtView(feed,{visible:!!(state.mission&&!state.preview)});
    toolAtChip.hidden=toolAt.hidden;
    toolAtChip.textContent=toolAt.hidden?"":toolAt.label;
    toolAtChip.dataset.known=toolAt.known?"true":"false";
    if(toolAt.hidden||!toolAt.title)toolAtChip.removeAttribute("title");
    else toolAtChip.title=toolAt.title;
    if(toolAt.hidden)toolAtChip.removeAttribute("aria-label");
    else toolAtChip.setAttribute("aria-label",toolAt.label);
  }
...
function reset(mission, options={}){
  ...
  toolAtFeedLoaded=options.toolAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
