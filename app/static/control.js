import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedApprovalAtFeed,lastApprovalAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, approvalAtFeedLoaded=false;
...
function renderHud(){
  ...
  const approvalAtChip=$("lastApprovalAtChip");
  if(approvalAtChip){
    const feed=state.preview?null:recordedApprovalAtFeed(eventLog,replayCursor,approvalAtFeedLoaded);
    const approvalAt=lastApprovalAtView(feed,{visible:!!(state.mission&&!state.preview)});
    approvalAtChip.hidden=approvalAt.hidden;
    approvalAtChip.textContent=approvalAt.hidden?"":approvalAt.label;
    approvalAtChip.dataset.known=approvalAt.known?"true":"false";
    if(approvalAt.hidden||!approvalAt.title)approvalAtChip.removeAttribute("title");
    else approvalAtChip.title=approvalAt.title;
    if(approvalAt.hidden)approvalAtChip.removeAttribute("aria-label");
    else approvalAtChip.setAttribute("aria-label",approvalAt.label);
  }
...
function reset(mission, options={}){
  ...
  approvalAtFeedLoaded=options.approvalAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{approvalAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
