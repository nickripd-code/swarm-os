import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedKilledAtFeed,lastKilledAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, killedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const killedAtChip=$("lastKilledAtChip");
  if(killedAtChip){
    const feed=state.preview?null:recordedKilledAtFeed(eventLog,replayCursor,killedAtFeedLoaded);
    const killedAt=lastKilledAtView(feed,{visible:!!(state.mission&&!state.preview)});
    killedAtChip.hidden=killedAt.hidden;
    killedAtChip.textContent=killedAt.hidden?"":killedAt.label;
    killedAtChip.dataset.known=killedAt.known?"true":"false";
    if(killedAt.hidden||!killedAt.title)killedAtChip.removeAttribute("title");
    else killedAtChip.title=killedAt.title;
    if(killedAt.hidden)killedAtChip.removeAttribute("aria-label");
    else killedAtChip.setAttribute("aria-label",killedAt.label);
  }
...
function reset(mission, options={}){
  ...
  killedAtFeedLoaded=options.killedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{killedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
