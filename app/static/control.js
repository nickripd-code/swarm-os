import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedModelCallFeed,modelCallCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, modelCallFeedLoaded=false;
...
function renderHud(){
  ...
  const modelChip=$("modelCallCountChip");
  if(modelChip){
    const visible=!!mission&&!state.preview;
    const feed=visible?recordedModelCallFeed(eventLog,replayCursor,modelCallFeedLoaded):null;
    const modelView=modelCallCountView(feed,{visible});
    modelChip.hidden=modelView.hidden;
    modelChip.textContent=modelView.label;
    modelChip.dataset.known=modelView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  modelCallFeedLoaded=options.modelCallFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{modelCallFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
