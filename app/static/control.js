import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedPauseAtFeed,lastPauseAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, pauseAtFeedLoaded=false;
...
function renderHud(){
  ...
  const pauseAtChip=$("lastPauseAtChip");
  if(pauseAtChip){
    const feed=state.preview?null:recordedPauseAtFeed(eventLog,replayCursor,pauseAtFeedLoaded);
    const pauseAt=lastPauseAtView(feed,{visible:!!(state.mission&&!state.preview)});
    pauseAtChip.hidden=pauseAt.hidden;
    pauseAtChip.textContent=pauseAt.hidden?"":pauseAt.label;
    pauseAtChip.dataset.known=pauseAt.known?"true":"false";
    if(pauseAt.hidden||!pauseAt.title)pauseAtChip.removeAttribute("title");
    else pauseAtChip.title=pauseAt.title;
    if(pauseAt.hidden)pauseAtChip.removeAttribute("aria-label");
    else pauseAtChip.setAttribute("aria-label",pauseAt.label);
  }
...
function reset(mission, options={}){
  ...
  pauseAtFeedLoaded=options.pauseAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{pauseAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
