import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedMissionRunningCountFeed,missionRunningCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, missionRunningFeedLoaded=false;
...
function renderHud(){
  ...
  const missionRunningChip=$("missionRunningCountChip");
  if(missionRunningChip){
    const feed=state.preview?null:recordedMissionRunningCountFeed(eventLog,replayCursor,missionRunningFeedLoaded);
    const missionView=missionRunningCountView(feed,{visible:!!(state.mission&&!state.preview)});
    missionRunningChip.hidden=missionView.hidden;
    missionRunningChip.textContent=missionView.hidden?"":missionView.label;
    missionRunningChip.dataset.known=missionView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  missionRunningFeedLoaded=options.missionRunningFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{missionRunningFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
