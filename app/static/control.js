import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskStoppedCountFeed,taskStoppedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskStoppedFeedLoaded=false;
...
function renderHud(){
  ...
  const taskStopChip=$("taskStoppedCountChip");
  if(taskStopChip){
    const feed=state.preview?null:recordedTaskStoppedCountFeed(eventLog,replayCursor,taskStoppedFeedLoaded);
    const taskStopView=taskStoppedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    taskStopChip.hidden=taskStopView.hidden;
    taskStopChip.textContent=taskStopView.hidden?"":taskStopView.label;
    taskStopChip.dataset.known=taskStopView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  taskStoppedFeedLoaded=options.taskStoppedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskStoppedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
