import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedPauseCountFeed,pauseCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, pauseCountFeedLoaded=false;
...
function renderHud(){
  ...
  const pauseChip=$("pauseCountChip");
  if(pauseChip){
    const feed=state.preview?null:recordedPauseCountFeed(eventLog,replayCursor,pauseCountFeedLoaded);
    const pauseView=pauseCountView(feed,{visible:!!(state.mission&&!state.preview)});
    pauseChip.hidden=pauseView.hidden;
    pauseChip.textContent=pauseView.label;
    pauseChip.dataset.known=pauseView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  pauseCountFeedLoaded=options.pauseCountFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{pauseCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
