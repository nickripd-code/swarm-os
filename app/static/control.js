import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolFailFeed,toolFailCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolFailFeedLoaded=false;
...
function renderHud(){
  ...
  const failChip=$("toolFailCountChip");
  if(failChip){
    const feed=state.preview?null:recordedToolFailFeed(eventLog,replayCursor,toolFailFeedLoaded);
    const failView=toolFailCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  toolFailFeedLoaded=options.toolFailFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolFailFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
