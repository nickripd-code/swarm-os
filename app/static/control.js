import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedBudgetWarningAtFeed,lastBudgetWarningAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, budgetWarnAtFeedLoaded=false;
...
function renderHud(){
  ...
  const budgetWarnAtChip=$("lastBudgetWarningAtChip");
  if(budgetWarnAtChip){
    const feed=state.preview?null:recordedBudgetWarningAtFeed(eventLog,replayCursor,budgetWarnAtFeedLoaded);
    const budgetWarnAt=lastBudgetWarningAtView(feed,{visible:!!(state.mission&&!state.preview)});
    budgetWarnAtChip.hidden=budgetWarnAt.hidden;
    budgetWarnAtChip.textContent=budgetWarnAt.hidden?"":budgetWarnAt.label;
    budgetWarnAtChip.dataset.known=budgetWarnAt.known?"true":"false";
    if(budgetWarnAt.hidden||!budgetWarnAt.title)budgetWarnAtChip.removeAttribute("title");
    else budgetWarnAtChip.title=budgetWarnAt.title;
    if(budgetWarnAt.hidden)budgetWarnAtChip.removeAttribute("aria-label");
    else budgetWarnAtChip.setAttribute("aria-label",budgetWarnAt.label);
  }
...
function reset(mission, options={}){
  ...
  budgetWarnAtFeedLoaded=options.budgetWarnAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{budgetWarnAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
