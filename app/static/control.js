import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmRetryFeed,llmRetryCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmRetryFeedLoaded=false;
...
function renderHud(){
  ...
  const retryChip=$("llmRetryCountChip");
  if(retryChip){
    const feed=state.preview?null:recordedLlmRetryFeed(eventLog,replayCursor,llmRetryFeedLoaded);
    const retryView=llmRetryCountView(feed,{visible:!!(state.mission&&!state.preview)});
    retryChip.hidden=retryView.hidden;
    retryChip.textContent=retryView.label;
    retryChip.dataset.known=retryView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  llmRetryFeedLoaded=options.llmRetryFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmRetryFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
