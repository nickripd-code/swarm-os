import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAgentSpawnedCountFeed,agentSpawnedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, agentSpawnedFeedLoaded=false;
...
function renderHud(){
  ...
  const spawnedChip=$("agentSpawnedCountChip");
  if(spawnedChip){
    const feed=state.preview?null:recordedAgentSpawnedCountFeed(eventLog,replayCursor,agentSpawnedFeedLoaded);
    const spawnedView=agentSpawnedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    spawnedChip.hidden=spawnedView.hidden;
    spawnedChip.textContent=spawnedView.hidden?"":spawnedView.label;
    spawnedChip.dataset.known=spawnedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  agentSpawnedFeedLoaded=options.agentSpawnedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{agentSpawnedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
