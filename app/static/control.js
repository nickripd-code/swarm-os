import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedOrgChangedCountFeed,orgChangedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, orgChangedCountFeedLoaded=false;
...
function renderHud(){
  ...
  const orgChangedChip=$("orgChangedCountChip");
  if(orgChangedChip){
    const feed=state.preview?null:recordedOrgChangedCountFeed(eventLog,replayCursor,orgChangedCountFeedLoaded);
    const orgChangedView=orgChangedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    orgChangedChip.hidden=orgChangedView.hidden;
    orgChangedChip.textContent=orgChangedView.hidden?"":orgChangedView.label;
    orgChangedChip.dataset.known=orgChangedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  orgChangedCountFeedLoaded=options.orgChangedCountFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        orgChangedCountFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{orgChangedCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
