import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedSpendAtFeed,lastSpendAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, spendAtFeedLoaded=false;
...
function renderHud(){
  ...
  const spendAtChip=$("lastSpendAtChip");
  if(spendAtChip){
    const feed=state.preview?null:recordedSpendAtFeed(eventLog,replayCursor,spendAtFeedLoaded);
    const spendAt=lastSpendAtView(feed,{visible:!!(state.mission&&!state.preview)});
    spendAtChip.hidden=spendAt.hidden;
    spendAtChip.textContent=spendAt.hidden?"":spendAt.label;
    spendAtChip.dataset.known=spendAt.known?"true":"false";
    if(spendAt.hidden||!spendAt.title)spendAtChip.removeAttribute("title");
    else spendAtChip.title=spendAt.title;
    if(spendAt.hidden)spendAtChip.removeAttribute("aria-label");
    else spendAtChip.setAttribute("aria-label",spendAt.label);
  }
...
function reset(mission, options={}){
  ...
  spendAtFeedLoaded=options.spendAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{spendAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
