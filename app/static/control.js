import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAnswerConsumedAtFeed,lastAnswerConsumedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, answerConsumedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const answerConsumedAtChip=$("lastAnswerConsumedAtChip");
  if(answerConsumedAtChip){
    const feed=state.preview?null:recordedAnswerConsumedAtFeed(eventLog,replayCursor,answerConsumedAtFeedLoaded);
    const answerConsumedAt=lastAnswerConsumedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    answerConsumedAtChip.hidden=answerConsumedAt.hidden;
    answerConsumedAtChip.textContent=answerConsumedAt.hidden?"":answerConsumedAt.label;
    answerConsumedAtChip.dataset.known=answerConsumedAt.known?"true":"false";
    if(answerConsumedAt.hidden||!answerConsumedAt.title)answerConsumedAtChip.removeAttribute("title");
    else answerConsumedAtChip.title=answerConsumedAt.title;
    if(answerConsumedAt.hidden)answerConsumedAtChip.removeAttribute("aria-label");
    else answerConsumedAtChip.setAttribute("aria-label",answerConsumedAt.label);
  }
...
function reset(mission, options={}){
  ...
  answerConsumedAtFeedLoaded=options.answerConsumedAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        answerConsumedAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{answerConsumedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
