import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,waitingAgeChip,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive} from "./state.mjs";
...
function renderHud(){
  ...
  let waitingAge=null;
  if($("waitingAgeChip")){
    waitingAge=waitingAgeChip(state,{now:Date.now()});
    const chip=$("waitingAgeChip");
    chip.hidden=waitingAge.hidden;
    chip.textContent=waitingAge.hidden?"":waitingAge.label;
    chip.className="hud-chip waiting-age "+(waitingAge.known?"known":"unavailable");
    if(waitingAge.hidden||!waitingAge.title)chip.removeAttribute("title");
    else chip.title=waitingAge.title;
  }
  const running=!!mission&&!terminal.has(status)&&replayLive&&!state.preview;
  const ageTicking=!!waitingAge&&waitingAge.known&&!waitingAge.hidden;
