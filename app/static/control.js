import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedMissionStartedCountFeed,missionStartedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, missionStartedFeedLoaded=false;
...
function renderHud(){
  ...
  const missionStartedChip=$("missionStartedCountChip");
  if(missionStartedChip){
    const feed=state.preview?null:recordedMissionStartedCountFeed(eventLog,replayCursor,missionStartedFeedLoaded);
    const missionView=missionStartedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    missionStartedChip.hidden=missionView.hidden;
    missionStartedChip.textContent=missionView.hidden?"":missionView.label;
    missionStartedChip.dataset.known=missionView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  missionStartedFeedLoaded=options.missionStartedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{missionStartedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
