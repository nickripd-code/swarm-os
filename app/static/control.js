import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedQuestionAtFeed,lastQuestionAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, questionAtFeedLoaded=false;
...
function renderHud(){
  ...
  const questionAtChip=$("lastQuestionAtChip");
  if(questionAtChip){
    const feed=state.preview?null:recordedQuestionAtFeed(eventLog,replayCursor,questionAtFeedLoaded);
    const questionAt=lastQuestionAtView(feed,{visible:!!(state.mission&&!state.preview)});
    questionAtChip.hidden=questionAt.hidden;
    questionAtChip.textContent=questionAt.hidden?"":questionAt.label;
    questionAtChip.dataset.known=questionAt.known?"true":"false";
    if(questionAt.hidden||!questionAt.title)questionAtChip.removeAttribute("title");
    else questionAtChip.title=questionAt.title;
    if(questionAt.hidden)questionAtChip.removeAttribute("aria-label");
    else questionAtChip.setAttribute("aria-label",questionAt.label);
  }
...
function reset(mission, options={}){
  ...
  questionAtFeedLoaded=options.questionAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{questionAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
