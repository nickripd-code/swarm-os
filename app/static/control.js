import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedResumeAtFeed,lastResumeAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, resumeAtFeedLoaded=false;
...
function renderHud(){
  ...
  const resumeAtChip=$("lastResumeAtChip");
  if(resumeAtChip){
    const feed=state.preview?null:recordedResumeAtFeed(eventLog,replayCursor,resumeAtFeedLoaded);
    const resumeAt=lastResumeAtView(feed,{visible:!!(state.mission&&!state.preview)});
    resumeAtChip.hidden=resumeAt.hidden;
    resumeAtChip.textContent=resumeAt.hidden?"":resumeAt.label;
    resumeAtChip.dataset.known=resumeAt.known?"true":"false";
    if(resumeAt.hidden||!resumeAt.title)resumeAtChip.removeAttribute("title");
    else resumeAtChip.title=resumeAt.title;
    if(resumeAt.hidden)resumeAtChip.removeAttribute("aria-label");
    else resumeAtChip.setAttribute("aria-label",resumeAt.label);
  }
...
function reset(mission, options={}){
  ...
  resumeAtFeedLoaded=options.resumeAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{resumeAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
