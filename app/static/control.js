import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJudgeDecisionAtFeed,lastJudgeDecisionAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, judgeDecisionAtFeedLoaded=false;
...
function renderHud(){
  ...
  const judgeDecisionAtChip=$("lastJudgeDecisionAtChip");
  if(judgeDecisionAtChip){
    const feed=state.preview?null:recordedJudgeDecisionAtFeed(eventLog,replayCursor,judgeDecisionAtFeedLoaded);
    const judgeDecisionAt=lastJudgeDecisionAtView(feed,{visible:!!(state.mission&&!state.preview)});
    judgeDecisionAtChip.hidden=judgeDecisionAt.hidden;
    judgeDecisionAtChip.textContent=judgeDecisionAt.hidden?"":judgeDecisionAt.label;
    judgeDecisionAtChip.dataset.known=judgeDecisionAt.known?"true":"false";
    if(judgeDecisionAt.hidden||!judgeDecisionAt.title)judgeDecisionAtChip.removeAttribute("title");
    else judgeDecisionAtChip.title=judgeDecisionAt.title;
    if(judgeDecisionAt.hidden)judgeDecisionAtChip.removeAttribute("aria-label");
    else judgeDecisionAtChip.setAttribute("aria-label",judgeDecisionAt.label);
  }
...
function reset(mission, options={}){
  ...
  judgeDecisionAtFeedLoaded=options.judgeDecisionAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        judgeDecisionAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{judgeDecisionAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
