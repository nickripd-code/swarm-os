import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedStopCountFeed,stopCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, stopCountFeedLoaded=false;
...
function renderHud(){
  ...
  const stopChip=$("stopCountChip");
  if(stopChip){
    const feed=state.preview?null:recordedStopCountFeed(eventLog,replayCursor,stopCountFeedLoaded);
    const stopView=stopCountView(feed,{visible:!!(state.mission&&!state.preview)});
    stopChip.hidden=stopView.hidden;
    stopChip.textContent=stopView.label;
    stopChip.dataset.known=stopView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  stopCountFeedLoaded=options.stopCountFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{stopCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
