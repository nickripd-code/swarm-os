import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmSuccessFeed,llmSuccessCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmSuccessFeedLoaded=false;
...
function renderHud(){
  ...
  const successChip=$("llmSuccessCountChip");
  if(successChip){
    const feed=state.preview?null:recordedLlmSuccessFeed(eventLog,replayCursor,llmSuccessFeedLoaded);
    const successView=llmSuccessCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  llmSuccessFeedLoaded=options.llmSuccessFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmSuccessFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
