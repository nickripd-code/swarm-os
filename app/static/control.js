import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedRetiredAtFeed,lastRetiredAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, retiredAtFeedLoaded=false;
...
function renderHud(){
  ...
  const retiredAtChip=$("lastRetiredAtChip");
  if(retiredAtChip){
    const feed=state.preview?null:recordedRetiredAtFeed(eventLog,replayCursor,retiredAtFeedLoaded);
    const retiredAt=lastRetiredAtView(feed,{visible:!!(state.mission&&!state.preview)});
    retiredAtChip.hidden=retiredAt.hidden;
    retiredAtChip.textContent=retiredAt.hidden?"":retiredAt.label;
    retiredAtChip.dataset.known=retiredAt.known?"true":"false";
    if(retiredAt.hidden||!retiredAt.title)retiredAtChip.removeAttribute("title");
    else retiredAtChip.title=retiredAt.title;
    if(retiredAt.hidden)retiredAtChip.removeAttribute("aria-label");
    else retiredAtChip.setAttribute("aria-label",retiredAt.label);
  }
...
function reset(mission, options={}){
  ...
  retiredAtFeedLoaded=options.retiredAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{retiredAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
