import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAnswerAtFeed,lastAnswerAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, answerAtFeedLoaded=false;
...
function renderHud(){
  ...
  const answerAtChip=$("lastAnswerAtChip");
  if(answerAtChip){
    const feed=state.preview?null:recordedAnswerAtFeed(eventLog,replayCursor,answerAtFeedLoaded);
    const answerAt=lastAnswerAtView(feed,{visible:!!(state.mission&&!state.preview)});
    answerAtChip.hidden=answerAt.hidden;
    answerAtChip.textContent=answerAt.hidden?"":answerAt.label;
    answerAtChip.dataset.known=answerAt.known?"true":"false";
    if(answerAt.hidden||!answerAt.title)answerAtChip.removeAttribute("title");
    else answerAtChip.title=answerAt.title;
    if(answerAt.hidden)answerAtChip.removeAttribute("aria-label");
    else answerAtChip.setAttribute("aria-label",answerAt.label);
  }
...
function reset(mission, options={}){
  ...
  answerAtFeedLoaded=options.answerAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{answerAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
