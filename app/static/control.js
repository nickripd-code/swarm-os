import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedParkAtFeed,lastParkAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, parkAtFeedLoaded=false;
...
function renderHud(){
  ...
  const parkAtChip=$("lastParkAtChip");
  if(parkAtChip){
    const feed=state.preview?null:recordedParkAtFeed(eventLog,replayCursor,parkAtFeedLoaded);
    const parkAt=lastParkAtView(feed,{visible:!!(state.mission&&!state.preview)});
    parkAtChip.hidden=parkAt.hidden;
    parkAtChip.textContent=parkAt.hidden?"":parkAt.label;
    parkAtChip.dataset.known=parkAt.known?"true":"false";
    if(parkAt.hidden||!parkAt.title)parkAtChip.removeAttribute("title");
    else parkAtChip.title=parkAt.title;
    if(parkAt.hidden)parkAtChip.removeAttribute("aria-label");
    else parkAtChip.setAttribute("aria-label",parkAt.label);
  }
...
function reset(mission, options={}){
  ...
  parkAtFeedLoaded=options.parkAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{parkAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
