import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmStartedCountFeed,llmStartedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmStartedCountFeedStatus="pending";
...
function renderHud(){
  ...
  const llmStartedChip=$("llmStartedCountChip");
  if(llmStartedChip){
    const pending=llmStartedCountFeedStatus==="pending";
    const ready=llmStartedCountFeedStatus==="ready";
    const feed=state.preview||pending?null:recordedLlmStartedCountFeed(eventLog,replayCursor,ready);
    const startedCount=llmStartedCountView(feed,{visible:!!(state.mission&&!state.preview),computable:!pending});
    llmStartedChip.hidden=startedCount.hidden;
    llmStartedChip.textContent=startedCount.label;
    llmStartedChip.dataset.known=startedCount.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  llmStartedCountFeedStatus=options.llmStartedCountFeedStatus==="ready"||options.llmStartedCountFeedStatus==="missing"?options.llmStartedCountFeedStatus:"pending";
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        llmStartedCountFeedStatus="ready";
      }else llmStartedCountFeedStatus="missing";
    }catch{llmStartedCountFeedStatus="missing";}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmStartedCountFeedStatus:"ready"});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
