import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedPendingToolFeed,pendingToolCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, pendingToolFeedLoaded=false;
...
function renderHud(){
  ...
  const pendingChip=$("pendingToolCountChip");
  if(pendingChip){
    const feed=state.preview?null:recordedPendingToolFeed(eventLog,replayCursor,pendingToolFeedLoaded);
    const pendingView=pendingToolCountView(feed,{visible:!!(state.mission&&!state.preview)});
    pendingChip.hidden=pendingView.hidden;
    pendingChip.textContent=pendingView.label;
...
function reset(mission, options={}){
  ...
  pendingToolFeedLoaded=options.pendingToolFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{pendingToolFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
