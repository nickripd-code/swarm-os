import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedActiveLlmFeed,activeLlmCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, activeLlmFeedLoaded=false;
...
function renderHud(){
  ...
  const llmChip=$("activeLlmCountChip");
  if(llmChip){
    const feed=state.preview?null:recordedActiveLlmFeed(eventLog,replayCursor,activeLlmFeedLoaded);
    const llmView=activeLlmCountView(feed,{visible:!!(state.mission&&!state.preview)});
    llmChip.hidden=llmView.hidden;
    llmChip.textContent=llmView.label;
    llmChip.dataset.known=llmView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  activeLlmFeedLoaded=options.activeLlmFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{activeLlmFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
