import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolOkAtFeed,lastToolOkAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolOkAtFeedLoaded=false;
...
function renderHud(){
  ...
  const toolOkAtChip=$("lastToolOkAtChip");
  if(toolOkAtChip){
    const feed=state.preview?null:recordedToolOkAtFeed(eventLog,replayCursor,toolOkAtFeedLoaded);
    const toolOkAt=lastToolOkAtView(feed,{visible:!!(state.mission&&!state.preview)});
    toolOkAtChip.hidden=toolOkAt.hidden;
    toolOkAtChip.textContent=toolOkAt.hidden?"":toolOkAt.label;
    toolOkAtChip.dataset.known=toolOkAt.known?"true":"false";
    if(toolOkAt.hidden||!toolOkAt.title)toolOkAtChip.removeAttribute("title");
    else toolOkAtChip.title=toolOkAt.title;
    if(toolOkAt.hidden)toolOkAtChip.removeAttribute("aria-label");
    else toolOkAtChip.setAttribute("aria-label",toolOkAt.label);
  }
...
function reset(mission,options={}){
  ...
  toolOkAtFeedLoaded=options.toolOkAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolOkAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
