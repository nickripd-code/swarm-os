import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAvgToolLatencyFeed,avgToolLatencyView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, avgToolLatencyFeedLoaded=false;
...
function renderHud(){
  ...
  const avgToolLatencyChip=$("avgToolLatencyChip");
  if(avgToolLatencyChip){
    const feed=state.preview?null:recordedAvgToolLatencyFeed(eventLog,replayCursor,avgToolLatencyFeedLoaded);
    const avgToolLatency=avgToolLatencyView(feed,{visible:!!(state.mission&&!state.preview)});
    avgToolLatencyChip.hidden=avgToolLatency.hidden;
    avgToolLatencyChip.textContent=avgToolLatency.label;
    avgToolLatencyChip.dataset.known=avgToolLatency.known?"true":"false";
  }
...
function reset(mission, options={}){
  stopReplayPlay();
  avgToolLatencyFeedLoaded=options.avgToolLatencyFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{avgToolLatencyFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
