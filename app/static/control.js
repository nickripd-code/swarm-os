import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,lastEventAtView} from "./state.mjs";
...
function renderLastEventAtChip(){
  const el=$("lastEventAtChip");
  if(!el)return;
  const view=lastEventAtView(eventLog,{mission:state.mission,preview:!!state.preview});
  el.hidden=!view.visible;
  el.textContent=view.visible?view.label:"";
  if(view.title)el.title=view.title;else el.removeAttribute("title");
  if(view.visible&&view.ariaLabel)el.setAttribute("aria-label",view.ariaLabel);
