import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJobFailedCountFeed,jobFailedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, jobFailedFeedLoaded=false;
...
function renderHud(){
  ...
  const jobFailedChip=$("jobFailedCountChip");
  if(jobFailedChip){
    const feed=state.preview?null:recordedJobFailedCountFeed(eventLog,replayCursor,jobFailedFeedLoaded);
    const jobView=jobFailedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    jobFailedChip.hidden=jobView.hidden;
    jobFailedChip.textContent=jobView.hidden?"":jobView.label;
    jobFailedChip.dataset.known=jobView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  jobFailedFeedLoaded=options.jobFailedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{jobFailedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
