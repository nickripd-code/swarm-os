import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJobEnqueuedAtFeed,lastJobEnqueuedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, jobEnqueuedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const jobEnqueuedAtChip=$("lastJobEnqueuedAtChip");
  if(jobEnqueuedAtChip){
    const feed=state.preview?null:recordedJobEnqueuedAtFeed(eventLog,replayCursor,jobEnqueuedAtFeedLoaded);
    const jobEnqueuedAt=lastJobEnqueuedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    jobEnqueuedAtChip.hidden=jobEnqueuedAt.hidden;
    jobEnqueuedAtChip.textContent=jobEnqueuedAt.hidden?"":jobEnqueuedAt.label;
    jobEnqueuedAtChip.dataset.known=jobEnqueuedAt.known?"true":"false";
    if(jobEnqueuedAt.hidden||!jobEnqueuedAt.title)jobEnqueuedAtChip.removeAttribute("title");
    else jobEnqueuedAtChip.title=jobEnqueuedAt.title;
    if(jobEnqueuedAt.hidden)jobEnqueuedAtChip.removeAttribute("aria-label");
    else jobEnqueuedAtChip.setAttribute("aria-label",jobEnqueuedAt.label);
  }
...
function reset(mission, options={}){
  ...
  jobEnqueuedAtFeedLoaded=options.jobEnqueuedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{jobEnqueuedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
