import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLeaseClaimedAtFeed,lastLeaseClaimedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, leaseClaimedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const leaseClaimedAtChip=$("lastLeaseClaimedAtChip");
  if(leaseClaimedAtChip){
    const feed=state.preview?null:recordedLeaseClaimedAtFeed(eventLog,replayCursor,leaseClaimedAtFeedLoaded);
    const leaseClaimedAt=lastLeaseClaimedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    leaseClaimedAtChip.hidden=leaseClaimedAt.hidden;
    leaseClaimedAtChip.textContent=leaseClaimedAt.hidden?"":leaseClaimedAt.label;
    leaseClaimedAtChip.dataset.known=leaseClaimedAt.known?"true":"false";
    if(leaseClaimedAt.hidden||!leaseClaimedAt.title)leaseClaimedAtChip.removeAttribute("title");
    else leaseClaimedAtChip.title=leaseClaimedAt.title;
    if(leaseClaimedAt.hidden)leaseClaimedAtChip.removeAttribute("aria-label");
    else leaseClaimedAtChip.setAttribute("aria-label",leaseClaimedAt.label);
  }
...
function reset(mission, options={}){
  ...
  leaseClaimedAtFeedLoaded=options.leaseClaimedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{leaseClaimedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
