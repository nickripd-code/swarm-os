import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedResumeCountFeed,resumeCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, resumeCountFeedLoaded=false;
...
function renderHud(){
  ...
  const resumeChip=$("resumeCountChip");
  if(resumeChip){
    const feed=state.preview?null:recordedResumeCountFeed(eventLog,replayCursor,resumeCountFeedLoaded);
    const resumeView=resumeCountView(feed,{visible:!!(state.mission&&!state.preview)});
    resumeChip.hidden=resumeView.hidden;
    resumeChip.textContent=resumeView.label;
    resumeChip.dataset.known=resumeView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  resumeCountFeedLoaded=options.resumeCountFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{resumeCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
