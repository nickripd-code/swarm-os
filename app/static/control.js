import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAgentMessageCountFeed,agentMessageCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, agentMessageFeedLoaded=false;
...
function renderHud(){
  ...
  const agentMessageChip=$("agentMessageCountChip");
  if(agentMessageChip){
    const feed=state.preview?null:recordedAgentMessageCountFeed(eventLog,replayCursor,agentMessageFeedLoaded);
    const agentMessageView=agentMessageCountView(feed,{visible:!!(state.mission&&!state.preview)});
    agentMessageChip.hidden=agentMessageView.hidden;
    agentMessageChip.textContent=agentMessageView.hidden?"":agentMessageView.label;
    agentMessageChip.dataset.known=agentMessageView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  agentMessageFeedLoaded=options.agentMessageFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{agentMessageFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
