import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedPendingLlmFeed,pendingLlmCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, pendingLlmFeedLoaded=false;
...
function renderHud(){
  ...
  const pendingLlmChip=$("pendingLlmCountChip");
  if(pendingLlmChip){
    const feed=state.preview?null:recordedPendingLlmFeed(eventLog,replayCursor,pendingLlmFeedLoaded);
    const pendingView=pendingLlmCountView(feed,{visible:!!(state.mission&&!state.preview)});
    pendingLlmChip.hidden=pendingView.hidden;
    pendingLlmChip.textContent=pendingView.label;
    pendingLlmChip.dataset.known=pendingView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  pendingLlmFeedLoaded=options.pendingLlmFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{pendingLlmFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
