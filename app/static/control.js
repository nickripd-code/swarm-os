import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolFailRateFeed,toolFailRateView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolFailRateFeedLoaded=false;
...
function renderHud(){
  ...
  const rateChip=$("toolFailRateChip");
  if(rateChip){
    const feed=state.preview?null:recordedToolFailRateFeed(eventLog,replayCursor,toolFailRateFeedLoaded);
    const rateView=toolFailRateView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  toolFailRateFeedLoaded=options.toolFailRateFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolFailRateFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
