import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedUserAnswerFeed,userAnswerCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, userAnswerFeedLoaded=false;
...
function renderHud(){
  ...
  const answerChip=$("userAnswerCountChip");
  if(answerChip){
    const feed=state.preview?null:recordedUserAnswerFeed(eventLog,replayCursor,userAnswerFeedLoaded);
    const answerView=userAnswerCountView(feed,{visible:!!(state.mission&&!state.preview)});
    answerChip.hidden=answerView.hidden;
    answerChip.textContent=answerView.label;
    answerChip.dataset.known=answerView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  userAnswerFeedLoaded=options.userAnswerFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{userAnswerFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
