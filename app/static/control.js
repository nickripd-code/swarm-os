import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedResumeRequestedCountFeed,resumeRequestedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, resumeRequestedFeedLoaded=false;
...
function renderHud(){
  ...
  const resumeRequestedChip=$("resumeRequestedCountChip");
  if(resumeRequestedChip){
    const feed=state.preview?null:recordedResumeRequestedCountFeed(eventLog,replayCursor,resumeRequestedFeedLoaded);
    const requestedView=resumeRequestedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    resumeRequestedChip.hidden=requestedView.hidden;
    resumeRequestedChip.textContent=requestedView.hidden?"":requestedView.label;
    resumeRequestedChip.dataset.known=requestedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  resumeRequestedFeedLoaded=options.resumeRequestedFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        resumeRequestedFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{resumeRequestedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
