import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedCompleteCountFeed,completeCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, completeCountFeedLoaded=false;
...
function renderHud(){
  ...
  const completeChip=$("completeCountChip");
  if(completeChip){
    const feed=state.preview?null:recordedCompleteCountFeed(eventLog,replayCursor,completeCountFeedLoaded);
    const completeView=completeCountView(feed,{visible:!!(state.mission&&!state.preview)});
    completeChip.hidden=completeView.hidden;
    completeChip.textContent=completeView.label;
    completeChip.dataset.known=completeView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  completeCountFeedLoaded=options.completeCountFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{completeCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
