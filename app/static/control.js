import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedBlockAtFeed,lastBlockAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, blockAtFeedLoaded=false;
...
function renderHud(){
  ...
  const blockAtChip=$("lastBlockAtChip");
  if(blockAtChip){
    const feed=state.preview?null:recordedBlockAtFeed(eventLog,replayCursor,blockAtFeedLoaded);
    const blockAt=lastBlockAtView(feed,{visible:!!(state.mission&&!state.preview)});
    blockAtChip.hidden=blockAt.hidden;
    blockAtChip.textContent=blockAt.hidden?"":blockAt.label;
    blockAtChip.dataset.known=blockAt.known?"true":"false";
    if(blockAt.hidden||!blockAt.title)blockAtChip.removeAttribute("title");
    else blockAtChip.title=blockAt.title;
    if(blockAt.hidden)blockAtChip.removeAttribute("aria-label");
    else blockAtChip.setAttribute("aria-label",blockAt.label);
  }
...
function reset(mission, options={}){
  ...
  blockAtFeedLoaded=options.blockAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{blockAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
