import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedResumeRequestedAtFeed,lastResumeRequestedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, resumeRequestedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const resumeRequestedAtChip=$("lastResumeRequestedAtChip");
  if(resumeRequestedAtChip){
    const feed=state.preview?null:recordedResumeRequestedAtFeed(eventLog,replayCursor,resumeRequestedAtFeedLoaded);
    const resumeRequestedAt=lastResumeRequestedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    resumeRequestedAtChip.hidden=resumeRequestedAt.hidden;
    resumeRequestedAtChip.textContent=resumeRequestedAt.hidden?"":resumeRequestedAt.label;
    resumeRequestedAtChip.dataset.known=resumeRequestedAt.known?"true":"false";
    if(resumeRequestedAt.hidden||!resumeRequestedAt.title)resumeRequestedAtChip.removeAttribute("title");
    else resumeRequestedAtChip.title=resumeRequestedAt.title;
    if(resumeRequestedAt.hidden)resumeRequestedAtChip.removeAttribute("aria-label");
    else resumeRequestedAtChip.setAttribute("aria-label",resumeRequestedAt.label);
  }
...
function reset(mission,options={}){
  ...
  resumeRequestedAtFeedLoaded=options.resumeRequestedAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        resumeRequestedAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{resumeRequestedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
