import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,completeAgeChip,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive} from "./state.mjs";
...
function renderHud(){
  ...
  let completeAge=null;
  if($("completeAgeChip")){
    completeAge=completeAgeChip(state,{now:Date.now()});
    const chip=$("completeAgeChip");
    chip.hidden=completeAge.hidden;
    chip.textContent=completeAge.hidden?"":completeAge.label;
    chip.className="hud-chip complete-age "+(completeAge.known?"known":"unavailable");
    if(completeAge.hidden||!completeAge.title)chip.removeAttribute("title");
    else chip.title=completeAge.title;
  }
  const running=!!mission&&!terminal.has(status)&&replayLive&&!state.preview;
  const ageTicking=!!completeAge&&completeAge.known&&!completeAge.hidden;
