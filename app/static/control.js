import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedWaitingCountFeed,waitingCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, waitingCountFeedLoaded=false;
...
function renderHud(){
  ...
  const waitingChip=$("waitingCountChip");
  if(waitingChip){
    const feed=state.preview?null:recordedWaitingCountFeed(eventLog,replayCursor,waitingCountFeedLoaded);
    const waitingView=waitingCountView(feed,{visible:!!(state.mission&&!state.preview)});
    waitingChip.hidden=waitingView.hidden;
    waitingChip.textContent=waitingView.label;
    waitingChip.dataset.known=waitingView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  waitingCountFeedLoaded=options.waitingCountFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{waitingCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
