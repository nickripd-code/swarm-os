import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedRetryAtFeed,lastRetryAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, retryAtFeedLoaded=false;
...
function renderHud(){
  ...
  const retryAtChip=$("lastRetryAtChip");
  if(retryAtChip){
    const feed=state.preview?null:recordedRetryAtFeed(eventLog,replayCursor,retryAtFeedLoaded);
    const retryAt=lastRetryAtView(feed,{visible:!!(state.mission&&!state.preview)});
    retryAtChip.hidden=retryAt.hidden;
    retryAtChip.textContent=retryAt.hidden?"":retryAt.label;
    retryAtChip.dataset.known=retryAt.known?"true":"false";
    if(retryAt.hidden||!retryAt.title)retryAtChip.removeAttribute("title");
    else retryAtChip.title=retryAt.title;
    if(retryAt.hidden)retryAtChip.removeAttribute("aria-label");
    else retryAtChip.setAttribute("aria-label",retryAt.label);
  }
...
function reset(mission, options={}){
  ...
  retryAtFeedLoaded=options.retryAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{retryAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
