import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLeaseExpiredCountFeed,leaseExpiredCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, leaseExpiredFeedLoaded=false;
...
function renderHud(){
  ...
  const leaseChip=$("leaseExpiredCountChip");
  if(leaseChip){
    const feed=state.preview?null:recordedLeaseExpiredCountFeed(eventLog,replayCursor,leaseExpiredFeedLoaded);
    const leaseView=leaseExpiredCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  leaseExpiredFeedLoaded=options.leaseExpiredFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{leaseExpiredFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
