import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAgentRetiredCountFeed,agentRetiredCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, agentRetiredFeedLoaded=false;
...
function renderHud(){
  ...
  const retiredChip=$("agentRetiredCountChip");
  if(retiredChip){
    const feed=state.preview?null:recordedAgentRetiredCountFeed(eventLog,replayCursor,agentRetiredFeedLoaded);
    const retiredView=agentRetiredCountView(feed,{visible:!!(state.mission&&!state.preview)});
    retiredChip.hidden=retiredView.hidden;
    retiredChip.textContent=retiredView.hidden?"":retiredView.label;
    retiredChip.dataset.known=retiredView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  agentRetiredFeedLoaded=options.agentRetiredFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{agentRetiredFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
