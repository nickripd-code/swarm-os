import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskCompletedCountFeed,taskCompletedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskCompletedFeedLoaded=false;
...
function renderHud(){
  ...
  const taskChip=$("taskCompletedCountChip");
  if(taskChip){
    const feed=state.preview?null:recordedTaskCompletedCountFeed(eventLog,replayCursor,taskCompletedFeedLoaded);
    const taskView=taskCompletedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    taskChip.hidden=taskView.hidden;
    taskChip.textContent=taskView.hidden?"":taskView.label;
    taskChip.dataset.known=taskView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  taskCompletedFeedLoaded=options.taskCompletedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskCompletedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
