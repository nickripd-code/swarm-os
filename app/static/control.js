import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedBudgetUpdateCountFeed,budgetUpdateCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, budgetUpdateCountFeedLoaded=false;
...
function renderHud(){
  ...
  const budgetUpdateChip=$("budgetUpdateCountChip");
  if(budgetUpdateChip){
    const feed=state.preview?null:recordedBudgetUpdateCountFeed(eventLog,replayCursor,budgetUpdateCountFeedLoaded);
    const updateCount=budgetUpdateCountView(feed,{visible:!!(state.mission&&!state.preview)});
    budgetUpdateChip.hidden=updateCount.hidden;
    budgetUpdateChip.textContent=updateCount.label;
    budgetUpdateChip.dataset.known=updateCount.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  budgetUpdateCountFeedLoaded=options.budgetUpdateCountFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        budgetUpdateCountFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{budgetUpdateCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
