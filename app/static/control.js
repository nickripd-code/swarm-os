import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationPassRateFeed,verificationPassRateView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationPassRateFeedLoaded=false;
...
function renderHud(){
  ...
  const verificationPassChip=$("verificationPassRateChip");
  if(verificationPassChip){
    const feed=state.preview?null:recordedVerificationPassRateFeed(eventLog,replayCursor,verificationPassRateFeedLoaded);
    const verificationPassView=verificationPassRateView(feed,{visible:!!(state.mission&&!state.preview)});
    verificationPassChip.hidden=verificationPassView.hidden;
    verificationPassChip.textContent=verificationPassView.label;
    verificationPassChip.dataset.known=verificationPassView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  verificationPassRateFeedLoaded=options.verificationPassRateFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        verificationPassRateFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationPassRateFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
