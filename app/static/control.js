import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedPaymentFeed,paymentCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, paymentFeedLoaded=false;
...
function renderHud(){
  ...
  const paymentChip=$("paymentCountChip");
  if(paymentChip){
    const feed=state.preview?null:recordedPaymentFeed(eventLog,replayCursor,paymentFeedLoaded);
    const paymentView=paymentCountView(feed,{visible:!!(state.mission&&!state.preview)});
    paymentChip.hidden=paymentView.hidden;
    paymentChip.textContent=paymentView.label;
    paymentChip.dataset.known=paymentView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  paymentFeedLoaded=options.paymentFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{paymentFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
