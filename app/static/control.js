import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedPlannerProposalFeed,plannerProposalCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, plannerProposalFeedLoaded=false;
...
function renderHud(){
  ...
  const proposalChip=$("plannerProposalCountChip");
  if(proposalChip){
    const feed=state.preview?null:recordedPlannerProposalFeed(eventLog,replayCursor,plannerProposalFeedLoaded);
    const proposalView=plannerProposalCountView(feed,{visible:!!(state.mission&&!state.preview)});
    proposalChip.hidden=proposalView.hidden;
    proposalChip.textContent=proposalView.label;
    proposalChip.dataset.known=proposalView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  plannerProposalFeedLoaded=options.plannerProposalFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        plannerProposalFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{plannerProposalFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
