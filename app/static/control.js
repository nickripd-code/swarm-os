import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLeaseReleasedCountFeed,leaseReleasedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, leaseReleasedFeedLoaded=false;
...
function renderHud(){
  ...
  const leaseChip=$("leaseReleasedCountChip");
  if(leaseChip){
    const feed=state.preview?null:recordedLeaseReleasedCountFeed(eventLog,replayCursor,leaseReleasedFeedLoaded);
    const leaseView=leaseReleasedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    leaseChip.hidden=leaseView.hidden;
    leaseChip.textContent=leaseView.hidden?"":leaseView.label;
    leaseChip.dataset.known=leaseView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  leaseReleasedFeedLoaded=options.leaseReleasedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{leaseReleasedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
