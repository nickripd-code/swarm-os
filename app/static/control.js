import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedApprovalQuestionFeed,approvalQuestionCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, approvalQuestionFeedLoaded=false;
...
function renderHud(){
  ...
  const approvalQuestionChip=$("approvalQuestionCountChip");
  if(approvalQuestionChip){
    const feed=state.preview?null:recordedApprovalQuestionFeed(eventLog,replayCursor,approvalQuestionFeedLoaded);
    const approvalQuestionView=approvalQuestionCountView(feed,{visible:!!(state.mission&&!state.preview)});
    approvalQuestionChip.hidden=approvalQuestionView.hidden;
    approvalQuestionChip.textContent=approvalQuestionView.label;
    approvalQuestionChip.dataset.known=approvalQuestionView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  approvalQuestionFeedLoaded=options.approvalQuestionFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        approvalQuestionFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{approvalQuestionFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
