import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJobEnqueuedCountFeed,jobEnqueuedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, jobEnqueuedFeedLoaded=false;
...
function renderHud(){
  ...
  const jobChip=$("jobEnqueuedCountChip");
  if(jobChip){
    const feed=state.preview?null:recordedJobEnqueuedCountFeed(eventLog,replayCursor,jobEnqueuedFeedLoaded);
    const jobView=jobEnqueuedCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  jobEnqueuedFeedLoaded=options.jobEnqueuedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{jobEnqueuedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
