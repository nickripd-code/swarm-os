import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmFailoverFeed,llmFailoverCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmFailoverFeedLoaded=false;
...
function renderHud(){
  ...
  const failoverChip=$("llmFailoverCountChip");
  if(failoverChip){
    const feed=state.preview?null:recordedLlmFailoverFeed(eventLog,replayCursor,llmFailoverFeedLoaded);
    const failoverView=llmFailoverCountView(feed,{visible:!!(state.mission&&!state.preview)});
    failoverChip.hidden=failoverView.hidden;
    failoverChip.textContent=failoverView.label;
    failoverChip.dataset.known=failoverView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  llmFailoverFeedLoaded=options.llmFailoverFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmFailoverFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
