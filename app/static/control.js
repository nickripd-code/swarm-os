import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLeaseClaimedCountFeed,leaseClaimedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, leaseClaimedFeedLoaded=false;
...
function renderHud(){
  ...
  const leaseChip=$("leaseClaimedCountChip");
  if(leaseChip){
    const feed=state.preview?null:recordedLeaseClaimedCountFeed(eventLog,replayCursor,leaseClaimedFeedLoaded);
    const leaseView=leaseClaimedCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  leaseClaimedFeedLoaded=options.leaseClaimedFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{leaseClaimedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
