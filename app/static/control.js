import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationFailFeed,verificationFailCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationFailFeedLoaded=false;
...
function renderHud(){
  ...
  const failChip=$("verificationFailCountChip");
  if(failChip){
    const feed=state.preview?null:recordedVerificationFailFeed(eventLog,replayCursor,verificationFailFeedLoaded);
    const failView=verificationFailCountView(feed,{visible:!!(state.mission&&!state.preview)});
    failChip.hidden=failView.hidden;
    failChip.textContent=failView.label;
    failChip.dataset.known=failView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  verificationFailFeedLoaded=options.verificationFailFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        verificationFailFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationFailFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
