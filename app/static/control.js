import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJobFailedAtFeed,lastJobFailedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, jobFailedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const jobFailedAtChip=$("lastJobFailedAtChip");
  if(jobFailedAtChip){
    const feed=state.preview?null:recordedJobFailedAtFeed(eventLog,replayCursor,jobFailedAtFeedLoaded);
    const jobFailedAt=lastJobFailedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    jobFailedAtChip.hidden=jobFailedAt.hidden;
    jobFailedAtChip.textContent=jobFailedAt.hidden?"":jobFailedAt.label;
    jobFailedAtChip.dataset.known=jobFailedAt.known?"true":"false";
    if(jobFailedAt.hidden||!jobFailedAt.title)jobFailedAtChip.removeAttribute("title");
    else jobFailedAtChip.title=jobFailedAt.title;
    if(jobFailedAt.hidden)jobFailedAtChip.removeAttribute("aria-label");
    else jobFailedAtChip.setAttribute("aria-label",jobFailedAt.label);
  }
...
function reset(mission,options={}){
  ...
  jobFailedAtFeedLoaded=options.jobFailedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{jobFailedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
