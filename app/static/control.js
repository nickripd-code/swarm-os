import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedEvidenceAtFeed,lastEvidenceAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, evidenceAtFeedLoaded=false;
...
function renderHud(){
  ...
  const evidenceAtChip=$("lastEvidenceAtChip");
  if(evidenceAtChip){
    const feed=state.preview?null:recordedEvidenceAtFeed(eventLog,replayCursor,evidenceAtFeedLoaded);
    const evidenceAt=lastEvidenceAtView(feed,{visible:!!(state.mission&&!state.preview)});
    evidenceAtChip.hidden=evidenceAt.hidden;
    evidenceAtChip.textContent=evidenceAt.hidden?"":evidenceAt.label;
    evidenceAtChip.dataset.known=evidenceAt.known?"true":"false";
    if(evidenceAt.hidden||!evidenceAt.title)evidenceAtChip.removeAttribute("title");
    else evidenceAtChip.title=evidenceAt.title;
    if(evidenceAt.hidden)evidenceAtChip.removeAttribute("aria-label");
    else evidenceAtChip.setAttribute("aria-label",evidenceAt.label);
  }
...
function reset(mission, options={}){
  ...
  evidenceAtFeedLoaded=options.evidenceAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{evidenceAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
