import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskBlockedAtFeed,lastTaskBlockedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskBlockedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const taskBlockedAtChip=$("lastTaskBlockedAtChip");
  if(taskBlockedAtChip){
    const feed=state.preview?null:recordedTaskBlockedAtFeed(eventLog,replayCursor,taskBlockedAtFeedLoaded);
    const taskBlockedAt=lastTaskBlockedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    taskBlockedAtChip.hidden=taskBlockedAt.hidden;
    taskBlockedAtChip.textContent=taskBlockedAt.hidden?"":taskBlockedAt.label;
    taskBlockedAtChip.dataset.known=taskBlockedAt.known?"true":"false";
    if(taskBlockedAt.hidden||!taskBlockedAt.title)taskBlockedAtChip.removeAttribute("title");
    else taskBlockedAtChip.title=taskBlockedAt.title;
    if(taskBlockedAt.hidden)taskBlockedAtChip.removeAttribute("aria-label");
    else taskBlockedAtChip.setAttribute("aria-label",taskBlockedAt.label);
  }
...
function reset(mission, options={}){
  ...
  taskBlockedAtFeedLoaded=options.taskBlockedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskBlockedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
