import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedOrgChangedAtFeed,lastOrgChangedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, orgChangedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const orgChangedAtChip=$("lastOrgChangedAtChip");
  if(orgChangedAtChip){
    const feed=state.preview?null:recordedOrgChangedAtFeed(eventLog,replayCursor,orgChangedAtFeedLoaded);
    const orgChangedAt=lastOrgChangedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    orgChangedAtChip.hidden=orgChangedAt.hidden;
    orgChangedAtChip.textContent=orgChangedAt.hidden?"":orgChangedAt.label;
    orgChangedAtChip.dataset.known=orgChangedAt.known?"true":"false";
    if(orgChangedAt.hidden||!orgChangedAt.title)orgChangedAtChip.removeAttribute("title");
    else orgChangedAtChip.title=orgChangedAt.title;
    if(orgChangedAt.hidden)orgChangedAtChip.removeAttribute("aria-label");
    else orgChangedAtChip.setAttribute("aria-label",orgChangedAt.label);
  }
...
function reset(mission, options={}){
  ...
  orgChangedAtFeedLoaded=options.orgChangedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{orgChangedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
