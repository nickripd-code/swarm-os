import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationPassAtFeed,lastVerificationPassAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationPassAtFeedLoaded=false;
...
function renderHud(){
  ...
  const verifyPassAtChip=$("lastVerificationPassAtChip");
  if(verifyPassAtChip){
    const feed=state.preview?null:recordedVerificationPassAtFeed(eventLog,replayCursor,verificationPassAtFeedLoaded);
    const verifyPassAt=lastVerificationPassAtView(feed,{visible:!!(state.mission&&!state.preview)});
    verifyPassAtChip.hidden=verifyPassAt.hidden;
    verifyPassAtChip.textContent=verifyPassAt.hidden?"":verifyPassAt.label;
    verifyPassAtChip.dataset.known=verifyPassAt.known?"true":"false";
    if(verifyPassAt.hidden||!verifyPassAt.title)verifyPassAtChip.removeAttribute("title");
    else verifyPassAtChip.title=verifyPassAt.title;
    if(verifyPassAt.hidden)verifyPassAtChip.removeAttribute("aria-label");
    else verifyPassAtChip.setAttribute("aria-label",verifyPassAt.label);
  }
...
function reset(mission, options={}){
  ...
  verificationPassAtFeedLoaded=options.verificationPassAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        verificationPassAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationPassAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
