import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedJobRetryScheduledAtFeed,lastJobRetryScheduledAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, jobRetryScheduledAtFeedLoaded=false;
...
function renderHud(){
  ...
  const jobRetryScheduledAtChip=$("lastJobRetryScheduledAtChip");
  if(jobRetryScheduledAtChip){
    const feed=state.preview?null:recordedJobRetryScheduledAtFeed(eventLog,replayCursor,jobRetryScheduledAtFeedLoaded);
    const jobRetryScheduledAt=lastJobRetryScheduledAtView(feed,{visible:!!(state.mission&&!state.preview)});
    jobRetryScheduledAtChip.hidden=jobRetryScheduledAt.hidden;
    jobRetryScheduledAtChip.textContent=jobRetryScheduledAt.hidden?"":jobRetryScheduledAt.label;
    jobRetryScheduledAtChip.dataset.known=jobRetryScheduledAt.known?"true":"false";
    if(jobRetryScheduledAt.hidden||!jobRetryScheduledAt.title)jobRetryScheduledAtChip.removeAttribute("title");
    else jobRetryScheduledAtChip.title=jobRetryScheduledAt.title;
    if(jobRetryScheduledAt.hidden)jobRetryScheduledAtChip.removeAttribute("aria-label");
    else jobRetryScheduledAtChip.setAttribute("aria-label",jobRetryScheduledAt.label);
  }
...
function reset(mission,options={}){
  ...
  jobRetryScheduledAtFeedLoaded=options.jobRetryScheduledAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        jobRetryScheduledAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{jobRetryScheduledAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
