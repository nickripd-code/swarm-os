import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedActiveVerificationFeed,activeVerificationCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, activeVerificationFeedLoaded=false;
...
function renderHud(){
  ...
  const verifyChip=$("activeVerificationCountChip");
  if(verifyChip){
    const feed=state.preview?null:recordedActiveVerificationFeed(eventLog,replayCursor,activeVerificationFeedLoaded);
    const verifyView=activeVerificationCountView(feed,{visible:!!(state.mission&&!state.preview)});
    verifyChip.hidden=verifyView.hidden;
    verifyChip.textContent=verifyView.label;
    verifyChip.dataset.known=verifyView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  activeVerificationFeedLoaded=options.activeVerificationFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        activeVerificationFeedLoaded=true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{activeVerificationFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
