import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLeaseExpiredAtFeed,lastLeaseExpiredAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, leaseExpiredAtFeedLoaded=false;
...
function renderHud(){
  ...
  const leaseExpiredAtChip=$("lastLeaseExpiredAtChip");
  if(leaseExpiredAtChip){
    const feed=state.preview?null:recordedLeaseExpiredAtFeed(eventLog,replayCursor,leaseExpiredAtFeedLoaded);
    const leaseExpiredAt=lastLeaseExpiredAtView(feed,{visible:!!(state.mission&&!state.preview)});
    leaseExpiredAtChip.hidden=leaseExpiredAt.hidden;
    leaseExpiredAtChip.textContent=leaseExpiredAt.hidden?"":leaseExpiredAt.label;
    leaseExpiredAtChip.dataset.known=leaseExpiredAt.known?"true":"false";
    if(leaseExpiredAt.hidden||!leaseExpiredAt.title)leaseExpiredAtChip.removeAttribute("title");
    else leaseExpiredAtChip.title=leaseExpiredAt.title;
    if(leaseExpiredAt.hidden)leaseExpiredAtChip.removeAttribute("aria-label");
    else leaseExpiredAtChip.setAttribute("aria-label",leaseExpiredAt.label);
  }
...
function reset(mission, options={}){
  ...
  leaseExpiredAtFeedLoaded=options.leaseExpiredAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{leaseExpiredAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
