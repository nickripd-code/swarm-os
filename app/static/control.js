import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolSuccessRateFeed,toolSuccessRateView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolSuccessRateFeedLoaded=false;
...
function renderHud(){
  ...
  const successChip=$("toolSuccessRateChip");
  if(successChip){
    const feed=state.preview?null:recordedToolSuccessRateFeed(eventLog,replayCursor,toolSuccessRateFeedLoaded);
    const successView=toolSuccessRateView(feed,{visible:!!(state.mission&&!state.preview)});
    successChip.hidden=successView.hidden;
    successChip.textContent=successView.label;
    successChip.dataset.known=successView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  toolSuccessRateFeedLoaded=options.toolSuccessRateFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        toolSuccessRateFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolSuccessRateFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
