import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmRetryRateFeed,llmRetryRateView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmRetryRateFeedLoaded=false;
...
function renderHud(){
  ...
  const llmRetryChip=$("llmRetryRateChip");
  if(llmRetryChip){
    const feed=state.preview?null:recordedLlmRetryRateFeed(eventLog,replayCursor,llmRetryRateFeedLoaded);
    const llmRetryView=llmRetryRateView(feed,{visible:!!(state.mission&&!state.preview)});
    llmRetryChip.hidden=llmRetryView.hidden;
    llmRetryChip.textContent=llmRetryView.label;
    llmRetryChip.dataset.known=llmRetryView.known?"true":"false";
  }
...
function reset(mission, options={}){
  stopReplayPlay();
  llmRetryRateFeedLoaded=options.llmRetryRateFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmRetryRateFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
