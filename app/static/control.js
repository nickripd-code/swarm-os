import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedStopAtFeed,lastStopAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, stopAtFeedLoaded=false;
...
function renderHud(){
  ...
  const stopAtChip=$("lastStopAtChip");
  if(stopAtChip){
    const feed=state.preview?null:recordedStopAtFeed(eventLog,replayCursor,stopAtFeedLoaded);
    const stopAt=lastStopAtView(feed,{visible:!!(state.mission&&!state.preview)});
    stopAtChip.hidden=stopAt.hidden;
    stopAtChip.textContent=stopAt.hidden?"":stopAt.label;
    stopAtChip.dataset.known=stopAt.known?"true":"false";
    if(stopAt.hidden||!stopAt.title)stopAtChip.removeAttribute("title");
    else stopAtChip.title=stopAt.title;
    if(stopAt.hidden)stopAtChip.removeAttribute("aria-label");
    else stopAtChip.setAttribute("aria-label",stopAt.label);
  }
...
function reset(mission, options={}){
  ...
  stopAtFeedLoaded=options.stopAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{stopAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
