import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedApprovalQuestionAtFeed,lastApprovalQuestionAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, approvalQuestionAtFeedLoaded=false;
...
function renderHud(){
  ...
  const approvalQuestionAtChip=$("lastApprovalQuestionAtChip");
  if(approvalQuestionAtChip){
    const feed=state.preview?null:recordedApprovalQuestionAtFeed(eventLog,replayCursor,approvalQuestionAtFeedLoaded);
    const approvalQuestionAt=lastApprovalQuestionAtView(feed,{visible:!!(state.mission&&!state.preview)});
    approvalQuestionAtChip.hidden=approvalQuestionAt.hidden;
    approvalQuestionAtChip.textContent=approvalQuestionAt.hidden?"":approvalQuestionAt.label;
    approvalQuestionAtChip.dataset.known=approvalQuestionAt.known?"true":"false";
    if(approvalQuestionAt.hidden||!approvalQuestionAt.title)approvalQuestionAtChip.removeAttribute("title");
    else approvalQuestionAtChip.title=approvalQuestionAt.title;
    if(approvalQuestionAt.hidden)approvalQuestionAtChip.removeAttribute("aria-label");
    else approvalQuestionAtChip.setAttribute("aria-label",approvalQuestionAt.label);
  }
...
function reset(mission, options={}){
  ...
  approvalQuestionAtFeedLoaded=options.approvalQuestionAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        approvalQuestionAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{approvalQuestionAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
