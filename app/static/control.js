import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,missionPrivacy,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive} from "./state.mjs";
...
function renderHud(){
  ...
  if($("privacyChip")){
    const el=$("privacyChip");
    if(!mission||state.preview){
      el.hidden=true;
    }else{
      const view=missionPrivacy(mission);
      el.hidden=false;
      el.dataset.known=view.known?"true":"false";
      el.textContent=view.label;
      el.className="hud-chip privacy "+(view.mode||"unavailable");
    }
  }
