import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedControllerDecisionAtFeed,lastControllerDecisionAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, controllerDecisionAtFeedLoaded=false;
...
function renderHud(){
  ...
  const controllerDecisionAtChip=$("lastControllerDecisionAtChip");
  if(controllerDecisionAtChip){
    const feed=state.preview?null:recordedControllerDecisionAtFeed(eventLog,replayCursor,controllerDecisionAtFeedLoaded);
    const controllerDecisionAt=lastControllerDecisionAtView(feed,{visible:!!(state.mission&&!state.preview)});
    controllerDecisionAtChip.hidden=controllerDecisionAt.hidden;
    controllerDecisionAtChip.textContent=controllerDecisionAt.hidden?"":controllerDecisionAt.label;
    controllerDecisionAtChip.dataset.known=controllerDecisionAt.known?"true":"false";
    if(controllerDecisionAt.hidden||!controllerDecisionAt.title)controllerDecisionAtChip.removeAttribute("title");
    else controllerDecisionAtChip.title=controllerDecisionAt.title;
    if(controllerDecisionAt.hidden)controllerDecisionAtChip.removeAttribute("aria-label");
    else controllerDecisionAtChip.setAttribute("aria-label",controllerDecisionAt.label);
  }
...
function reset(mission, options={}){
  ...
  controllerDecisionAtFeedLoaded=options.controllerDecisionAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        controllerDecisionAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{controllerDecisionAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
