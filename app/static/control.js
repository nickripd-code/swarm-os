import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedEvidenceFeed,evidenceCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, evidenceFeedLoaded=false;
...
function renderHud(){
  ...
  const evidenceChip=$("evidenceCountChip");
  if(evidenceChip){
    const feed=state.preview?null:recordedEvidenceFeed(eventLog,replayCursor,evidenceFeedLoaded);
    const evidenceView=evidenceCountView(feed,{visible:!!(state.mission&&!state.preview)});
    evidenceChip.hidden=evidenceView.hidden;
    evidenceChip.textContent=evidenceView.label;
    evidenceChip.dataset.known=evidenceView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  evidenceFeedLoaded=options.evidenceFeedLoaded===true;
