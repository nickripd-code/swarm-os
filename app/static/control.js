import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolSuccessFeed,toolSuccessCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolSuccessFeedLoaded=false;
...
function renderHud(){
  ...
  const successChip=$("toolSuccessCountChip");
  if(successChip){
    const feed=state.preview?null:recordedToolSuccessFeed(eventLog,replayCursor,toolSuccessFeedLoaded);
    const successView=toolSuccessCountView(feed,{visible:!!(state.mission&&!state.preview)});
...
function reset(mission, options={}){
  ...
  toolSuccessFeedLoaded=options.toolSuccessFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolSuccessFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
