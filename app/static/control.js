import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmAtFeed,lastLlmAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmAtFeedLoaded=false;
...
function renderHud(){
  ...
  const llmAtChip=$("lastLlmAtChip");
  if(llmAtChip){
    const feed=state.preview?null:recordedLlmAtFeed(eventLog,replayCursor,llmAtFeedLoaded);
    const llmAt=lastLlmAtView(feed,{visible:!!(state.mission&&!state.preview)});
    llmAtChip.hidden=llmAt.hidden;
    llmAtChip.textContent=llmAt.hidden?"":llmAt.label;
    llmAtChip.dataset.known=llmAt.known?"true":"false";
    if(llmAt.hidden||!llmAt.title)llmAtChip.removeAttribute("title");
    else llmAtChip.title=llmAt.title;
    if(llmAt.hidden)llmAtChip.removeAttribute("aria-label");
    else llmAtChip.setAttribute("aria-label",llmAt.label);
  }
...
function reset(mission, options={}){
  ...
  llmAtFeedLoaded=options.llmAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
