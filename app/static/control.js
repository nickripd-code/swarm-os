import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,eventCountView} from "./state.mjs";
...
let eventLog=[], eventFeed=null, replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null;
...
function reset(mission){
  stopReplayPlay();
  eventLog=[];eventFeed=null;replayCursor=-1;replayLive=true;sourceMission=mission||null;
...
function renderEventCountChip(){
  const chip=$("eventCountChip");
  if(!chip)return;
  const view=eventCountView(eventFeed,{mission:state.mission,preview:!!state.preview});
  chip.hidden=view.hidden;
  if(view.hidden)return;
  chip.textContent=view.label;
  chip.dataset.known=view.known?"true":"false";
  chip.className="hud-chip event-count"+(view.known?"":" unavailable");
}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m);eventFeed=eventLog;localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
