import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedBudgetWarningCountFeed,budgetWarningCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, budgetWarningCountFeedLoaded=false;
...
function renderHud(){
  ...
  const budgetWarnCountChip=$("budgetWarningCountChip");
  if(budgetWarnCountChip){
    const feed=state.preview?null:recordedBudgetWarningCountFeed(eventLog,replayCursor,budgetWarningCountFeedLoaded);
    const warnCountView=budgetWarningCountView(feed,{visible:!!(state.mission&&!state.preview)});
    budgetWarnCountChip.hidden=warnCountView.hidden;
    budgetWarnCountChip.textContent=warnCountView.label;
    budgetWarnCountChip.dataset.known=warnCountView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  budgetWarningCountFeedLoaded=options.budgetWarningCountFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        budgetWarningCountFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{budgetWarningCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
