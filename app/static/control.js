import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedToolCompletedCountFeed,toolCompletedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, toolCompletedCountFeedLoaded=false;
...
function renderHud(){
  ...
  const toolCompletedChip=$("toolCompletedCountChip");
  if(toolCompletedChip){
    const feed=state.preview?null:recordedToolCompletedCountFeed(eventLog,replayCursor,toolCompletedCountFeedLoaded);
    const toolCompletedView=toolCompletedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    toolCompletedChip.hidden=toolCompletedView.hidden;
    toolCompletedChip.textContent=toolCompletedView.label;
    toolCompletedChip.dataset.known=toolCompletedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  toolCompletedCountFeedLoaded=options.toolCompletedCountFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        toolCompletedCountFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{toolCompletedCountFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
