import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedWaitingAtFeed,lastWaitingAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, waitingAtFeedLoaded=false;
...
function renderHud(){
  ...
  const waitingAtChip=$("lastWaitingAtChip");
  if(waitingAtChip){
    const feed=state.preview?null:recordedWaitingAtFeed(eventLog,replayCursor,waitingAtFeedLoaded);
    const waitingAt=lastWaitingAtView(feed,{visible:!!(state.mission&&!state.preview)});
    waitingAtChip.hidden=waitingAt.hidden;
    waitingAtChip.textContent=waitingAt.hidden?"":waitingAt.label;
    waitingAtChip.dataset.known=waitingAt.known?"true":"false";
    if(waitingAt.hidden||!waitingAt.title)waitingAtChip.removeAttribute("title");
    else waitingAtChip.title=waitingAt.title;
    if(waitingAt.hidden)waitingAtChip.removeAttribute("aria-label");
    else waitingAtChip.setAttribute("aria-label",waitingAt.label);
  }
...
function reset(mission, options={}){
  ...
  waitingAtFeedLoaded=options.waitingAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{waitingAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
