import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedEvidenceStartedCountFeed,evidenceStartedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, evidenceStartedFeedLoaded=false;
...
function renderHud(){
  ...
  const evidenceStartedChip=$("evidenceStartedCountChip");
  if(evidenceStartedChip){
    const feed=state.preview?null:recordedEvidenceStartedCountFeed(eventLog,replayCursor,evidenceStartedFeedLoaded);
    const evidenceView=evidenceStartedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    evidenceStartedChip.hidden=evidenceView.hidden;
    evidenceStartedChip.textContent=evidenceView.hidden?"":evidenceView.label;
    evidenceStartedChip.dataset.known=evidenceView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  evidenceStartedFeedLoaded=options.evidenceStartedFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        evidenceStartedFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{evidenceStartedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
