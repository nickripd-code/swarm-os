import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationFailAtFeed,lastVerificationFailAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationFailAtFeedLoaded=false;
...
function renderHud(){
  ...
  const verifyFailAtChip=$("lastVerificationFailAtChip");
  if(verifyFailAtChip){
    const feed=state.preview?null:recordedVerificationFailAtFeed(eventLog,replayCursor,verificationFailAtFeedLoaded);
    const verifyFailAt=lastVerificationFailAtView(feed,{visible:!!(state.mission&&!state.preview)});
    verifyFailAtChip.hidden=verifyFailAt.hidden;
    verifyFailAtChip.textContent=verifyFailAt.hidden?"":verifyFailAt.label;
    verifyFailAtChip.dataset.known=verifyFailAt.known?"true":"false";
    if(verifyFailAt.hidden||!verifyFailAt.title)verifyFailAtChip.removeAttribute("title");
    else verifyFailAtChip.title=verifyFailAt.title;
    if(verifyFailAt.hidden)verifyFailAtChip.removeAttribute("aria-label");
    else verifyFailAtChip.setAttribute("aria-label",verifyFailAt.label);
  }
...
function reset(mission, options={}){
  ...
  verificationFailAtFeedLoaded=options.verificationFailAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        verificationFailAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationFailAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
