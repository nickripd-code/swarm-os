import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedActiveToolFeed,activeToolCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, activeToolFeedLoaded=false;
...
function renderHud(){
  ...
  const activeChip=$("activeToolCountChip");
  if(activeChip){
    const feed=state.preview?null:recordedActiveToolFeed(eventLog,replayCursor,activeToolFeedLoaded);
    const activeView=activeToolCountView(feed,{visible:!!(state.mission&&!state.preview)});
    activeChip.hidden=activeView.hidden;
    activeChip.textContent=activeView.label;
    activeChip.dataset.known=activeView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  activeToolFeedLoaded=options.activeToolFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{activeToolFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
