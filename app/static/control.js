import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmFailoverRateFeed,llmFailoverRateView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmFailoverRateFeedLoaded=false;
...
function renderHud(){
  ...
  const llmFailoverChip=$("llmFailoverRateChip");
  if(llmFailoverChip){
    const feed=state.preview?null:recordedLlmFailoverRateFeed(eventLog,replayCursor,llmFailoverRateFeedLoaded);
    const llmFailoverView=llmFailoverRateView(feed,{visible:!!(state.mission&&!state.preview)});
    llmFailoverChip.hidden=llmFailoverView.hidden;
    llmFailoverChip.textContent=llmFailoverView.label;
    llmFailoverChip.dataset.known=llmFailoverView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  llmFailoverRateFeedLoaded=options.llmFailoverRateFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        llmFailoverRateFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmFailoverRateFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
