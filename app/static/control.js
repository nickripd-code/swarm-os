import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmFailAtFeed,lastLlmFailAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmFailAtFeedLoaded=false;
...
function renderHud(){
  ...
  const llmFailAtChip=$("lastLlmFailAtChip");
  if(llmFailAtChip){
    const feed=state.preview?null:recordedLlmFailAtFeed(eventLog,replayCursor,llmFailAtFeedLoaded);
    const llmFailAt=lastLlmFailAtView(feed,{visible:!!(state.mission&&!state.preview)});
    llmFailAtChip.hidden=llmFailAt.hidden;
    llmFailAtChip.textContent=llmFailAt.hidden?"":llmFailAt.label;
    llmFailAtChip.dataset.known=llmFailAt.known?"true":"false";
    if(llmFailAt.hidden||!llmFailAt.title)llmFailAtChip.removeAttribute("title");
    else llmFailAtChip.title=llmFailAt.title;
    if(llmFailAt.hidden)llmFailAtChip.removeAttribute("aria-label");
    else llmFailAtChip.setAttribute("aria-label",llmFailAt.label);
  }
...
function reset(mission, options={}){
  ...
  llmFailAtFeedLoaded=options.llmFailAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmFailAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
