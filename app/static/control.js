import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedPlannerProposalAtFeed,lastPlannerProposalAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, plannerProposalAtFeedLoaded=false;
...
function renderHud(){
  ...
  const plannerProposalAtChip=$("lastPlannerProposalAtChip");
  if(plannerProposalAtChip){
    const feed=state.preview?null:recordedPlannerProposalAtFeed(eventLog,replayCursor,plannerProposalAtFeedLoaded);
    const plannerProposalAt=lastPlannerProposalAtView(feed,{visible:!!(state.mission&&!state.preview)});
    plannerProposalAtChip.hidden=plannerProposalAt.hidden;
    plannerProposalAtChip.textContent=plannerProposalAt.hidden?"":plannerProposalAt.label;
    plannerProposalAtChip.dataset.known=plannerProposalAt.known?"true":"false";
    if(plannerProposalAt.hidden||!plannerProposalAt.title)plannerProposalAtChip.removeAttribute("title");
    else plannerProposalAtChip.title=plannerProposalAt.title;
    if(plannerProposalAt.hidden)plannerProposalAtChip.removeAttribute("aria-label");
    else plannerProposalAtChip.setAttribute("aria-label",plannerProposalAt.label);
  }
...
function reset(mission, options={}){
  ...
  plannerProposalAtFeedLoaded=options.plannerProposalAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        plannerProposalAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{plannerProposalAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
