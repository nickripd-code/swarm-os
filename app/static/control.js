import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedFailAtFeed,lastMissionFailAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, failAtFeedLoaded=false;
...
function renderHud(){
  ...
  const failAtChip=$("lastMissionFailAtChip");
  if(failAtChip){
    const feed=state.preview?null:recordedFailAtFeed(eventLog,replayCursor,failAtFeedLoaded);
    const failAt=lastMissionFailAtView(feed,{visible:!!(state.mission&&!state.preview)});
    failAtChip.hidden=failAt.hidden;
    failAtChip.textContent=failAt.hidden?"":failAt.label;
    failAtChip.dataset.known=failAt.known?"true":"false";
    if(failAt.hidden||!failAt.title)failAtChip.removeAttribute("title");
    else failAtChip.title=failAt.title;
    if(failAt.hidden)failAtChip.removeAttribute("aria-label");
    else failAtChip.setAttribute("aria-label",failAt.label);
  }
...
function reset(mission, options={}){
  ...
  failAtFeedLoaded=options.failAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{failAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
