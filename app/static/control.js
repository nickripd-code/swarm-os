import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedUpdatedAtFeed,lastUpdatedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, updatedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const updatedAtChip=$("lastUpdatedAtChip");
  if(updatedAtChip){
    const feed=state.preview?null:recordedUpdatedAtFeed(eventLog,replayCursor,updatedAtFeedLoaded);
    const updatedAt=lastUpdatedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    updatedAtChip.hidden=updatedAt.hidden;
    updatedAtChip.textContent=updatedAt.hidden?"":updatedAt.label;
    updatedAtChip.dataset.known=updatedAt.known?"true":"false";
    if(updatedAt.hidden||!updatedAt.title)updatedAtChip.removeAttribute("title");
    else updatedAtChip.title=updatedAt.title;
    if(updatedAt.hidden)updatedAtChip.removeAttribute("aria-label");
    else updatedAtChip.setAttribute("aria-label",updatedAt.label);
  }
...
function reset(mission, options={}){
  ...
  updatedAtFeedLoaded=options.updatedAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{updatedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
