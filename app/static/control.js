import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedMissionStartedAtFeed,lastMissionStartedAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, missionStartedAtFeedLoaded=false;
...
function renderHud(){
  ...
  const missionStartedAtChip=$("lastMissionStartedAtChip");
  if(missionStartedAtChip){
    const feed=state.preview?null:recordedMissionStartedAtFeed(eventLog,replayCursor,missionStartedAtFeedLoaded);
    const missionStartedAt=lastMissionStartedAtView(feed,{visible:!!(state.mission&&!state.preview)});
    missionStartedAtChip.hidden=missionStartedAt.hidden;
    missionStartedAtChip.textContent=missionStartedAt.hidden?"":missionStartedAt.label;
    missionStartedAtChip.dataset.known=missionStartedAt.known?"true":"false";
    if(missionStartedAt.hidden||!missionStartedAt.title)missionStartedAtChip.removeAttribute("title");
    else missionStartedAtChip.title=missionStartedAt.title;
    if(missionStartedAt.hidden)missionStartedAtChip.removeAttribute("aria-label");
    else missionStartedAtChip.setAttribute("aria-label",missionStartedAt.label);
  }
...
function reset(mission,options={}){
  ...
  missionStartedAtFeedLoaded=options.missionStartedAtFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        missionStartedAtFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{missionStartedAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
