import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationStartedFeed,verificationStartedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationStartedFeedLoaded=false;
...
function renderHud(){
  ...
  const startedChip=$("verificationStartedCountChip");
  if(startedChip){
    const feed=state.preview?null:recordedVerificationStartedFeed(eventLog,replayCursor,verificationStartedFeedLoaded);
    const startedView=verificationStartedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    startedChip.hidden=startedView.hidden;
    startedChip.textContent=startedView.label;
    startedChip.dataset.known=startedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  verificationStartedFeedLoaded=options.verificationStartedFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        verificationStartedFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationStartedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
