import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedLlmOkAtFeed,lastLlmOkAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, llmOkAtFeedLoaded=false;
...
function renderHud(){
  ...
  const llmOkAtChip=$("lastLlmOkAtChip");
  if(llmOkAtChip){
    const feed=state.preview?null:recordedLlmOkAtFeed(eventLog,replayCursor,llmOkAtFeedLoaded);
    const llmOkAt=lastLlmOkAtView(feed,{visible:!!(state.mission&&!state.preview)});
    llmOkAtChip.hidden=llmOkAt.hidden;
    llmOkAtChip.textContent=llmOkAt.hidden?"":llmOkAt.label;
    llmOkAtChip.dataset.known=llmOkAt.known?"true":"false";
    if(llmOkAt.hidden||!llmOkAt.title)llmOkAtChip.removeAttribute("title");
    else llmOkAtChip.title=llmOkAt.title;
    if(llmOkAt.hidden)llmOkAtChip.removeAttribute("aria-label");
    else llmOkAtChip.setAttribute("aria-label",llmOkAt.label);
  }
...
function reset(mission, options={}){
  ...
  llmOkAtFeedLoaded=options.llmOkAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{llmOkAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
