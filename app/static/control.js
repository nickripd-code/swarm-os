import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationPassedFeed,verificationPassedCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationPassedFeedLoaded=false;
...
function renderHud(){
  ...
  const passedChip=$("verificationPassedCountChip");
  if(passedChip){
    const feed=state.preview?null:recordedVerificationPassedFeed(eventLog,replayCursor,verificationPassedFeedLoaded);
    const passedView=verificationPassedCountView(feed,{visible:!!(state.mission&&!state.preview)});
    passedChip.hidden=passedView.hidden;
    passedChip.textContent=passedView.label;
    passedChip.dataset.known=passedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  verificationPassedFeedLoaded=options.verificationPassedFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        verificationPassedFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationPassedFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
