import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedAgentKilledCountFeed,agentKilledCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, agentKilledFeedLoaded=false;
...
function renderHud(){
  ...
  const killedChip=$("agentKilledCountChip");
  if(killedChip){
    const feed=state.preview?null:recordedAgentKilledCountFeed(eventLog,replayCursor,agentKilledFeedLoaded);
    const killedView=agentKilledCountView(feed,{visible:!!(state.mission&&!state.preview)});
    killedChip.hidden=killedView.hidden;
    killedChip.textContent=killedView.hidden?"":killedView.label;
    killedChip.dataset.known=killedView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  agentKilledFeedLoaded=options.agentKilledFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{agentKilledFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
