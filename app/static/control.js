import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedVerificationPassFeed,verificationPassCountView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, verificationPassFeedLoaded=false;
...
function renderHud(){
  ...
  const passChip=$("verificationPassCountChip");
  if(passChip){
    const feed=state.preview?null:recordedVerificationPassFeed(eventLog,replayCursor,verificationPassFeedLoaded);
    const passView=verificationPassCountView(feed,{visible:!!(state.mission&&!state.preview)});
    passChip.hidden=passView.hidden;
    passChip.textContent=passView.label;
    passChip.dataset.known=passView.known?"true":"false";
  }
...
function reset(mission, options={}){
  ...
  verificationPassFeedLoaded=options.verificationPassFeedLoaded===true;
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(events)){
        ...
        verificationPassFeedLoaded=true;
      }
    }catch{}
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{verificationPassFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
