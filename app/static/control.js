import {newState,applyEvent,layoutTree,terminal,alertFromEvent,resultMetaText,missionMode,costHudView,formatUsd,tokenTotal,parseCommand,resolveCommand,killRoutePresent,recordEvent,projectEvents,replayView,stepReplay,clampReplayIndex,isReplayLive,recordedTaskPendingAtFeed,lastTaskPendingAtView} from "./state.mjs";
...
let eventLog=[], replayCursor=-1, replayLive=true, replayTimer=null, sourceMission=null, taskPendingAtFeedLoaded=false;
...
function renderHud(){
  ...
  const taskPendingAtChip=$("lastTaskPendingAtChip");
  if(taskPendingAtChip){
    const feed=state.preview?null:recordedTaskPendingAtFeed(eventLog,replayCursor,taskPendingAtFeedLoaded);
    const taskPendingAt=lastTaskPendingAtView(feed,{visible:!!(state.mission&&!state.preview)});
    taskPendingAtChip.hidden=taskPendingAt.hidden;
    taskPendingAtChip.textContent=taskPendingAt.hidden?"":taskPendingAt.label;
    taskPendingAtChip.dataset.known=taskPendingAt.known?"true":"false";
    if(taskPendingAt.hidden||!taskPendingAt.title)taskPendingAtChip.removeAttribute("title");
    else taskPendingAtChip.title=taskPendingAt.title;
    if(taskPendingAt.hidden)taskPendingAtChip.removeAttribute("aria-label");
    else taskPendingAtChip.setAttribute("aria-label",taskPendingAt.label);
  }
...
function reset(mission, options={}){
  ...
  taskPendingAtFeedLoaded=options.taskPendingAtFeedLoaded===true;
...
$("missionForm").addEventListener("submit",async e=>{
  ...
  try{
    ...
    disconnect();reset(m,{taskPendingAtFeedLoaded:true});localStorage.setItem("swarm.mission",m.id);connect(m.id,generation);
