...
export const PAUSE_COUNT_UNAVAILABLE = "unavailable";
...
const PAUSE_COUNT_EVENT = "mission.paused";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedPauseCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded mission.paused events on the loaded feed.
 * The shell pause path is exactly that type (applyEvent). Other lifecycle
 * updates are ignored unless the type matches. Hidden with no mission or in
 * preview. A missing or unreadable feed is PAUSES unavailable, not 0. An
 ...
 */
export function pauseCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "PAUSES " + PAUSE_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === PAUSE_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "PAUSES " + count};
}
