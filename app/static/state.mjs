...
export const MODEL_CALLS_UNAVAILABLE = "unavailable";
...
/** Prefix of a loaded event log. Missing feed stays null — never an invented []. */
export function recordedModelCallFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded model calls. Runtime model_call emits one llm.started.
 * Completions, failures, retries, and failovers are outcomes of that call, not extra calls.
 * Hidden with no mission or in preview. Missing feed is unavailable, not 0.
 * An explicit empty list (including a cursor before any event) is 0.
 */
export function modelCallCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "CALLS " + MODEL_CALLS_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "llm.started") count += 1;
  }
  return {hidden: false, known: true, count, label: "CALLS " + count};
}
