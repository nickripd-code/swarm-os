...
export const LLM_FAILS_UNAVAILABLE = "unavailable";
...
export function recordedLlmFailFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded llm.failed events.
 ...
 */
export function llmFailCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "LLM FAILS " + LLM_FAILS_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "llm.failed") count += 1;
  }
  return {hidden: false, known: true, count, label: "LLM FAILS " + count};
}
