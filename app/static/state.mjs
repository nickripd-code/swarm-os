...
export const TASK_COMPLETED_COUNT_UNAVAILABLE = "unavailable";
const TASK_COMPLETED_COUNT_EVENT = "task.completed";
...
export function recordedTaskCompletedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded task.completed events on the loaded feed.
 * The runtime emits that catalog type when a specialist task finishes.
 * Other events are ignored unless the type matches. Hidden with no mission
 * or in preview. A missing or unreadable feed is TASKS unavailable, not 0.
 ...
 * count only — never spend. mission.completed is not a task completion.
 */
export function taskCompletedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "TASKS " + TASK_COMPLETED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === TASK_COMPLETED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "TASKS " + count};
}
