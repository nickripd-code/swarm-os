...
export const MISSION_RUNNING_COUNT_UNAVAILABLE = "unavailable";
const MISSION_RUNNING_COUNT_EVENT = "mission.running";
...
export function recordedMissionRunningCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded mission.running events on the loaded feed.
 * Catalog EventType.MISSION_RUNNING. mission.started, mission.resumed,
 * mission.resume_requested, mission.waiting, mission.paused, task.started,
 * llm.started, tool.started, and verification.started are ignored. Hidden
 * with no mission or in preview. A missing or unreadable feed is MISSION
 * RUNNING unavailable, not 0. An explicit empty list (including a cursor
 * before any event) is 0. A non-string event_type is unreadable and fails
 * closed. The label is a count only — never a timestamp or spend.
 * job.started, worker.heartbeat, and task.suspended are not catalog events
 * and are not invented. The status chip still reflects current mission status.
 */
export function missionRunningCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "MISSION RUNNING " + MISSION_RUNNING_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === MISSION_RUNNING_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "MISSION RUNNING " + count};
}
