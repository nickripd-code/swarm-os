...
export const VERIFICATION_FAIL_UNAVAILABLE = "unavailable";
...
export function recordedVerificationFailFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded verification.failed events.
 ...
 * Evidence-step failures, passes, and mission failure events are not counted.
 */
export function verificationFailCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: VERIFICATION_FAIL_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "verification.failed") count += 1;
  }
