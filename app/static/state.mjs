...
export const LLM_OK_UNAVAILABLE = "unavailable";
...
export function recordedLlmSuccessFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded llm.completed events.
 ...
 */
export function llmSuccessCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "LLM OK " + LLM_OK_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "llm.completed") count += 1;
  }
  return {hidden: false, known: true, count, label: "LLM OK " + count};
}
