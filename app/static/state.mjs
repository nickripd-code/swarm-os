...
export const VERIFICATION_PASS_RATE_UNAVAILABLE = "unavailable";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedVerificationPassRateFeed(log, cursor, loaded) {
...
function formatVerificationPassRatePercent(percent) {
...
/**
 * Read-only verification.passed / completed attempts from the loaded shell feed.
 * A completed attempt is EventType VERIFICATION_PASSED or VERIFICATION_FAILED
 * ("verification.passed" or "verification.failed").
 * verification.started and verification.evidence.* are not attempts.
 * Hidden with no mission, in preview, or when no completed attempt is visible yet.
 * A missing feed is unavailable, not 0%.
 */
export function verificationPassRateView(feed, options = {}) {
  const visible = options.visible === true;
  const empty = {hidden: true, known: false, passed: null, fails: null, attempts: null, rate: null, label: ""};
  if (!visible) return empty;
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, passed: null, fails: null, attempts: null, rate: null, label: "VERIFICATION PASS RATE " + VERIFICATION_PASS_RATE_UNAVAILABLE};
  }
  let passed = 0;
  let fails = 0;
  for (const event of feed) {
    if (!event || typeof event !== "object") continue;
    if (event.event_type === "verification.passed") passed += 1;
    else if (event.event_type === "verification.failed") fails += 1;
  }
  const attempts = passed + fails;
  if (attempts === 0) {
    return {hidden: true, known: false, passed: 0, fails: 0, attempts: 0, rate: null, label: ""};
  }
  const rate = passed / attempts;
  const percent = formatVerificationPassRatePercent(rate * 100);
  if (percent === null) {
    return {hidden: false, known: false, passed, fails, attempts, rate: null, label: "VERIFICATION PASS RATE " + VERIFICATION_PASS_RATE_UNAVAILABLE};
  }
  return {hidden: false, known: true, passed, fails, attempts, rate, label: "VERIFICATION PASS RATE " + percent};
}
