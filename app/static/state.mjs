...
export const TOOL_CALLS_UNAVAILABLE = "unavailable";
...
export function recordedToolFeed(log, cursor, loaded) {
...
/**
 * Read-only count of charged tool calls. Matches tool_calls_used: tool.started only.
 * Hidden with no mission/preview. Missing feed is unavailable, not 0.
 * An explicit empty list (including a cursor before any event) is 0.
 */
export function toolCallCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "TOOLS " + TOOL_CALLS_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "tool.started") count += 1;
  }
