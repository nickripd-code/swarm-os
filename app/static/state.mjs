...
export const MISSION_STARTED_COUNT_UNAVAILABLE = "unavailable";
const MISSION_STARTED_COUNT_EVENT = "mission.started";
...
export function recordedMissionStartedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded mission.started events on the loaded feed.
 * Catalog EventType.MISSION_STARTED. mission.resumed, mission.running,
 * mission.resume_requested, mission.paused, task.started, llm.started,
 * tool.started, and verification.started are ignored. Hidden with no mission
 * or in preview. A missing or unreadable feed is MISSION STARTED unavailable,
 * not 0. An explicit empty list (including a cursor before any event) is 0.
 * A non-string event_type is unreadable and fails closed. The label is a
 * count only — never a timestamp or spend. job.started, worker.heartbeat,
 * and task.suspended are not catalog events and are not invented.
 */
export function missionStartedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "MISSION STARTED " + MISSION_STARTED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === MISSION_STARTED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "MISSION STARTED " + count};
}
