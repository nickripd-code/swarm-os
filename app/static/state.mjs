...
export const AGENT_MESSAGE_COUNT_UNAVAILABLE = "unavailable";
const AGENT_MESSAGE_COUNT_EVENT = "agent.message";
export function recordedAgentMessageCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded agent.message events on the loaded feed.
 * The runtime emits that catalog type for a spawn/replace assignment and for
 * a worker result sent to the controller. Other events are ignored unless the
 * type matches. Hidden with no mission or in preview. A missing or unreadable
 * feed is AGENT MESSAGES unavailable, not 0. An explicit empty list (including
 * a cursor before any event) is 0. A non-string event_type is unreadable and
 * fails closed. The label is a count only — never spend.
 */
export function agentMessageCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "AGENT MESSAGES " + AGENT_MESSAGE_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === AGENT_MESSAGE_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "AGENT MESSAGES " + count};
}
