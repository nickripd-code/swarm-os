...
export const TOOL_FAILS_UNAVAILABLE = "unavailable";
...
export function recordedToolFailFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded tool.failed events.
 ...
 */
export function toolFailCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "FAILS " + TOOL_FAILS_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "tool.failed") count += 1;
  }
