...
export const LLM_FAILOVER_RATE_UNAVAILABLE = "unavailable";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedLlmFailoverRateFeed(log, cursor, loaded) {
...
function formatLlmFailoverRatePercent(percent) {
...
/**
 * Read-only llm.failover / completed attempts from the loaded shell feed.
 * A completed attempt is llm.failed or llm.completed. llm.started and llm.retry
 * are neither failovers nor attempts. Hidden with no mission, in preview, or
 * when no completed attempt is visible yet. A missing feed is unavailable, not 0%.
 */
export function llmFailoverRateView(feed, options = {}) {
  const visible = options.visible === true;
  const empty = {hidden: true, known: false, failovers: null, completed: null, failed: null, attempts: null, rate: null, label: ""};
  if (!visible) return empty;
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, failovers: null, completed: null, failed: null, attempts: null, rate: null, label: "LLM FAILOVER RATE " + LLM_FAILOVER_RATE_UNAVAILABLE};
  }
  let failovers = 0;
  let completed = 0;
  let failed = 0;
  for (const event of feed) {
    ...
    if (event.event_type === "llm.failover") failovers += 1;
  ...
  if (attempts === 0) {
    return {hidden: true, known: false, failovers, completed, failed, attempts: 0, rate: null, label: ""};
  }
  const rate = failovers / attempts;
  const percent = formatLlmFailoverRatePercent(rate * 100);
  if (percent === null) {
    return {hidden: false, known: false, failovers, completed, failed, attempts, rate: null, label: "LLM FAILOVER RATE " + LLM_FAILOVER_RATE_UNAVAILABLE};
  }
  return {hidden: false, known: true, failovers, completed, failed, attempts, rate, label: "LLM FAILOVER RATE " + percent};
}
