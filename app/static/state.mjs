...
export const FAIL_COUNT_UNAVAILABLE = "unavailable";
...
const FAIL_COUNT_EVENT = "mission.failed";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedFailCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded mission.failed events on the loaded feed.
 * Catalog mission failure is exactly that type (alertFromEvent). Other
 * events are ignored unless the type matches. Hidden with no mission or in
 * preview. A missing or unreadable feed is FAILS unavailable, not 0. An
 ...
 */
export function failCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "FAILS " + FAIL_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === FAIL_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "FAILS " + count};
}
