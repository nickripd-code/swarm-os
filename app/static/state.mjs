...
export const LLM_FAILOVERS_UNAVAILABLE = "unavailable";
...
export function recordedLlmFailoverFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded llm.failover events.
 ...
 */
export function llmFailoverCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "LLM FAILOVERS " + LLM_FAILOVERS_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "llm.failover") count += 1;
  }
  return {hidden: false, known: true, count, label: "LLM FAILOVERS " + count};
}
