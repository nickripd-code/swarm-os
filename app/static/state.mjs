...
export const JOB_COMPLETED_COUNT_UNAVAILABLE = "unavailable";
const JOB_COMPLETED_COUNT_EVENT = "job.completed";
...
export function recordedJobCompletedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded job.completed events on the loaded feed.
 * SwarmRuntime emits that catalog type when a durable work item completes.
 * job.enqueued (open PR #272), last-job-completed-at (open PR #274),
 * job.failed, job.retry_scheduled, lease.released, tool.completed,
 * mission.completed, and task.completed are ignored. Hidden with no mission
 * or in preview. A missing or unreadable feed is JOB COMPLETED unavailable,
 ...
 * count only — never a timestamp or spend.
 */
export function jobCompletedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "JOB COMPLETED " + JOB_COMPLETED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === JOB_COMPLETED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "JOB COMPLETED " + count};
}
