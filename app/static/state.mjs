...
export const EVIDENCE_STARTED_COUNT_UNAVAILABLE = "unavailable";
const EVIDENCE_STARTED_COUNT_EVENT = "verification.evidence.started";
...
export function recordedEvidenceStartedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded verification.evidence.started events on the loaded feed.
 * Catalog EventType.VERIFICATION_EVIDENCE_STARTED. verification.evidence.passed,
 * verification.evidence.failed, verification.started, verification.passed,
 * verification.failed, tool.started, and mission.started are ignored. Hidden
 * with no mission or in preview. A missing or unreadable feed is EVIDENCE
 * STARTED unavailable, not 0. An explicit empty list (including a cursor
 * before any event) is 0. A non-string event_type is unreadable and fails
 * closed. The label is a count only — never a timestamp or spend. Payload
 * count fields are not summed. job.started, worker.heartbeat, and
 * task.suspended are not catalog events and are not invented.
 */
export function evidenceStartedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "EVIDENCE STARTED " + EVIDENCE_STARTED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === EVIDENCE_STARTED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "EVIDENCE STARTED " + count};
}
