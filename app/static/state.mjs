...
export const VERIFICATION_FAIL_RATE_UNAVAILABLE = "unavailable";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedVerificationFailRateFeed(log, cursor, loaded) {
...
function formatVerificationFailRatePercent(percent) {
...
/**
 * Read-only verification.failed / completed attempts from the loaded shell feed.
 * A completed attempt is verification.failed or verification.passed.
 ...
 */
export function verificationFailRateView(feed, options = {}) {
  const visible = options.visible === true;
  const empty = {hidden: true, known: false, fails: null, passed: null, attempts: null, rate: null, label: ""};
  if (!visible) return empty;
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, fails: null, passed: null, attempts: null, rate: null, label: "VERIFICATION FAIL RATE " + VERIFICATION_FAIL_RATE_UNAVAILABLE};
  }
  let fails = 0;
  let passed = 0;
  for (const event of feed) {
    if (!event || typeof event !== "object") continue;
    if (event.event_type === "verification.failed") fails += 1;
    else if (event.event_type === "verification.passed") passed += 1;
  }
  const attempts = fails + passed;
  if (attempts === 0) {
    return {hidden: true, known: false, fails: 0, passed: 0, attempts: 0, rate: null, label: ""};
  }
  const rate = fails / attempts;
  const percent = formatVerificationFailRatePercent(rate * 100);
  if (percent === null) {
    return {hidden: false, known: false, fails, passed, attempts, rate: null, label: "VERIFICATION FAIL RATE " + VERIFICATION_FAIL_RATE_UNAVAILABLE};
  }
  return {hidden: false, known: true, fails, passed, attempts, rate, label: "VERIFICATION FAIL RATE " + percent};
}
