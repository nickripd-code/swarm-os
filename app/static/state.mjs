...
export const WAITING_COUNT_UNAVAILABLE = "unavailable";
...
const WAITING_COUNT_EVENT = "mission.waiting";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedWaitingCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded mission.waiting events on the loaded feed.
 * Same event type as the waiting-age clock: only event_type mission.waiting.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * WAITING unavailable, not 0. An explicit empty list (including a cursor
 * before any event) is 0. A non-string event_type is unreadable and fails
 * closed. The label is a count only — never spend.
 */
export function waitingCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "WAITING " + WAITING_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === WAITING_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "WAITING " + count};
}
