...
export const JUDGE_DECISION_UNAVAILABLE = "unavailable";
const JUDGE_DECISION_EVENT = "judge.decision";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedJudgeDecisionFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded judge.decision events on the loaded feed.
 * Multi-planner synthesis emits that catalog type once per judged decide.
 * controller.decision, planner.proposal, org.changed, agent.message,
 * llm.completed, and mission.completed are not judge decisions. Payload
 * action fields and numeric counters are not a count.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * JUDGE DECISIONS unavailable, not 0. An explicit empty list (including a
 * cursor before any event) is 0. A non-string event_type is unreadable and
 * fails closed. The label is a count only.
 */
export function judgeDecisionCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "JUDGE DECISIONS " + JUDGE_DECISION_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === JUDGE_DECISION_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "JUDGE DECISIONS " + count};
}
