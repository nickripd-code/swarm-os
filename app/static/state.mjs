...
export const TASK_PENDING_COUNT_UNAVAILABLE = "unavailable";
const TASK_PENDING_COUNT_EVENT = "task.pending";
...
export function recordedTaskPendingCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded task.pending events on the loaded feed.
 * The runtime emits that catalog type when a specialist task is created pending.
 * Other events are ignored unless the type matches. Hidden with no mission
 * or in preview. A missing or unreadable feed is PENDING unavailable, not 0.
 ...
 * count only — never spend. task.completed and status:pending payloads are not pending tasks.
 */
export function taskPendingCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "PENDING " + TASK_PENDING_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === TASK_PENDING_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "PENDING " + count};
}
