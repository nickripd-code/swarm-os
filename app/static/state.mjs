...
export const AGENT_KILLED_COUNT_UNAVAILABLE = "unavailable";
const AGENT_KILLED_COUNT_EVENT = "agent.killed";
...
export function recordedAgentKilledCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded agent.killed events on the loaded feed.
 * Human per-agent kill emits that catalog type. Other events are ignored
 * unless the type matches. Hidden with no mission or in preview. A missing
 * or unreadable feed is KILLS unavailable, not 0. An explicit empty list
 ...
 */
export function agentKilledCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "KILLS " + AGENT_KILLED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === AGENT_KILLED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "KILLS " + count};
}
