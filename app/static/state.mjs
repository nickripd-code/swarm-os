...
export const TOOL_COMPLETED_COUNT_UNAVAILABLE = "unavailable";
const TOOL_COMPLETED_COUNT_EVENT = "tool.completed";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedToolCompletedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded tool.completed events on the loaded feed.
 * The catalog value is exactly tool.completed. tool.started, tool.failed,
 * mission.completed, task.completed, llm.completed, and job.completed are
 * not tool completions. Hidden with no mission or in preview.
 * A missing or unreadable feed is TOOLS unavailable, not 0. An explicit
 ...
 */
export function toolCompletedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "TOOLS " + TOOL_COMPLETED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === TOOL_COMPLETED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "TOOLS " + count};
}
