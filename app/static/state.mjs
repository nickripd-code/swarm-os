...
export const AGENT_UPDATED_COUNT_UNAVAILABLE = "unavailable";
const AGENT_UPDATED_COUNT_EVENT = "agent.updated";
...
export function recordedAgentUpdatedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded agent.updated events on the loaded feed.
 * The runtime emits that catalog type when an agent row changes. Other events
 * are ignored unless the type matches. Hidden with no mission or in preview.
 * A missing or unreadable feed is UPDATED unavailable, not 0. An explicit
 ...
 */
export function agentUpdatedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "UPDATED " + AGENT_UPDATED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === AGENT_UPDATED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "UPDATED " + count};
}
