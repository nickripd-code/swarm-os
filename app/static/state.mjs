...
export const VERIFICATION_STARTED_UNAVAILABLE = "unavailable";
...
const VERIFICATION_STARTED_EVENT = "verification.started";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedVerificationStartedFeed(log, cursor, loaded) {
...
/**
 * Read-only count of verification.started events on the loaded feed.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * VERIFICATION STARTED unavailable, not 0. An explicit empty list (including a
 * cursor before any event) is 0. A non-string event_type is unreadable and fails closed.
 * Evidence events, passes, failures, and tool or model starts are not counted.
 * The label is a count only — never spend.
 */
export function verificationStartedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "VERIFICATION STARTED " + VERIFICATION_STARTED_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === VERIFICATION_STARTED_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "VERIFICATION STARTED " + count};
}
