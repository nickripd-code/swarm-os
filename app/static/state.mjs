...
function sameVerification(start, finished) {
...
function takeMatchingVerificationStart(open, finished) {
  for (let i = open.length - 1; i >= 0; i--) {
    if (sameVerification(open[i], finished)) return open.splice(i, 1)[0];
  }
  return null;
}
/**
 * Read-only mean duration of verification.passed and verification.failed
 * outcomes on the loaded shell feed. Distinct from newest-only
 * last-verification-at. An explicit non-negative latency_ms / duration_ms /
 * elapsed_ms wins. Otherwise the span is the recorded created_at delta from
 * the matching verification.started. Hidden with no mission, in preview, or
 * when no completed verification outcome is visible yet. A missing feed, or
 * any completed outcome without an honest duration, is unavailable — never a
 * partial mean and never an invented 0ms.
 */
export function avgVerificationLatencyView(feed, options = {}) {
  ...
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, meanMs: null, label: AVG_VERIFICATION_LATENCY_LABEL + AVG_VERIFICATION_LATENCY_UNAVAILABLE};
  }
  const open = [];
  const durations = [];
  let completed = 0;
  ...
  for (const event of feed) {
    ...
    const passedOutcome = event.event_type === "verification.passed";
    const failedOutcome = event.event_type === "verification.failed";
    if (!passedOutcome && !failedOutcome) continue;
    const started = takeMatchingVerificationStart(open, event);
    completed += 1;
    const explicit = explicitVerificationDuration(verificationPayload(event));
  ...
  if (unreadable || durations.length !== completed) {
    return {hidden: false, known: false, count: completed, meanMs: null, label: AVG_VERIFICATION_LATENCY_LABEL + AVG_VERIFICATION_LATENCY_UNAVAILABLE};
  }
