...
export const QUESTION_COUNT_UNAVAILABLE = "unavailable";
...
const QUESTION_COUNT_EVENT = "mission.question";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedQuestionFeed(log, cursor, loaded) {
...
/**
 * Read-only count of mission.question events on the loaded feed.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * QUESTIONS unavailable, not 0. An explicit empty list (including a cursor
 * before any event) is 0. A non-string event_type is unreadable and fails closed.
 * Answers, answer consumption, and waiting or pause events are not questions.
 * The label is a count only — never spend or question text.
 */
export function questionCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "QUESTIONS " + QUESTION_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === QUESTION_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "QUESTIONS " + count};
}
