...
export const LLM_RETRIES_UNAVAILABLE = "unavailable";
...
export function recordedLlmRetryFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded llm.retry events.
 ...
 */
export function llmRetryCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "LLM RETRIES " + LLM_RETRIES_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "llm.retry") count += 1;
  }
  return {hidden: false, known: true, count, label: "LLM RETRIES " + count};
}
