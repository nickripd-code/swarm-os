...
export const TOOL_SUCCESS_UNAVAILABLE = "unavailable";
...
export function recordedToolSuccessFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded tool.completed events.
 ...
 */
export function toolSuccessCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "OK " + TOOL_SUCCESS_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    if (event && event.event_type === "tool.completed") count += 1;
  }
  return {hidden: false, known: true, count, label: "OK " + count};
}
