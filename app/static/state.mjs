...
export const AGENT_SPAWNED_COUNT_UNAVAILABLE = "unavailable";
const AGENT_SPAWNED_COUNT_EVENT = "agent.spawned";
...
export function recordedAgentSpawnedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded agent.spawned events on the loaded feed.
 * SwarmRuntime.spawn emits that catalog type. Other events are ignored
 * unless the type matches. Hidden with no mission or in preview. A missing
 * or unreadable feed is SPAWNED unavailable, not 0. An explicit empty list
 ...
 */
export function agentSpawnedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "SPAWNED " + AGENT_SPAWNED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === AGENT_SPAWNED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "SPAWNED " + count};
}
