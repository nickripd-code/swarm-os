...
export const TASK_BLOCKED_COUNT_UNAVAILABLE = "unavailable";
const TASK_BLOCKED_COUNT_EVENT = "task.blocked";
export function recordedTaskBlockedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded task.blocked events on the loaded feed.
 * The runtime emits that catalog type when a worker result status is blocked
 * and the task row is saved as blocked. Controller action "blocked" emits
 * mission.blocked instead and is not counted. Other events are ignored unless
 * the type matches. Hidden with no mission or in preview. A missing or
 * unreadable feed is TASK BLOCKS unavailable, not 0. An explicit empty list
 ...
 */
export function taskBlockedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "TASK BLOCKS " + TASK_BLOCKED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === TASK_BLOCKED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "TASK BLOCKS " + count};
}
