 ...
 * shell log. Companion to the task-started count. mission.started,
 * llm.started, tool.started, verification.started, and other task states
 * do not set this clock. Hidden with no mission, in preview, or when no
 * task.started is visible yet. A missing feed, or a visible start whose
 * own stamp is unreadable, is LAST TASK START unavailable. The newest
 * valid event time wins. The log-order last task.started must itself have
 * a valid stamp. Never invents spend.
 */
