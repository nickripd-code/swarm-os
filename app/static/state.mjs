...
export const PAYMENT_COUNT_UNAVAILABLE = "unavailable";
...
const PAYMENT_COUNT_EVENT = "payment.created";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedPaymentFeed(log, cursor, loaded) {
...
/**
 * Read-only count of payment.created events on the loaded feed.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * PAYMENTS unavailable, not 0. An explicit empty list (including a cursor
 * before any event) is 0. A non-string event_type is unreadable and fails closed.
 * The label is a count only — never an amount or spend figure.
 */
export function paymentCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "PAYMENTS " + PAYMENT_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === PAYMENT_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "PAYMENTS " + count};
}
