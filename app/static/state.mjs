...
export const LEASE_EXPIRED_COUNT_UNAVAILABLE = "unavailable";
const LEASE_EXPIRED_COUNT_EVENT = "lease.expired";
...
export function recordedLeaseExpiredCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded lease.expired events on the loaded feed.
 * SwarmRuntime emits that catalog type when an expired worker or job lease is reclaimed.
 * lease.claimed, lease.released, and job events are ignored.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * LEASE EXPIRED unavailable, not 0. An explicit empty list (including a cursor
 * before any event) is 0. A non-string event_type is unreadable and fails
 * closed. The label is a count only — never spend.
 */
export function leaseExpiredCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "LEASE EXPIRED " + LEASE_EXPIRED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === LEASE_EXPIRED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "LEASE EXPIRED " + count};
}
