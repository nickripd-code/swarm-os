...
export const JOB_ENQUEUED_COUNT_UNAVAILABLE = "unavailable";
const JOB_ENQUEUED_COUNT_EVENT = "job.enqueued";
...
export function recordedJobEnqueuedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded job.enqueued events on the loaded feed.
 * SwarmRuntime emits that catalog type when a durable work item is enqueued.
 * job.completed, job.failed, job.retry_scheduled, and lease events are ignored.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * JOB ENQUEUED unavailable, not 0. An explicit empty list (including a cursor
 * before any event) is 0. A non-string event_type is unreadable and fails
 * closed. The label is a count only — never spend.
 */
export function jobEnqueuedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "JOB ENQUEUED " + JOB_ENQUEUED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === JOB_ENQUEUED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "JOB ENQUEUED " + count};
}
