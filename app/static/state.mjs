...
export const STOP_COUNT_UNAVAILABLE = "unavailable";
...
const STOP_COUNT_EVENT = "mission.stopped";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedStopCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded mission.stopped events on the loaded feed.
 * Same stop type as last-stop-at / stop-age, and the type applyEvent treats
 * as a mission stop. task.stopped and other lifecycle updates are ignored.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * STOPS unavailable, not 0. An explicit empty list (including a cursor before
 * any event) is 0. A non-string event_type is unreadable and fails closed.
 * The label is a count only — never spend.
 */
export function stopCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "STOPS " + STOP_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === STOP_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "STOPS " + count};
}
