...
export const SUSPEND_COUNT_UNAVAILABLE = "unavailable";
const SUSPEND_COUNT_EVENT = "mission.suspended";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedSuspendCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded mission.suspended events on the loaded feed.
 * Same catalog type as last-suspend-at and suspend-age. Pause, resume,
 * stop, park, fail, waiting, and complete are ignored unless the type is
 * exactly mission.suspended. Hidden with no mission or in preview. A missing
 * or unreadable feed is SUSPENDS unavailable, not 0. An explicit empty list
 ...
 */
export function suspendCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "SUSPENDS " + SUSPEND_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === SUSPEND_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "SUSPENDS " + count};
}
