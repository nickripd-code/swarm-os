...
export const AGENT_RETIRED_COUNT_UNAVAILABLE = "unavailable";
const AGENT_RETIRED_COUNT_EVENT = "agent.retired";
...
export function recordedAgentRetiredCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded agent.retired events on the loaded feed.
 * OrganizationDesigner retire emits that catalog type. Other events are ignored
 * unless the type matches. Hidden with no mission or in preview. A missing
 * or unreadable feed is RETIRED unavailable, not 0. An explicit empty list
 ...
 */
export function agentRetiredCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "RETIRED " + AGENT_RETIRED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === AGENT_RETIRED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "RETIRED " + count};
}
