...
export const RESUME_REQUESTED_COUNT_UNAVAILABLE = "unavailable";
const RESUME_REQUESTED_COUNT_EVENT = "mission.resume_requested";
...
export function recordedResumeRequestedCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded mission.resume_requested events on the loaded feed.
 * Catalog EventType.MISSION_RESUME_REQUESTED. mission.resumed, mission.started,
 * mission.running, mission.waiting, mission.paused, mission.completed,
 * mission.failed, and user.answered are ignored. Hidden with no mission or in
 * preview. A missing or unreadable feed is RESUME REQUESTED unavailable, not 0.
 * An explicit empty list (including a cursor before any event) is 0. A
 * non-string event_type is unreadable and fails closed. The label is a count
 * only — never a timestamp or spend.
 */
export function resumeRequestedCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "RESUME REQUESTED " + RESUME_REQUESTED_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === RESUME_REQUESTED_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "RESUME REQUESTED " + count};
}
