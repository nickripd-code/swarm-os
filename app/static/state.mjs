...
export const AGENT_REPARENT_COUNT_UNAVAILABLE = "unavailable";
const AGENT_REPARENT_COUNT_EVENT = "agent.reparented";
...
export function recordedAgentReparentCountFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded agent.reparented events on the loaded feed.
 * OrganizationDesigner emits that catalog type when a specialist changes parent.
 * org.changed (including op reparent) and other agent events are ignored.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * REPARENTS unavailable, not 0. An explicit empty list (including a cursor
 * before any event) is 0. A non-string event_type is unreadable and fails
 * closed. The label is a count only — never spend.
 */
export function agentReparentCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "REPARENTS " + AGENT_REPARENT_COUNT_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === AGENT_REPARENT_COUNT_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "REPARENTS " + count};
}
