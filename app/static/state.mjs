...
export const PENDING_TOOLS_UNAVAILABLE = "unavailable";
/**
 ...
 */
export function recordedPendingToolFeed(log, cursor, loaded) {
...
function pendingToolChargeKey(payload) {
...
function unknownPendingTools() {
  return {hidden: false, known: false, count: null, label: PENDING_TOOLS_UNAVAILABLE};
}
/**
 * Read-only pending / in-flight tool count: tool.started still unmatched by a
 * later tool.completed or tool.failed with the same tool name and charge counter.
 * The catalog has no tool.requested event, so a start that has not closed is the
 * only honest pending tool. Hidden with an empty label when no mission is visible.
 * Missing feed is unavailable, never 0. An explicit empty list is a known 0.
 * payload.used is a match key only — it is never displayed as the count.
 */
export function pendingToolCountView(feed, options = {}) {
  if (options.visible !== true) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) return unknownPendingTools();
  const open = new Set();
  ...
  for (const event of feed) {
    ...
    const key = pendingToolChargeKey(event.payload);
    if (type === "tool.started") {
      if (!key || started.has(key)) return unknownPendingTools();
      started.add(key);
      open.add(key);
      continue;
    }
