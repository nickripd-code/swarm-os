...
export const PLANNER_PROPOSAL_UNAVAILABLE = "unavailable";
const PLANNER_PROPOSAL_EVENT = "planner.proposal";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedPlannerProposalFeed(log, cursor, loaded) {
...
/**
 * Read-only count of recorded planner.proposal events on the loaded feed.
 * Multi-planner decide emits that catalog type once per proposal. judge.decision,
 * controller.decision, org.changed, agent.message, llm.completed, and
 * mission.completed are not planner proposals. Payload proposal fields and
 * numeric counters are not a count.
 * Hidden with no mission or in preview. A missing or unreadable feed is
 * PLANNER PROPOSALS unavailable, not 0. An explicit empty list (including a
 * cursor before any event) is 0. A non-string event_type is unreadable and
 * fails closed. The label is a count only.
 */
export function plannerProposalCountView(feed, options = {}) {
  const visible = options.visible === true;
  if (!visible) return {hidden: true, known: false, count: null, label: ""};
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "PLANNER PROPOSALS " + PLANNER_PROPOSAL_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === PLANNER_PROPOSAL_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "PLANNER PROPOSALS " + count};
}
