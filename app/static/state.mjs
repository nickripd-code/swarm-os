...
export const LLM_START_UNAVAILABLE = "unavailable";
...
const LLM_STARTED_EVENT = "llm.started";
/** Prefix of a loaded event log. A missing feed stays null — never an invented []. */
export function recordedLlmStartedFeed(log, cursor, loaded) {
 ...
 * LLM START unavailable, not 0. An explicit empty list (including a cursor
...
export function llmStartedCountView(feed, options = {}) {
  ...
  if (!Array.isArray(feed)) {
    return {hidden: false, known: false, count: null, label: "LLM START " + LLM_START_UNAVAILABLE};
  }
  let count = 0;
  for (const event of feed) {
    ...
    if (type === LLM_STARTED_EVENT) count += 1;
  }
  return {hidden: false, known: true, count, label: "LLM START " + count};
}
