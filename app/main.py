...
async def create_mission(request: MissionCreate):
    if not runtime.controller.configured():
        raise HTTPException(503, "No model provider is configured. Set OPENAI_API_KEY, OPENROUTER_API_KEY, XAI_API_KEY, ANTHROPIC_API_KEY, MISTRAL_API_KEY, GEMINI_API_KEY, COHERE_API_KEY, DEEPSEEK_API_KEY, or OLLAMA_MODEL / OLLAMA_BASE_URL on the server before launching.")
